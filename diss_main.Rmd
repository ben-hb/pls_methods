...
SI_model_amended2 <- '
                ...
                # mediation
                   set.seed(1000)
                    prem <- rnorm(10)
                    le <- 0.5*prem + rnorm(10)
                    deftrap <- 0.7*le + rnorm(10)
                    dat <- data.frame(prem, le, deftrap, estMethod = "standard", bootstrap =
                          1000, test = TRUE, ci = T, ciWidth = 95, pm = FALSE, paths = T, 
                          label = FALSE, estPlot = FALSE)
                    le_med <- med(dat, dep = "deftrap", pred = "prem", med = "le")

                # moderation
                    deftrap <- rnorm(10)
                    mmtsm <- rnorm(10)
                    deftrap_mmtsm <- deftrap*mmtsm
                    si<- 0.7*deftrap + 0.1*mmtsm + 4.2*deftrap_mmtsm + rnorm(10)
                    dat2 <- data.frame(deftrap, mmtsm, si)
                    mmtsm_mod <- mod(dat2, dep = "si", pred = "deftrap", mod = "mmtsm")
                    
                # structural model
                    deftrap ~ prem + prem_med
                    si ~ deftrap + mmtsm_mod
'
# inf, sexual_2, satisfied_overall

fit_train_a2 <- sem(SI_model_amended2, data=results_tr_com1, ordered=T)
summary_train_a2 <- summary(fit_train_a2, fit.measures=TRUE, standardized = TRUE, rsquare=T)
fitMeasures(fit_train_a2, fit.measures = "all")
parameterestimates(fit_train_a2, boot.ci.type = "bca.simple", standardized = TRUE) 
...
semPaths(fit_test_20, layout = "tree")
...
## Kolmogorov-Smirnov Tests between samples
```{r ks_tests}
# training v. testing
ks.test(testdf.v26$age, traindf.v28$age)
ks.test(testdf.v26$si_type, traindf.v28$si_type)
ks.test(testdf.v26$gender_tgd, traindf.v28$gender_tgd)
ks.test(testdf.v26$sexual, traindf.v28$sexual)
ks.test(testdf.v26$race, traindf.v28$race)
ks.test(testdf.v26$dx_mh, traindf.v28$dx_mh)

# 18-19 v. 20-25
ks.test(test20.v26$age, test18.v26$age)
ks.test(test20.v26$si_type, test18.v26$si_type)
ks.test(test20.v26$gender_tgd, test18.v26$gender_tgd)
ks.test(test20.v26$sexual, test18.v26$sexual)
ks.test(test20.v26$race, test18.v26$race)
ks.test(test20.v26$dx_mh, test18.v26$dx_mh)
```
