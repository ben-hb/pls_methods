...
clm_dia <- clm2(si_type ~ age + sexual + gender_noncis + finpast + sib_freq + body_sr +
                       dx_dep + dx_bi + dx_adhd + meds_count, data = clm_dia)
summary(m_dia)
coef(clm_dia)
vcov(clm_dia)

## NOT WORKING: race + + meds_dep + ed_any +  meds_anx + meds_mood + meds_sle + + dx_pers + dx_tr 
...
m_env <- clm2(si_type ~ discrim + satisfied_overall + ther_vis + talk + activ_yn + international + 
                ther_vis + divers + + military + school2_type + residenc + env_mh, data = clm_env)
summary(m_env)

#  NOT WORKING: + religios 
...
clm_le <- data.v19 %>% select(si_type, ins_cover, abuse_year, gpa_sr, aca_impa, fincur, assault_phys, 
                              binge_fr, assault_sex, assault_emo, psyhx, drugs_yn)

clm_le <- NA_preproc(clm_le)
clm_le <- as.data.frame(clm_le)

clm_le <- clm2(si_type ~ ins_cover + abuse_year + gpa_sr + aca_impa + fincur + assault_phys + binge_fr +
               assault_sex + assault_emo + + psyhx + drugs_yn, data = clm_le)
summary(clm_le)
...
clm_def <- clm2(si_type ~ aca_impa + dep_mod + dep_sev, data = clm_def)
summary(clm_def)
...
data.v22 <- data.v22 %>%
  ...
  mutate(anx_sev = case_when(anx_score > 15 ~ 1,
                             anx_score < 16 ~ 0))
# Check outputs
table(data.v22$anx_none, useNA = "always")
table(data.v22$anx_mild, useNA = "always")
table(data.v22$anx_mod, useNA = "always")
table(data.v22$anx_sev, useNA = "always")
...
data_tsm <- data.v23 %>% select(persist, anx_none, anx_mild, anx_mod, anx_sev, dep_secret, achieve4, AAQ_tot)
...
##EFA TSM
```{r EFA TSM}
data_tsm <- data.v23 %>% select(persist, anx_mild, anx_mod, anx_sev, dep_secret, achieve4, AAQ_tot)
...
results_tsm <- dummy_cols(data_tsm, select_columns = c("persist", "anx_mild", "anx_mod", "anx_sev", "dep_secret", "achieve4", "AAQ_tot"),
                         remove_first_dummy = TRUE)
