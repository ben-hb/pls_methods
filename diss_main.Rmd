...
table(traindf.v11$medther, useNA = "always")
```
...
clm_env <- traindf.v15 %>% select(si_type, religios, residenc, activ_no, school2_type, 
                                  international, discrim)
clm_env <- NA_preproc(clm_env) %>% as.data.frame()

# run ordinal regression
clm_env2 <- clm(si_type ~ religios + residenc + activ_no + school2_type + discrim,
                data = clm_env)
...
clm_le <- traindf.v19 %>% select(si_type, ins_no, gpa_sr, assault, binge, drugs_yn, drug_mar, psyhx)
clm_le <- na.omit(clm_le)

# run regression
clm_le <- clm(si_type ~ ins_no + gpa_sr + assault + binge + drugs_yn + drug_mar + psyhx,
              data = clm_le)
summary(clm_le)
coef(clm_le)
...
clm_prem <- traindf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                 religios, activ_no, school2_type, ins_no, residenc, medther,
                                 gpa_sr, assault, drugs_yn, drug_mar, psyhx)
...
clm_prem <- clm(si_type ~ age + sexual + gender_tgd + sib_freq + race + ed_any + dx_mh +
                ...
                drugs_yn + drug_mar + psyhx + medther,
              data = clm_prem)
summary(clm_prem)
...
var_names <- c(
  ...
  # environment
  "religios", "school2_type", "activ_no", "residenc", "discrim",
  # life events
...
traindf_sem1 <- traindf.v28 %>% select(
  ...
  age, sexual, gender_tgd, sib_freq, ed_any, dx_mh, medther,
  # environment
  religios, school2_type, activ_no, residenc,
  # life events
  ins_no, gpa_sr, binge, psyhx, drugs_yn, assault, drug_mar,
  # defeat
  ...
  dep_impa, percneed,
  # motivational moderators
  inf, flourish_dum, belong1, satisfied_overall, mh_stigma, brs_dum)  #mh_stigma, 
...
SI_model_tr1 <- '
                # measurement model
                  ...
                  prem =~  sib_freq + sexual_2 + sexual_3 + gender_tgd +
                           ...
                           residenc_3 + residenc_4 +gpa_sr + binge + psyhx +
...
SI_model_trM <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual_2 + sexual_3 + gender_tgd + medther +
                           ed_any + dx_mh + religios  +activ_no + ins_no + 
                           gpa_sr + binge + psyhx + drugs_yn + assault + drug_mar
