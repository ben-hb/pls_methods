...
# need to figure out where these are from....
table(data.v9$dep_or_a_all)
table(data.v9$dep_or_anx)
table(data.v9$dep_any)
table(data.v9$anx_any)

# likely frontrunners for eval
table(data.v9$dx_bi)
table(data.v9$dx_ea)
table(data.v9$dx_tr)
table(data.v9$dx_adhd)
...
data_dia <- data.v12 %>% select(age, sexual, gender_noncis, finpast, sib_freq, race, disab_1, ed_scoff, body_sr,
...
results_dia <- dummy_cols(data_dia, select_columns = c("age", "sexual", "gender_noncis", "finpast", "sib_freq", "race", "ed_any",
