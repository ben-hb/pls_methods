...
# create si_type var
  mutate(si_type = case_when((sui_idea == 1 & suic_plan == 1) ~ 3,                   # active SI
...
traindf_dia <- traindf.v12 %>% select(age, sexual, gender_tgd, finpast, sib_freq, race, ed_scoff, body_sr,
                                ...
                                meds_sle, meds_oth, meds_ther, meds_count)
...
traindf_dia <- traindf.v12 %>% select(age, sexual, gender_tgd, finpast, sib_freq, race, ed_scoff, body_sr,
                                dx_mh, meds_count, ed_any, meds_ther)
...
traindf_env <- traindf.v15 %>% select(discrim, religios, residenc, activ_no, divers, 
                                      school2_type, international, satisfied_overall)
...
clm_env <- traindf.v15 %>% select(si_type, religios, residenc, activ_no, school2_type, satisfied_overall)
clm_env <- NA_preproc(clm_env) %>% as.data.frame()

# run ordinal regression
clm_env2 <- clm(si_type ~ religios + residenc + activ_no + school2_type + satisfied_overall, 
                data = clm_env)
...
traindf_prem <- traindf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       ...
                                       gpa_sr, assault, drugs_yn, drug_mar, psyhx, satisfied_overall)
...
traindf_prem <- traindf.v19 %>% select(age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       ...
                                       gpa_sr, assault, drugs_yn, drug_mar, psyhx, fincur, satisfied_overall)
...
results_prem <- dummy_cols(traindf_prem, select_columns = c("age", "sexual", "gender_tgd", "sib_freq",
                                                            "race", "ed_any", "dx_mh", "meds_ther",
                                                            "religios", "satisfied_overall",
...
clm_prem <- clm(si_type ~ age + sexual + gender_tgd + sib_freq + race + ed_any + dx_mh +
                ...
                drugs_yn + drug_mar + psyhx + meds_ther + fincur + satisfied_overall,
              data = clm_prem)
summary(clm_prem)
...
traindf_def <- traindf.v21 %>% select(aca_impa, anx_score, deprawsc)
...
traindf_tsm <- traindf.v23 %>% select(persist, aaq_tot)
...
results_tsm <- dummy_cols(traindf_tsm, select_columns = c("persist", "aaq_tot"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
```{r train CLM TSM}
# prepare dataframe for regression
clm_tsm <- traindf.v23 %>% select(si_type, persist, aaq_tot)
clm_tsm <- na.omit(clm_tsm)

# run regression
clm_tsm <- clm(si_type ~ persist + aaq_tot, data = clm_tsm) 
...
traindf_mm <- traindf.v27 %>% select(inf, flourish, belong1, mh_stigma, brs_tot)
...
### EFA MM
```{r train EFA MM}
traindf_mm <- traindf.v27 %>% select(inf, flourish, belong1, mh_stigma)
...
clm_mm <- traindf.v27 %>% select(si_type, inf, flourish, belong1, mh_stigma)
clm_mm <- na.omit(clm_mm)

# run MM regression
clm_mm <- clm(si_type ~ inf + flourish + belong1 + mh_stigma, data = clm_mm)
...
var_names <- c(
  ...
  # environment
  "religios", "activ_no", "satisfied_overall",
  # life events
...
traindf_sem <- traindf.v28 %>% select(
  ...
  si_type,
  # diathesis
  age, sexual, sib_freq, ed_any, dx_mh, meds_ther,
  # environment
  religios, activ_no, satisfied_overall,
  # life events
  ins_no, gpa_sr, psyhx, fincur, drugs_yn, assault,
  # defeat
  aca_impa, anx_score, deprawsc,
  # threats to self-moderation
  persist, aaq_tot,
  # entrapment
  dep_impa, percneed,
...
results_tr_com1 <- dummy_cols(traindf_sem1, select_columns = c("si_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
SI_model_tr1 <- '
                # measurement model
                  ...
                  si =~    si_type_3 + si_type_2
    
                  prem =~  sib_freq + sexual + gender_tgd +
                           ed_any + dx_mh + activ_no + ins_no + meds_ther + gpa_sr + 
                           psyhx + drugs_yn + assault + satisfied_overall
...
SI_model_trM <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual + gender_tgd + meds_ther +
                           ed_any + dx_mh + activ_no + ins_no + 
                           gpa_sr + psyhx + drugs_yn + assault + satisfied_overall
...
SI_model_trM2 <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual_2 + sexual_3 + gender_tgd + assault +
                           ed_any + dx_mh + psyhx + drugs_yn + meds_ther + satisfied_overall
...
traindf_sem1 <- traindf.v28 %>% select(
  ...
  si_type,
  # diathesis
  age, sexual, sib_freq, ed_any, dx_mh, meds_ther, religios, activ_no, satisfied_overall,
  # life events
  ins_no, gpa_sr, psyhx, drugs_yn, assault,
  # defeat
  aca_impa, deprawsc, anx_score, dep_impa, percneed,
  # MM+ TSM
  inf, flourish, persist, aaq_tot) 
...
# prepare data for future age comparison
```{r 18-19 v 20-25}
...
table(testdf.v14$residenc, useNA = "always")  # check output
```
...
jarque.bera.test(testdf.v14$satisfied_overall)
```
...
# preparing dataframe for regression
clm_env <- testdf.v15 %>% select(si_type, religios, residenc, activ_no, 
                                 satisfied_overall, school2_type)
clm_env <- NA_preproc(clm_env) %>% as.data.frame()

# run ordinal regression
clm_env2 <- clm(si_type ~ religios + residenc + activ_no + satisfied_overall + school2_type, 
                data = clm_env)
...
```{r test results_prem}
# Define diathesis vars for correlation matrix
testdf_prem <- testdf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, 
                                     dx_mh, meds_ther, religios, activ_no, school2_type, satisfied_overall,
                                     fincur, ins_no, residenc, gpa_sr, assault, drugs_yn, 
                                     drug_mar, psyhx)
...
```{r test CLM defeat}
# prepare regression analysis
results2_def <- testdf.v21 %>% select(si_type, aca_impa, deprawsc, anx_score)
clm_def <- na.omit(results2_def)
...
```{r test CLM TSM}
# prepare dataframe for regression
clm_tsm <- testdf.v23 %>% select(si_type, persist, aaq_tot)
clm_tsm <- na.omit(clm_tsm)
...
```{r test CLM mm}
# prepare data for regression
clm_mm <- testdf.v27 %>% select(si_type, inf, flourish, belong1, mh_stigma)
clm_mm <- na.omit(clm_mm)
...
testdf_sem <- testdf.v26 %>% select(
  ...
  si_type,
  # diathesis
  age, sexual, sib_freq, ed_any, dx_mh, meds_ther, undercls,
  # environment
  religios, activ_no, satisfied_overall,
  # life events
  ins_no, gpa_sr, psyhx, fincur, drugs_yn, assault,
...
results_test_sem1 <- dummy_cols(testdf_sem, select_columns = c(
    ...
    "si_type",
    # diathesis
    "age", "sexual", "sib_freq", "ed_any", "dx_mh", "meds_count", "meds_ther", "undercls",
    # environment
    "religios", "activ_no", "satisfied_overall",
    # life events
    "ins_no", "gpa_sr", "psyhx", "fincur", "drugs_yn", "assault",
...
```{r SI_test_new}
# Prep SEM data vars
test_sem <- testdf.v26 %>% select(
  # outcome var
  si_type,
  # premotivation
  age, sexual, sib_freq, ed_any, dx_mh, meds_ther, religios, activ_no, satisfied_overall, undercls,
  # life events
  ins_no, gpa_sr, psyhx, drugs_yn, assault,
  # defeat
  aca_impa, dep_type, anx_type, dep_impa, percneed,
  # MM+ TSM
  inf, flourish_dum, persist, aaq_dum) 

# create sem matrix 
test_sem <- data.matrix(test_sem)
  
## Dummy col SEM w/ continuous
results_test_com <- dummy_cols(test_sem, select_columns = c("si_type", "sexual"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
#view(results_tr_com1)
SI_model_test <- '
                # measurement model
                    si =~      si_type_4 + si_type_3 + si_type_2
    
                    prem =~    meds_ther + dx_mh + sib_freq + sexual_3 + satisfied_overall
                    
                    le =~      psyhx + assault + drugs_yn
                          
                    deftrap =~ dep_type + dep_impa + anx_type + aca_impa + percneed
                              
                    mmtsm =~   aaq_dum + flourish_dum + persist 
                    
                # structural model
                    si ~ deftrap + mmtsm + mmtsm*deftrap
                    deftrap ~ prem + le   '

fit_test <- sem(SI_model_test, data=results_test_com, ordered=T)
summary_test <- summary(fit_test, fit.measures=TRUE, standardized = TRUE, rsquare=T)
fitMeasures(fit_test, fit.measures = "all")
parameterestimates(fit_test, boot.ci.type = "bca.simple", standardized = TRUE) 
modificationindices(fit_test, minimum.value = 20)
semPaths(fit_test, layout = "tree2")

# anova(fit_train_a, fit_test) 

# save output for offline SMART PLS
results_test_com <- as.data.frame(results_test_com)
save(results_test_com, file = "results_test_com.RData")
load("results_test_com.RData")
write_xlsx(results_test_com, path = "results_test_com.xls", col_names = TRUE, format_headers = TRUE)
```
...
## Dummy col SEM w/ continuous
results_test_com18 <- dummy_cols(test18_sem, select_columns = c("si_type", "sexual"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
#view(results_tr_com1)
SI_model_1819 <- '
                # measurement model
...
fit_test_18 <- sem(SI_model_1819, data=results_test_com18, ordered=T)
summary_test_18 <- summary(fit_test_18, fit.measures=TRUE, standardized = TRUE, rsquare=T)
fitMeasures(fit_test_18, fit.measures = "all")
parameterestimates(fit_test_18, boot.ci.type = "bca.simple", standardized = TRUE) 
modificationindices(fit_test_18, minimum.value = 20)

semPaths(fit_train_a, layout = "tree")
...
test20_sem <- test20.v26 %>% select(
  ...
  si_type,
  # diathesis
  age, sexual, sib_freq, ed_any, dx_mh, meds_ther, religios, activ_no,
  # life events
  ins_no, gpa_sr, psyhx, drugs_yn, assault,
  # defeat
  aca_impa, dep_type, anx_type, dep_impa, percneed,
  # MM+ TSM
  inf, flourish_dum, satisfied_overall, persist, aaq_dum) 

# create sem matrix 
test20_sem <- data.matrix(test20_sem)
  
## Dummy col SEM w/ continuous
results_test_com20 <- dummy_cols(test20_sem, select_columns = c("si_type", "sexual"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
#view(results_tr_com1)
SI_model_20 <- '
                # measurement model
                    ...
                    prem =~    meds_ther + dx_mh + sib_freq + sexual_3 
...
fit_test_20 <- sem(SI_model_20, data=results_test_com20, ordered=T)
summary_test_20 <- summary(fit_test_20, fit.measures=TRUE, standardized = TRUE, rsquare=T)
fitMeasures(fit_test_20, fit.measures = "all")
parameterestimates(fit_test_20, boot.ci.type = "bca.simple", standardized = TRUE) 
modificationindices(fit_test_20, minimum.value = 20)
semPaths(fit_test_20, layout = "tree", edge.label.cex = 1, fade = FALSE)
...
anova(fit_test_20, fit_test_18) 
# xlsx output
# save(test20_sem_test, file = "test20_sem_test.RData")
# load("test20_sem_test.RData")
# write_xlsx(test20_sem_test, path = "test20_sem_test.xls", col_names = TRUE, format_headers = TRUE)
...
# training v. testing
wilcox.test(testdf.v26B$age,traindf.v28B$age, alternative = "two.sided") # W = 15938984, p-value = 0.4527
wilcox.test(testdf.v26B$si_type,traindf.v28B$si_type, alternative = "two.sided") # W = 15882104, p-value = 0.08518
wilcox.test(testdf.v26B$gender_tgd,traindf.v28B$gender_tgd, alternative = "two.sided") # W = 16002839, p-value = 0.23
wilcox.test(testdf.v26B$sexual, traindf.v28B$sexual, alternative = "two.sided") # W = 15992592, p-value = 0.5647
wilcox.test(testdf.v26B$race, traindf.v28B$race, alternative = "two.sided") # W = 16081642, p-value = 0.9284
wilcox.test(testdf.v26B$dx_mh, traindf.v28B$dx_mh, alternative = "two.sided") # W = 16022528, p-value = 0.7558

# 18-19 v. 20-25
wilcox.test(test20.v26B$age, test18.v26B$age, alternative = "two.sided") # W = 7098356, p-value < 2.2e-16
wilcox.test(test20.v26B$si_type, test18.v26B$si_type, alternative = "two.sided") # W = 3475826, p-value = 0.0384
wilcox.test(test20.v26B$gender_tgd, test18.v26B$gender_tgd, alternative = "two.sided") # W = 3530267, p-value = 0.2836
wilcox.test(test20.v26B$sexual, test18.v26B$sexual, alternative = "two.sided") # W = 3371776, p-value = 4.786e-05
wilcox.test(test20.v26B$race, test18.v26B$race, alternative = "two.sided") # W = 3590905, p-value = 0.399
wilcox.test(test20.v26B$dx_mh, test18.v26B$dx_mh, alternative = "two.sided") # W = 3686756, p-value = 0.005003
``` 
