...
clm_mod <- glm(si_type ~ (age + sexual + gender_noncis + finpast + sib_freq + body_sr + dx_tr + race + dx_anx +
                        ed_any + dx_pers + dx_anx + dx_dep + dx_bi + dx_adhd + meds_count)^2, 
               data = dia_mod)
summary(clm_mod)
coef(clm_mod)
...
clm_env <- glm(si_type ~ (discrim + satisfied_overall + ther_vis + talk + activ_yn + international + religios +
                ther_vis + divers + military + school2_type + residenc + env_mh)^2, data = clm_env)
summary(clm_env)
coef(clm_env)
...
clm_le <- glm(si_type ~ (ins_cover + trauma_year + gpa_sr + aca_impa + fincur + binge_fr + drug_mar + psyhx + drugs_yn)^2, 
              data = clm_le)
summary(clm_le)
coef(clm_le)
...
clm_def <- glm(si_type ~ (aca_impa + deprawsc)^2, data = clm_def)
...
clm_tsm <- glm(si_type ~ (persist + anx_score + dep_secret + aaq_dum)^2, data = clm_tsm)
...
clm_entrap <- glm(si_type ~ (dep_impa + gad7_impa + percneed_cur)^2, data = clm_entrap)
