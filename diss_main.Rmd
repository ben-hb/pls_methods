...
data.v3$gender_noncis <- data.v3$gender

# create dichotomous var for non-cisgender v. cisgender
data.v3 <- data.v3 %>%
  mutate(gender_noncis = case_when("gender diverse" == 1 ~ 1,
                                  "transgender" == 1 ~ 1,
                                  "cis-gender" == 1 ~ 0))
data.v3$gender_noncis <- as.numeric(data.v3$gender_noncis)
table(data.v3$gender_noncis)
summary(data.v3$gender_noncis)
...
```{r drug_type}
## add summary of each
table(data.v16$drug_any)
table(data.v16$drug_none)
table(data.v16$drug_mar)  
table(data.v16$drug_coc)
table(data.v16$drug_her)
table(data.v16$drug_met)
table(data.v16$drug_stim)
table(data.v16$drug_ect)
table(data.v16$drug_opi)
table(data.v16$drug_other)
```
...
```{r}
data_le <- data.v19 %>% select(ins_cover, abuse_year, gpa_sr, aca_impa, fincur, assault_phys, 
                               binge_fr, assault_sex, assault_emo, deprawsc, psyhx, drug_any)
...
results_le <- dummy_cols(data_le, select_columns = c("ins_cover", "abuse_year", "gpa_sr", "aca_impa", 
                                                     ...
                                                     "assault_emo", "deprawsc", "psyhx", "drug_any"),
                         remove_first_dummy = TRUE)
...
drug_any, assault_sex, assault_phys, assault_emo, isi_tot
```{r clm_LE}
