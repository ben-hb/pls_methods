...
## EFA LE
```{r train le_pca}
traindf_le <- traindf.v19 %>% select(ins_no, gpa_sr, fincur, 
...
results_prem <- dummy_cols(traindf_prem, select_columns = c("age", "sexual", "gender_tgd", "sib_freq",
                                                            ...
                                                            "gpa_sr", "fincur", "assault", "binge", "drugs_yn",
...
clm_prem <- clm(si_type ~ age + sexual + gender_tgd + sib_freq + race + ed_any + dx_mh + meds_count +
                ...
                binge + drugs_yn + drug_mar + psyhx + international,
              data = clm_prem)
summary(clm_prem)
coef(clm_prem)
...
traindf.v26 <- traindf.v26 %>% 
  ...
  mutate(talk = case_when((talk1_9 == 1 | talk1no == 1) ~ 1,                                    # no support
                          ...
                          (talk1_1 == 1 | talk1_6 == 1 | talk1_7 == 1 | talk1_8 == 1) ~ 2)) %>%  # formal and informal support,
...
traindf_si <- na.omit(traindf_si)

# create corr matrix, with dummy vars 
traindf_si <- traindf_si %>% 
  mutate_if(is.numeric, as.factor) %>% 
  dummy_cols(remove_first_dummy = TRUE) %>% 
  data.matrix()
traindf_si   # check output
results_si <- dummy_cols(traindf_prem, select_columns = c(
    # diathesis
    "age", "sexual", "gender_tgd", "sib_freq", "ed_any", "dx_mh", "meds_count",
    # environment
    "religios", "school2_type", "activ_no", "international", "residenc",
    ...
    "inf", "flourish_dum", "belong1", "satisfied_overall", "mh_stigma"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
si_pca <- PCA(results_si, graph=FALSE)  # apply PCA
eigval_si <- get_eigenvalue(si_pca)
eigval_si

# Visualization of PCA, FAMD
fviz_screeplot(si_pca, addlabels = TRUE, ylim = c(0, 50))
fviz_pca_var(si_pca,
             col.var = "contrib", # Color by contributions to the PC
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE     # Avoid text overlapping
             )
fviz_famd_var(si_pca,
             col.var = "contrib", # Color by contributions to the PC
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE     # Avoid text overlapping
             )
fviz_cos2(si_pca, choice = "var", axes = 1, 
          fill = "steelblue", color = "steelblue",
          sort.val = c("desc", "asc", "none"),
          top = 40,
          xtickslab.rt = 45,
...
summary(clm_si)
coef_si <- coef(summary(clm_si))

ci_si <- confint(clm_si)
ci_si

model.si <- tidy(clm_si)  # Convert model to dataframe for easy manipulation
model.si
...
SI_model_tr1 <- '
                # measurement model
                  ...
                  prem =~  sib_freq + sexual_2 + sexual_3 + gender_tgd +  
                           ed_any + dx_mh + meds_count + activ_no + ins_no + 
