...
data.v3$gender[data.v3$gender_text %in% c("Literally no clue as of late", "Kynigender", "Adrogynous", "Hermaphrodite", "idk yet", 
                                          ...
                                          "I don't know, still trying to purse it out", "Undecided")] <- 1
...
table(data.v3$gender_noncis, useNA = "always")   # check output
...
data_dia <- data.v12 %>% select(age, sexual, gender_noncis, finpast, sib_freq, race, ed_scoff, body_sr,
                                dx_dep, dx_any, dx_bi, dx_adhd, dx_ea, dx_tr, meds_count, ed_any,
...
data_dia <- data.v12 %>% select(age, sexual, gender_noncis, finpast, sib_freq, race, body_sr,
                                dx_dep, dx_bi, dx_adhd, dx_ea, dx_tr, dx_pers, meds_dep,
                                meds_anx, meds_mood, meds_sle, meds_count)
...
data_dia <- data.v12 %>% select(age, sexual, gender_noncis, finpast, sib_freq, race, body_sr,
                                dx_dep, dx_bi, dx_adhd, dx_tr, dx_pers, meds_dep, ed_any,
                                meds_anx, meds_mood, meds_sle, meds_count)
...
results_dia <- dummy_cols(data_dia, select_columns = c("age", "sexual", "gender_noncis", "finpast", "sib_freq", "race",
                                                       "body_sr", "dx_dep", "dx_bi", "dx_adhd", "dx_tr",
                                                       "ed_any", "dx_pers", "meds_dep","meds_anx", "meds_mood",
                                                       "meds_sle", "meds_count"), 
...
clm_dia <- data.v12 %>% select(si_type, age, sexual, gender_noncis, finpast, sib_freq, race, body_sr,
                                dx_dep, dx_bi, dx_adhd, dx_tr, dx_pers, meds_dep, ed_any,
                                meds_anx, meds_mood, meds_sle, meds_count)
...
m_dia <- clm(si_type ~ age +sexual + gender_noncis + finpast + sib_freq + race + body_sr +
                       dx_dep + dx_bi + dx_adhd + dx_tr + dx_pers + meds_dep + ed_any +
                       meds_anx + meds_mood + meds_sle + meds_count, data = train_dia)
summary(m_dia)
...
data_env <- data.v15 %>% select(discrim, religios, env_mh, residenc, ther_vis, activ_yn, divers, school2_type,
...
```{r drug_type}
## add summary of each
table(data.v16$drug_none, useNA="always")
table(data.v16$drug_mar, useNA="always")  
table(data.v16$drug_coc, useNA="always")
table(data.v16$drug_her, useNA="always")  # too small
table(data.v16$drug_met, useNA="always")  # too small
table(data.v16$drug_stim, useNA="always")
table(data.v16$drug_ect, useNA="always")  # too small
table(data.v16$drug_opi, useNA="always")   # too small
table(data.v16$drug_other, useNA="always")  # too vague, small
...
data_le <- data.v19 %>% select(ins_cover, abuse_year, gpa_sr, aca_impa, fincur, assault_phys, 
                               binge_fr, assault_sex, assault_emo, deprawsc, psyhx, drugs_yn)
...
results_le <- dummy_cols(data_le, select_columns = c("ins_cover", "abuse_year", "gpa_sr", "aca_impa", 
                                                     ...
                                                     "assault_emo", "deprawsc", "psyhx", "drugs_yn"),
                         remove_first_dummy = TRUE)
...
data.v20 <- data.v20 %>%
  mutate(dep_none = case_when(deprawsc < 6 ~ 1,
                              deprawsc > 5 ~ 0)) %>%
  mutate(dep_mild = case_when((deprawsc > 5 & deprawsc < 11) ~ 1,
                              (deprawsc < 6 | deprawsc > 10) ~ 0)) %>%
  mutate(dep_mod = case_when((deprawsc > 10 & deprawsc < 16) ~ 1,
                             (deprawsc > 15 | deprawsc < 11) ~ 0)) %>%
...
# Check outputs
table(data.v20$dep_none, useNA = "always")
table(data.v20$dep_mild, useNA = "always")
table(data.v20$dep_mod, useNA = "always")
table(data.v20$dep_modsev, useNA = "always")
table(data.v20$dep_sev, useNA = "always")
...
data_def <- data.v21 %>% select(aca_impa, dep_mild, dep_mod, dep_modsev, dep_sev, dep_none)
data <- data_def
sig <- 0.6
...
# Redefining defeat variable to account for significant overlaps

data.v21 <- data.v21 %>%
  mutate(dep_mod = case_when(dep_mod == 1 ~ 1,
                             dep_modsev == 1 ~ 1,
                             dep_sev == 1 ~ 0,
                             dep_mild == 1 ~ 0,
                             dep_none == 1 ~ 0))
table(data.v21$dep_mod)

data_def <- data.v21 %>% select(aca_impa, dep_mild, dep_mod, dep_sev)
data <- data_def
sig <- 0.6
...
corr <- as.data.frame(as.table(corr))         # turn into a 3-column table
corr <- na.omit(corr)                         # remove the NA values from above 
corr <- corr[order(-abs(corr$Freq)), sig>]         # sort by highest correlation
print(corr)
...
## EFA defeat
```{r}
data_def <- data.v21 %>% select(aca_impa, dep_mild, dep_mod, dep_modsev, dep_sev)
...
results_def <- dummy_cols(data_def, select_columns = c("aca_impa", "dep_mild", "dep_mod", "dep_modsev", "dep_sev"),
                         remove_first_dummy = TRUE)
...
results2_def <- data.v21 %>% select(si_type, aca_impa, dep_mild, dep_mod, dep_modsev, dep_sev)
