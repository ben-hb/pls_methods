...
#Find p-values, CI, OR
ctable_clmdia <- coef(summary(clm_dia))
p <- pnorm(abs(ctable_clmdia[, "t value"]), lower.tail = FALSE) * 2    # p-value computation
ctable_clmdia <- cbind(ctable_clmdia, "p value" = p)
ctable_clmdia                                                         # display p-value
ci_drugs <- confint(clm_dia)                           # find CIs
exp(cbind(OR = coef(clm_dia), ci_drugs))     # display odds ratio and CI outputs
...
data.v22 <- data.v22 %>%
  ...
  mutate(AAQ_yn = case_when(AAQ_tot > 24 ~ 1,
                            AAQ_tot < 25 ~ 0))
table(data.v22$AAQ_yn, useNA = "always")
...
data_tsm <- data.v23 %>% select(persist, anx_mild, anx_mod, anx_sev, dep_secret, achieve4, aaq_yn)
...
data_tsm <- data_tsm %>% 
  mutate_if(is.numeric, as.factor) %>% 
    mutate(
    aaq_yn = as.numeric(aaq_yn),
  ) %>% 
  dummy_cols(remove_first_dummy = TRUE) %>% 
  data.matrix()
data_tsm   # check output

results_tsm <- dummy_cols(data_tsm, select_columns = c("persist", "anx_mild", "anx_mod", "anx_sev", "dep_secret", "achieve4", "aaq_yn"),
                         remove_first_dummy = TRUE)
...
## CLM TSM
```{r CLM TSM}
clm_tsm <- data.v23 %>% select(si_type, persist, anx_mod, anx_sev, dep_secret, aaq_yn)

clm_tsm <- NA_preproc(clm_tsm)
clm_tsm <- as.data.frame(clm_tsm)

clm_tsm <- clm2(si_type ~ persist + anx_mod + anx_sev + dep_secret + aaq_yn, data = clm_tsm)
...
var_names <- c(
  ...
  # threats to self-moderation
  "persist", "anx_mod", "anx_sev", "dep_secret", "aaq_yn",
  # entrapment
...
SI_model_tr <- '# measurement model
                  ...
                  tsm_sem =~ persist + anx_mod + anx_sev + dep_secret + AAQ_yn
