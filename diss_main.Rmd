...
fitMeasures(fit_testM, fit.measures = "all")
```
...
## Missingness of each var w/in each var
```{r sub missingness between vars}
missing_dfs <- list()
for (k in 1:length(var_names)) {
...
fitMeasures(fit_sub18M, fit.measures = "all")
```
