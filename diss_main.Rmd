...
save(results_tr_sem, file = "traindf_sem.RData")
load("traindf_sem.RData")
write_xlsx(results_tr_sem, path = "traindf_sem.xls", col_names = TRUE, format_headers = TRUE)
```
...
SI_model_tr <- '
              # measurement model
                  ...
                  le_sem =~  ins_cover + gpa_sr + binge_fr + psyhx + fincur + drugs_yn + 
                             assault_emo + assault_phys + assault_sex + drug_mar
...
results_tr_sem <- as.matrix(results_tr_sem)
...
testdf_sem <- testdf.v28 %>% select(
  ...
  divers, satisfied_overall, talk, religios, discrim, school2_type, activ_yn, military,
  # life events
  ins_cover, gpa_sr, binge_fr, psyhx, fincur, drugs_yn, assault_emo, assault_phys, assault_sex, drug_mar,
  # defeat
  aca_impa, deprawsc,
  # threats to self-moderation
  persist, anx_score, aaq_tot, dep_secret,
