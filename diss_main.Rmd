...
results_def <- dummy_cols(data_def, select_columns = c("aca_impa", "dep_mild", "dep_mod", "dep_sev"),
                         remove_first_dummy = TRUE)
...
results2_def <- data.v21 %>% select(si_type, aca_impa, dep_mod, dep_sev)

clm_def <- NA_preproc(results2_def)
clm_def <- as.data.frame(clm_def)

fm1 <- clm2(si_type ~ aca_impa + dep_mod + dep_sev, data = clm_def)
summary(fm1)
