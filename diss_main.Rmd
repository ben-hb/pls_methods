...
clm_env <- traindf.v15 %>% select(si_type, religios, residenc, activ_no, school2_type, 
                                  international, discrim, international)
clm_env <- NA_preproc(clm_env) %>% as.data.frame()

# run ordinal regression
clm_env2 <- clm(si_type ~ religios + residenc + activ_no + school2_type + discrim + international, 
                data = clm_env)
...
traindf_prem <- traindf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       medther, religios, activ_no, school2_type, fincur, ins_no, residenc,
...
clm_prem <- clm(si_type ~ age + sexual + gender_tgd + sib_freq + race + ed_any + dx_mh +
                ...
                drugs_yn + drug_mar + psyhx + medther + fincur,
              data = clm_prem)
summary(clm_prem)
...
var_names <- c(
  ...
  # environment
  "religios", "activ_no", "discrim", "international",
  # life events
  "ins_no", "gpa_sr", "binge", "psyhx", "fincur", "drugs_yn", "assault",
  # defeat
...
traindf_sem1 <- traindf.v28 %>% select(
  ...
  religios, activ_no,
  # life events
  ins_no, gpa_sr, binge, psyhx, drugs_yn, assault,
  # defeat
  aca_impa, dep_type, anx_type,
  # threats to self-moderation
  persist, aaq_dum,
...
SI_model_tr1 <- '
                # measurement model
                  ...
                  prem =~  sib_freq + sexual_2 + sexual_3 + gender_tgd +
                           ed_any + dx_mh + activ_no + ins_no + medther + gpa_sr + 
                           binge + psyhx + drugs_yn + assault 
...
SI_model_trM <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual_2 + sexual_3 + gender_tgd + medther +
                           ...
                           gpa_sr + binge + psyhx + drugs_yn + assault
...
testdf_sem1 <- testdf.v26 %>% select(
  ...
  dep_impa,
  # motivational moderators
  inf, flourish_dum, satisfied_overall)  #mh_stigma, 
