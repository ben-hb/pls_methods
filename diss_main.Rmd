...
# create sem matrix 
test_sem <- test_sem %>% data.matrix()
test_sem <- na.omit(test_sem)

## Dummy col SEM w/ continuous
results_test <- dummy_cols(test_sem, select_columns = c("si_type", "sexual"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
SI_model_new2 <- '
                # measurement model
                    ...
                    prem =~    sib_freq + sexual_2 + sexual_3 +
                               dx_mh + meds_ther + psyhx + assault
                          
                    deftrap =~ dep_type + anx_type + dep_impa + aca_impa
...
fit_testM4 <- sem(SI_model_new2, data=results_test , ordered=T)
...
results_test_com <- as.data.frame(results_test)
...
# testing data
test18_sem_test <- describe(test18_sem)
view(test18_sem_test)

# xlsx output
save(test18_sem_test, file = "test18_sem_test.RData")
load("test18_sem_test.RData")
write_xlsx(test18_sem_test, path = "test18_sem_test.xls", col_names = TRUE, format_headers = TRUE)
...
fit_test18 <- sem(SI_model_18, data=results_test , ordered=T)
summary_test18 <- summary(fit_test18, fit.measures=TRUE, standardized = TRUE, rsquare=T)
fitMeasures(fit_test18, fit.measures = "all")
parameterestimates(fit_test18, boot.ci.type = "bca.simple", standardized = TRUE) 
modificationindices(fit_test18, minimum.value = 20)
...
# testing data
test20_sem_test <- describe(test20_sem)
view(test20_sem_test)
...
fit_test20 <- sem(SI_model_20, data=results_test , ordered=T)
summary_test20 <- summary(fit_test20, fit.measures=TRUE, standardized = TRUE, rsquare=T)
fitMeasures(fit_test20, fit.measures = "all")
parameterestimates(fit_test20, boot.ci.type = "bca.simple", standardized = TRUE) 
modificationindices(fit_test20, minimum.value = 20)

semPaths(fit_test20, layout = "circle2")
