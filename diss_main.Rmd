...
```{r tsne_dia}
# set up df for t-sne
data_dia2 <- data.v12 %>% select(age, sexual, gender_noncis, finpast, sib_freq, race, body_sr,
                                dx_dep, dx_bi, dx_adhd, dx_tr, dx_pers, meds_dep, ed_any,
                                meds_anx, meds_mood, meds_sle, meds_count)
...
diagnostic <- clm_dia %>% 
  summarize(
    anx_count = sum(!is.na(meds_anx))
  )

clm_mod<- clm(si_type ~ age + sexual + gender_noncis + finpast + sib_freq + body_sr + dx_tr +
...
# create corr matrix, with dummy vars
set.seed(10)
tsne_model_env = Rtsne(as.matrix(results_env2), check_duplicates=FALSE, pca=TRUE, perplexity=30, theta=0.5, dims=2)
d_tsne_env = as.data.frame(tsne_model_env$Y)  

## plotting the results without clustering
ggplot(d_tsne_env, aes(x=V1, y=V2)) +  
  geom_point(size=0.25) +
  guides(colour=guide_legend(override.aes=list(size=6))) +
  xlab("") + ylab("") +
  ggtitle("t-SNE") +
...
## keeping original data
d_tsne_env_original=d_tsne_env

## Creating k-means clustering model, and assigning the result to the data used to create the tsne
fit_cluster_kmeans=kmeans(scale(d_tsne_env), 3)  
d_tsne_env_original$cl_kmeans = factor(fit_cluster_kmeans$cluster)

## Creating hierarchical cluster model, and assigning the result to the data used to create the tsne
fit_cluster_hierarchical=hclust(dist(scale(d_tsne_env)))

## setting 3 clusters as output
d_tsne_env_original$cl_hierarchical = factor(cutree(fit_cluster_hierarchical, k=3))  
...
plot_k=plot_cluster(d_tsne_env_original, "cl_kmeans", "Accent")  
plot_h=plot_cluster(d_tsne_env_original, "cl_hierarchical", "Set1")
...
```{r tsne_le}
# PCA map with eigenvalues
data_le2 <- data.v19 %>% select(ins_cover, abuse_year, gpa_sr, aca_impa, drug_mar, fincur, assault_phys, binge_fr, assault_sex,
...
# t-SNE
colors = rainbow(length(unique(results_le2)))
names(colors) = unique(results_le2)

tsne<-Rtsne(results_le2,dims=2,perplexity=30,verbose=T,max_iter=1500,check_duplicates=F)

# display the results of t-SNE
cols <- rainbow(5)
plot(tsne$Y, t='n')
text(tsne$Y, labels=as.numeric(results_le2[,1]), col=cols[results_le2[,1]])
...
# create corr matrix, with dummy vars
set.seed(10)
tsne_model_le = Rtsne(as.matrix(results_le2), check_duplicates=FALSE, pca=TRUE, perplexity=30, theta=0.5, dims=2)
d_tsne_le = as.data.frame(tsne_model_le$Y)  

## plotting the results without clustering
ggplot(d_tsne_le, aes(x=V1, y=V2)) +  
  geom_point(size=0.25) +
  guides(colour=guide_legend(override.aes=list(size=6))) +
  xlab("") + ylab("") +
  ggtitle("t-SNE") +
...
## keeping original data
d_tsne_le_original=d_tsne_le

## Creating k-means clustering model, and assigning the result to the data used to create the tsne
fit_cluster_kmeans=kmeans(scale(d_tsne_le), 3)  
d_tsne_le_original$cl_kmeans = factor(fit_cluster_kmeans$cluster)

## Creating hierarchical cluster model, and assigning the result to the data used to create the tsne
fit_cluster_hierarchical=hclust(dist(scale(d_tsne_le)))

## setting 3 clusters as output
d_tsne_le_original$cl_hierarchical = factor(cutree(fit_cluster_hierarchical, k=3))  
...
plot_k=plot_cluster(d_tsne_le_original, "cl_kmeans", "Accent")  
plot_h=plot_cluster(d_tsne_le_original, "cl_hierarchical", "Set1")
...
# create corr matrix, with dummy vars
set.seed(10)
tsne_model_def = Rtsne(as.matrix(results_def2), check_duplicates=FALSE, pca=TRUE, perplexity=30, theta=0.5, dims=2)
d_tsne_def = as.data.frame(tsne_model_def$Y)  

## plotting the results without clustering
ggplot(d_tsne_def, aes(x=V1, y=V2)) +  
  geom_point(size=0.25) +
  guides(colour=guide_legend(override.aes=list(size=6))) +
  xlab("") + ylab("") +
  ggtitle("t-SNE") +
...
## keeping original data
d_tsne_def_original=d_tsne_def

## Creating k-means clustering model, and assigning the result to the data used to create the tsne
fit_cluster_kmeans=kmeans(scale(d_tsne_def), 3)  
d_tsne_def_original$cl_kmeans = factor(fit_cluster_kmeans$cluster)

## Creating hierarchical cluster model, and assigning the result to the data used to create the tsne
fit_cluster_hierarchical=hclust(dist(scale(d_tsne_def)))

## setting 3 clusters as output
d_tsne_def_original$cl_hierarchical = factor(cutree(fit_cluster_hierarchical, k=3))  
...
```{r T-SNE TSM}
# set up df for t-sne
data_tsm2 <- data.v23 %>% select(persist, anx_mild, anx_mod, anx_sev, dep_secret, achieve4, aaq_yn)
...
# t-SNE
colors = rainbow(length(unique(results_tsm2)))
names(colors) = unique(results_tsm2)

tsne<-Rtsne(results_tsm2,dims=2,perplexity=30,verbose=T,max_iter=1500,check_duplicates=F)
```
...
results_entrap2 <- unique(results_entrap2) # Remove duplicates

# colors = rainbow(length(unique(results_entrap2)))
# names(colors) = unique(results_entrap2)
set.seed(10)
...
# tsne<-Rtsne(results_entrap2,dims=2,perplexity=30,verbose=T,max_iter=1500,check_duplicates=F)
plot(tsne_out$Y, col=tsne_out$Y, asp=1)

# display the results of t-SNE
cols <- rainbow(length(unique(results_entrap2)))
plot(tsne$Y, t='n')
text(tsne$Y, labels=as.numeric(results_entrap2[,1]), col=cols[results_entrap2[,1]])

plot(tsne$Y, t='n', main="BarnesHutSNE")
text(tsne$Y, labels=rownames(results_entrap2))
...
```{r tsne_mm}
# set up df for t-sne
data_mm2 <- data.v27 %>% select(inf, flourish, belong1, belong2, belong8, belong9, stig_pcv_2, stig_pcv_3, brs_tot)
...
# t-SNE
colors = rainbow(length(unique(results_mm2)))
names(colors) = unique(results_mm2)

tsne<-Rtsne(results_mm2,dims=2,perplexity=30,verbose=T,max_iter=1500,check_duplicates=F)

# display the results of t-SNE
cols <- rainbow(length(unique(results_mm2)))
plot(tsne$Y, t='n')
text(tsne$Y, labels=as.numeric(results_mm2[,1]), col=cols[results_mm2[,1]])
```
...
tsne_sem <- results_sem
# create corr matrix, with dummy vars
tsne_sem <- na.omit(tsne_sem) %>%
  mutate_if(is.numeric, as.factor) %>%
  NA_preproc() %>%
  data.matrix()
...
tsne_sem <- unique(tsne_sem) # Remove duplicates

set.seed(10)
tsne_model_1 = Rtsne(as.matrix(tsne_sem), check_duplicates=FALSE, pca=TRUE, perplexity=30, theta=0.5, dims=2)
d_tsne_1 = as.data.frame(tsne_model_1$Y)  
