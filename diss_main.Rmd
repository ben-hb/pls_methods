...
## CLM TSM
```{r CLM TSM}
clm_tsm <- data.v23 %>% select(si_type, persist, anx_mod, anx_sev, dep_secret, aaq_dum)

clm_tsm <- NA_preproc(clm_tsm)
clm_tsm <- as.data.frame(clm_tsm)

clm_tsm <- clm(si_type ~ persist + anx_mod + anx_sev + dep_secret + aaq_dum, data = clm_tsm)
...
data_mm <- data.v27 %>% select(inf, flourish_dum, belong1, belong2, belong8, belong9, stig_pcv_2, stig_pcv_3, brs_dum)
data <- data_mm
sig <- 0.6
...
results_mm <- dummy_cols(data_mm, select_columns = c("inf", "flourish_dum", "belong1", "belong2", "belong8",
                                                     "belong9", "stig_pcv_2", "stig_pcv_3", "brs_dum"),
                         remove_first_dummy = TRUE)
...
```{r tsne_mm}
# set up df for t-sne
data_mm2 <- data.v27 %>% select(inf, flourish_dum, belong1, belong2, belong8, belong9, stig_pcv_2, stig_pcv_3, brs_dum)
...
Variables for consideration:  inf, flourish_dum, belong1, belong2, belong8, belong9, stig_pcv_2, stig_pcv_3, brs_dum
```{r CLM mm}

clm_mm <- data.v27 %>% select(si_type, inf, flourish_dum, belong1, belong2, belong8, belong9, 
                              stig_pcv_2, stig_pcv_3, brs_dum)

clm_mm <- NA_preproc(clm_mm)
clm_mm <- as.data.frame(clm_mm)

clm_mm <- clm(si_type ~ inf + flourish_dum + belong1 + belong2 + belong8 + belong9 + stig_pcv_2 + 
                stig_pcv_3 + brs_dum, data = clm_mm)
...
var_names <- c(
  ...
  # motivational moderators
  "inf", "flourish_dum", "belong1", "belong2", "belong8", "belong9", "stig_pcv_2", "stig_pcv_3", "brs_dum")  
...
save(traindf.v29, file = "traindf.v29.dat")
load("traindf.v29.dat")
write_xlsx(traindf.v29, path = "sem.xls", col_names = TRUE, format_headers = TRUE)
...
## t-SNE for train
```{r}
tsne_train <- traindf.v29
# create corr matrix, with dummy vars
tsne_train <- na.omit(tsne_train) %>%
  mutate_if(is.numeric, as.factor) %>%
  NA_preproc() %>%
  data.matrix()
...
# create corr matrix, with dummy vars
set.seed(10)
tsne_model_1 = Rtsne(as.matrix(tsne_train), check_duplicates=FALSE, pca=TRUE, perplexity=50, theta=0.5, dims=2)
d_tsne_1 = as.data.frame(tsne_model_1$Y)  
...
## t-SNE for test data
```{r}
tsne_test <- testdf.v29
# create corr matrix, with dummy vars
tsne_test <- na.omit(tsne_test) %>%
  mutate_if(is.numeric, as.factor) %>%
  NA_preproc() %>%
  data.matrix()
...
# create corr matrix, with dummy vars
set.seed(10)
tsne_model_1 = Rtsne(as.matrix(tsne_train), check_duplicates=FALSE, pca=TRUE, perplexity=30, theta=0.5, dims=2)
d_tsne_1 = as.data.frame(tsne_model_1$Y)  
...
SI_model_tr <- '# measurement model
                  ...
                  mm_sem =~  inf + flourish_dum + belong1 + belong2 + belong8 + belong9 + 
                             stig_pcv_2 + stig_pcv_3 + brs_dum
                  si_sem =~  si_type
...
meds_dep -> dia_sem, , NA
meds_anx -> dia_sem, , NA
meds_mood -> dia_sem, , NA
meds_sle -> dia_sem, , NA
meds_count -> dia_sem, , NA

env_mh -> env_sem, , NA
ther_vis -> env_sem, , NA
divers -> env_sem, , NA
satisfied_overall -> env_sem, , NA
residenc, -> env_sem, , NA
talk -> env_sem, , NA
religios -> env_sem, , NA
discrim -> env_sem, , NA
school2_type -> env_sem, , NA
ctiv_yn -> env_sem, , NA
military -> env_sem, , NA
international -> env_sem, , NA

ins_cover -> le_sem, , NA
gpa_sr  -> le_sem, , NA
assault_phys -> le_sem, , NA
binge_fr -> le_sem, , NA
assault_sex -> le_sem, , NA
assault_emo -> le_sem, , NA
psyhx -> le_sem, , NA
fincur -> le_sem, , NA
drugs_yn -> le_sem, , NA
abuse_year -> le_sem, , NA

aca_impa -> def_sem, , NA
deprawsc -> def_sem, , NA

persist  -> tsm_sem, , NA
anx_score -> tsm_sem, , NA
dep_secret -> tsm_sem, , NA
aaq_dum -> tsm_sem, , NA

dep_impa -> entrap_sem, , NA
gad7_impa -> entrap_sem, , NA
percneed_cur -> entrap_sem, , NA

inf -> mm_sem, , NA
flourish_dum -> mm_sem, , NA
belong1 -> mm_sem, , NA
belong2 -> mm_sem, , NA
belong8 -> mm_sem, , NA
belong9 -> mm_sem, , NA
stig_pcv_2 -> mm_sem, , NA
tig_pcv_3 -> mm_sem, , NA
brs_dum -> mm_sem, , NA   
...
mod.hs <- cfa()
  dia_sem: age, sexual, gender_noncis, finpast, sib_freq, race, body_sr, ed_any, audit_tot, dx_dep, 
           dx_bi, dx_tr, dx_adhd, dx_pers, meds_sti, meds_dep, meds_anx, meds_mood, meds_sle, meds_count
  ...
  tsm_sem: persist, anx_score, dep_secret, aaq_dum
  entrap_sem: dep_impa, gad7_impa, percneed_cur
  mm_sem: inf, flourish_dum, belong1, belong2, belong8, belong9, stig_pcv_2, stig_pcv_3, brs_dum
  si_sem: si_type
