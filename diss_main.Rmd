...
clm_dia <- clm(si_type ~ age + sexual + gender_noncis + finpast + sib_freq + body_sr +
...
clm_env <- clm(si_type ~ discrim + satisfied_overall + ther_vis + talk + activ_yn + international + 
                ther_vis + divers + + military + school2_type + residenc + env_mh, data = clm_env)
summary(clm_env)

#  NOT WORKING: + religios ... need to figure out why
...
data_le <- data.v19 %>% select(ins_cover, abuse_year, gpa_sr, aca_impa, fincur, assault_phys, drug_mar,
...
results_le <- dummy_cols(data_le, select_columns = c("ins_cover", "abuse_year", "gpa_sr", "aca_impa", "drug_mar",
...
clm_le <- data.v19 %>% select(si_type, ins_cover, abuse_year, gpa_sr, aca_impa, fincur, assault_phys, drug_mar,
                              binge_fr, assault_sex, assault_emo, psyhx, drugs_yn)

clm_le <- NA_preproc(clm_le)
clm_le <- as.data.frame(clm_le)

clm_le <- clm(si_type ~ ins_cover + abuse_year + gpa_sr + aca_impa + fincur + assault_phys + binge_fr + drug_mar +
...
clm_def <- clm(si_type ~ aca_impa + dep_mod + dep_sev, data = clm_def)
summary(clm_def)
coef(clm_def)
vcov(clm_def)

# # Find p-values, CI, OR
# ctable_def <- coef(summary(clm_def))
# p <- pnorm(abs(ctable_def[, "t value"]), lower.tail = FALSE) * 2    # p-value computation
# ctable_def <- cbind(ctable_def, "p value" = p)
# ctable_def                                                          # display p-value
# ci_def <- confint(clm_def)                           # find CIs
# exp(cbind(OR = coef(clm_def), ci_def))     # display odds ratio and CI outputs
...
data_tsm <- data.v23 %>% select(persist, anx_none, anx_mild, anx_mod, anx_sev, dep_secret, achieve4, aaq_yn)
...
clm_tsm <- clm(si_type ~ persist + anx_mod + anx_sev + dep_secret + aaq_yn, data = clm_tsm)
...
clm_entrap <- clm(si_type ~ dep_impa + gad7_impa + percneed_cur, data = clm_entrap)
...
SI_model_tr <- '# measurement model
                  dia_sem =~ age + sexual + gender_noncis + finpast + sib_freq + 
                             race + body_sr + ed_any + audit_tot + dx_dep + dx_bi + 
                             dx_tr + dx_adhd + dx_pers + meds_sti + meds_dep + 
                             meds_anx + meds_mood + meds_sle + meds_count
                  env_sem =~ env_mh + ther_vis + divers + satisfied_overall +
                             residenc + talk + religios + discrim + school2_type + 
                             activ_yn + military + international
                  le_sem =~  ins_cover + gpa_sr + assault_phys + binge_fr +
                             assault_sex + assault_emo + psyhx + fincur + drugs_yn + abuse_year
                  ...
                  mm_sem =~  inf + flourish + belong1 + belong2 + belong8 + belong9 + 
                             stig_pcv_2 + stig_pcv_3 + BRS_tot
                  si_sem =~  si_type
