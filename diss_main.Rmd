...
traindf_tsm <- traindf.v23 %>% select(persist, anx_score, dep_secret, achieve4, aaq_dum)
...
results_tsm <- dummy_cols(traindf_tsm, select_columns = c("persist", "anx_score", "dep_secret", "achieve4", "aaq_dum"),
                         remove_first_dummy = TRUE)
...
## CLM TSM
```{r CLM TSM}
clm_tsm <- traindf.v23 %>% select(si_type, persist, anx_score, dep_secret, aaq_dum)
...
# create corr matrix, with dummy vars 
traindf_entrap <- traindf_entrap %>% 
  mutate_if(is.numeric, as.factor) %>% 
  mutate_if(is.character, as.factor) %>% 
  data.matrix()
traindf_entrap   # check output
...
```{r CLM mm}
clm_mm <- traindf.v27 %>% select(si_type, inf, flourish_dum, belong1, mh_stigma, brs_dum)
