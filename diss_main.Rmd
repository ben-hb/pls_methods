...
traindf.v11 <- traindf.v11 %>%
  filter(!is.na(medther))
traindf.v11$medther <- as.logical(traindf.v11$medther)  # boolean var

table(traindf.v11$medther, useNA = "always")  # check output
```
...
pca_dia <- traindf.v11 %>% select(age, sexual, gender_tgd, finpast, sib_freq, race,
                                dx_mh, ed_any, medther)
pca_dia <- na.omit(pca_dia)    # filters NAs out
view(pca_dia)
...
traindf_prem <- traindf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       medther, religios, activ_no, school2_type, ins_no, residenc,
...
results_prem <- dummy_cols(traindf_prem, select_columns = c("age", "sexual", "gender_tgd", "sib_freq",
                                                            "race", "ed_any", "dx_mh", "medther", "religios",
...
clm_prem <- traindf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                 meds_count, religios, activ_no, school2_type, ins_no, residenc, medther,
                                 gpa_sr, fincur, assault, drugs_yn, drug_mar, psyhx, international)
...
clm_prem <- clm(si_type ~ age + sexual + gender_tgd + sib_freq + race + ed_any + dx_mh + meds_count +
                ...
                drugs_yn + drug_mar + psyhx + international + medther,
              data = clm_prem)
summary(clm_prem)
coef(clm_prem)
...
var_names <- c(
  # diathesis
  "age", "sexual", "gender_tgd", "sib_freq", "ed_any", "dx_mh", "medther",
  # environment
...
traindf_sem <- traindf.v28 %>% select(
  ...
  si_type,
  # diathesis
  age, sexual, gender_tgd, sib_freq, ed_any, dx_mh, meds_count, medther,
  # environment
  religios, school2_type, activ_no, residenc, discrim, international,
  # life events
  ins_no, gpa_sr, binge, psyhx, fincur, drugs_yn, assault, drug_mar,
...
SI_model_tr1 <- '
                # measurement model
                  ...
                  prem =~  sib_freq + sexual_2 + sexual_3 + gender_tgd + discrim +
                           ed_any + dx_mh + activ_no + ins_no + medther + residenc_2 +
...
SI_model_trM <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual_2 + sexual_3 + gender_tgd + medther +
...
SI_model_trM2 <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual_2 + sexual_3 + gender_tgd + assault +
                           ed_any + dx_mh + psyhx + fincur + drugs_yn + medther
...
SI_model_new<- '
                # measurement model
                    ...
                    prem =~    sib_freq + sexual_2 + sexual_3 + gender_tgd +
                               ed_any + dx_mh + medther + gpa_sr + binge + psyhx + 
...
testdf.v11 <- testdf.v11 %>%
  filter(!is.na(medther))
testdf.v11$medther <- as.logical(testdf.v11$medther)  # boolean var

table(testdf.v11$medther, useNA = "always")  # check output
```
...
testdf_sem1 <- testdf.v26 %>% select(
  ...
  si_type,
  # diathesis
  sexual, gender_tgd, sib_freq, ed_any, dx_mh, medther,
  # environment
...
SI_model_test1 <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual_2 + sexual_3 + gender_tgd + assault +
                           ed_any + dx_mh + medther + psyhx + fincur + drugs_yn 
...
# 18
smart.pls18<-matrix(c(
0.10676412749451619, 0.12108264502149285, 0.07531025680293642, 0.17217659197447677, 
0.24998163183041647, 0.24205234302277726, 0.1793750365895833, 0.29953228359405426, 
0.14215063168112282, 0.1441768183359829, 0.08751248257833301, 0.20077275559190547, 
0.5827085521419574, 0.5864981538920709, 0.5559313613225414, 0.6178903356722308
), byrow=TRUE, nrow=4, ncol=4)
rownames(smart.pls18)<-c("Moderating Effect 1 -> si_","deftrap -> si_","mmtsm -> si_","prem -> deftrap")
colnames(smart.pls18)<-c("Original Sample (O)","Sample Mean (M)","2.5%","97.5%")
```
