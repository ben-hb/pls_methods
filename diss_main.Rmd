...
SI_model_trM2 <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual_2 + sexual_3 + gender_tgd + assault +
                           ed_any + dx_mh + psyhx + drugs_yn + meds_ther
...
# Baseline model for amended model
SI_model_amended <- '
              # measurement model
                    si =~      si_type_4 + si_type_3 + si_type_2
    
                    prem =~    meds_ther + dx_mh + sib_freq + sexual_2 + sexual_3 + ed_any 
                    
                    le =~      psyhx + assault + drugs_yn + ins_no
                          
                    deftrap =~ dep_type + dep_impa + anx_type + aca_impa + percneed
                              
                    mmtsm =~   aaq_dum + flourish_dum + inf + persist + satisfied_overall
                    
                # structural model
                    si ~ deftrap + mmtsm + mmtsm*deftrap
                    deftrap ~ prem + le  '

fit_train_a <- sem(SI_model_amended, data=results_tr_com1 , ordered=T)
summary_train_a <- summary(fit_train_a, fit.measures=TRUE, standardized = TRUE, rsquare=T)
fitMeasures(fit_train_a, fit.measures = "all")
parameterestimates(fit_train_a, boot.ci.type = "bca.simple", standardized = TRUE) 
modificationindices(fit_train_a, minimum.value = 20)
```
...
SI_model_amended2 <- '
               ...
               ## regressions
                    ...
                    total := c + a*b
               # structural model
                    si ~ deftrap + mmtsm + mmtsm_mod
'
                    
# inf, sexual_2, satisfied_overall

fit_train_a2 <- sem(SI_model_amended2, data=results_tr_com1, ordered=T)
summary_train_a2 <- summary(fit_train_a2, fit.measures=T, standardized = T, rsquare=T)
fitMeasures(fit_train_a2, fit.measures = "all")
parameterestimates(fit_train_a2, boot.ci.type = "bca.simple", standardized = T) 
modificationindices(fit_train_a2, minimum.value = 20)
semPaths(fit_train_a2, layout = "tree")
```
...
# preparing dataframe for regression
clm_env <- testdf.v15 %>% select(si_type, religios, residenc, activ_no, school2_type)
clm_env <- NA_preproc(clm_env) %>% as.data.frame()
...
testdf_prem <- testdf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, 
                                     dx_mh, meds_ther, religios, activ_no, school2_type, 
...
results_test_sem1 <- dummy_cols(testdf_sem, select_columns = c(
    ...
    "si_type",
    # diathesis
    "age", "sexual", "sib_freq", "ed_any", "dx_mh", "meds_count", "meds_ther",
    # environment
    "religios", "activ_no",
    # life events
    "ins_no", "gpa_sr", "psyhx", "fincur", "drugs_yn", "assault",
