...
data.v3$gender[data.v3$gender_text %in% c("Albino stingray", "Karate Chop Action", "Flying Spaghetti Monster", "Pio Nono",
                                          ...
                                          "penguin wearing a sombrero and sunglasses")] <- "non-answers"
...
data.v4$sexual[data.v4$sexual_text %in% c("potentially interested", "somewhere between", "homoflexible", "heteroflexible",
                                          ...
                                          "Attracted to masculinity and androgony", "Androsexual")] <- "Queer"
...
data.v4$sexual[data.v4$sexual_text %in% c("Mostly asexual", "Most likely asexual", "Masexual", "Tridium Vykon Jace 8000", 
                                          ...
                                          "lithromantic", "Pan-Quoiromantic Asexual", "demiromantic asexual")] <- "Asexual/Demi"
...
data.v5 <- data.v4 %>%
  ...
  mutate(sexual = factor(sexual, levels=1:6,
                       labels=c("Heterosexual", "Gay/Lesbian", "Bisexual", "Queer/non-binary", 
                                "Questioning", "Non-answers")))
...
data.v6$race[data.v6$race_other_text %in% c("Volvo", "Vulcan", "Snowman", "tree", "General Electric", "Cat", "Earthling", 
                                            ...
                                            "Desert Camoflauge",  "Attack helicopter", "Ethnic Kekistani")] <- "Non-answers"
...
data.v6$race[data.v6$race_other_text %in% c("Japanese, Chinese, French, British, Greek", "Asian/White",
                                            ...
                                            "White & Asian", "Chinese and White")] <- "Other/Mixed"
...
results_dia <- dummy_cols(data_dia, select_columns = c("age", "sexual", "gender_noncis", "finpast", 
                                                       "sib_freq", "race", "body_sr", "dx_dep", "dx_bi",
                                                       "dx_adhd", "dx_tr", "ed_any", "dx_pers", "meds_dep",
                                                       "meds_anx", "meds_mood", "meds_sle", "meds_count"), 
...
fviz_contrib(dia_pca, choice = "var", axes = 1, top = 10)
fviz_contrib(dia_pca, choice = "var", axes = 2, top = 10)
fviz_contrib(dia_pca, choice = "var", axes = 3, top = 10)
fviz_contrib(dia_pca, choice = "var", axes = 4, top = 10)
...
ggmaplot(dia_pca, main = expression("WT vs DBR"), fdr = 0.05, fc = 2, size = 0.4, palette = c("#B31B21", "#1465AC", "darkgray"), genenames = as.vector(dia_pca$MiRTS), legend = "top", top = 20, font.label = c("bold", 11), label.rectangle = TRUE, font.legend = "bold", font.main = "bold", ggtheme = ggplot2::theme_minimal())
...
clm_dia <- data.v12 %>% select(si_type, age, sexual, gender_noncis, finpast, sib_freq, race, 
                               body_sr, dx_dep, dx_bi, dx_adhd, dx_tr, dx_pers, meds_dep, ed_any,
                               meds_anx, meds_mood, meds_sle, meds_count)

clm_dia <- NA_preproc(clm_dia)
clm_dia <- as.data.frame(clm_dia)

clm_dia <- clm(si_type ~ age + sexual + gender_noncis + finpast + sib_freq + body_sr + dx_tr +
                         race + meds_dep + ed_any + meds_anx + meds_mood + meds_sle + dx_pers +
...
model.dia <- tidy(clm_dia)  # Convert model to dataframe for easy manipulation
model.dia 

model.dia  %>%
  mutate(or = exp(estimate),  # Odds ratio/gradient
         var.diag = diag(vcov(clm_dia)),  # Variance of each coefficient
...
# plot((env_pca), xlab = "Principle Component",
#      ylab = "Cumulative Proportion of Variance Explained",
#      type = "b"
#      )
# plot(cumsum(env_pca), xlab = "Principle Component",
#      ylab = "Cumulative Proportion of Variance Explained",
#      type = "b"
#      )

fviz_contrib(env_pca, choice = "var", axes = 1, top = 10)
fviz_contrib(env_pca, choice = "var", axes = 2, top = 10)
fviz_contrib(env_pca, choice = "var", axes = 3, top = 10)
fviz_contrib(env_pca, choice = "var", axes = 4, top = 10)
...
res <- hcut(data_env, k = 4, stand = TRUE)
...
my_data <- scale(data_env)
fviz_nbclust(my_data, kmeans, method = "gap_stat")
```


```{r T-SNE ENV}
data_env2 <- data.v15 %>% select(si_type, discrim, religios, env_mh, residenc, ther_vis, activ_yn, divers, 
                                school2_type, satisfied_overall, ther_vis, talk, military, international)
...
# PCA map with eigenvalues
results_env2 <- dummy_cols(data_env2, select_columns = c("si_type", "discrim", "religios", "env_mh", "residenc", "ther_vis",
                                                       "activ_yn", "divers", "school2_type", "satisfied_overall", 
                                                       "ther_vis", "talk", "military", "international"),
                          remove_first_dummy = TRUE)
clm_env <- randomForest(si_type ~ discrim + satisfied_overall + ther_vis + talk + activ_yn + international + 
                ther_vis + divers +  military + school2_type + residenc + env_mh, data = results_env2)

## for plotting
colors = rainbow(length(unique(clm_env)))
names(colors) = unique(clm_env)

data(results_env2, na.action = na.roughfix)

## Executing the algorithm on curated data
tsne <- Rtsne(results_env2, dims = 3, perplexity=30, verbose=TRUE, max_iter=500)
exeTimeTsne<- system.time(Rtsne(results_env2, dims = 2, perplexity=30, verbose=TRUE, max_iter=500))

## Plotting
plot(tsne$Y, t='n', main="tsne")
text(tsne$Y, labels=results_env2, col=colors[results_env2])
...
clm_env <- data.v15 %>% select(si_type, discrim, religios, env_mh, residenc, ther_vis, activ_yn, 
...
data_le <- data.v19 %>% select(ins_cover, abuse_year, gpa_sr, aca_impa, fincur, assault_phys,
                               drug_mar, binge_fr, assault_sex, assault_emo, deprawsc, psyhx, drugs_yn)
...
results_le <- dummy_cols(data_le, select_columns = c("ins_cover", "abuse_year", "gpa_sr",
                                                     "aca_impa", "drug_mar", "fincur",
                                                     "assault_phys", "binge_fr", "assault_sex", 
                                                     "assault_emo", "deprawsc", "psyhx",
                                                     "drugs_yn"),
                         remove_first_dummy = TRUE)
...
fviz_contrib(le_pca, choice = "var", axes = 1, top = 10)
fviz_contrib(le_pca, choice = "var", axes = 2, top = 10)
fviz_contrib(le_pca, choice = "var", axes = 3, top = 10)
fviz_contrib(le_pca, choice = "var", axes = 4, top = 10)
...
clm_le <- data.v19 %>% select(si_type, ins_cover, abuse_year, gpa_sr, aca_impa, fincur,
                              assault_phys, drug_mar, binge_fr, assault_sex, assault_emo, 
                              psyhx, drugs_yn)

clm_le <- NA_preproc(clm_le)
clm_le <- as.data.frame(clm_le)

clm_le <- clm(si_type ~ ins_cover + abuse_year + gpa_sr + aca_impa + fincur + assault_phys + 
                binge_fr + drug_mar + assault_sex + assault_emo + + psyhx + drugs_yn, 
              data = clm_le)
summary(clm_le)
coef(clm_le)
...
#Visualize PCA, FAMD results
fviz_screeplot(def_pca, addlabels = TRUE, ylim = c(0, 50))  # scree plot

fviz_contrib(def_pca, choice = "var", axes = 1, top = 10)
fviz_contrib(def_pca, choice = "var", axes = 2, top = 10)
fviz_contrib(def_pca, choice = "var", axes = 3, top = 10)
fviz_contrib(def_pca, choice = "var", axes = 4, top = 10)
...
fviz_contrib(tsm_pca, choice = "var", axes = 1, top = 10)
fviz_contrib(tsm_pca, choice = "var", axes = 2, top = 10)
fviz_contrib(tsm_pca, choice = "var", axes = 3, top = 10)
fviz_contrib(tsm_pca, choice = "var", axes = 4, top = 10)
...
fviz_contrib(entrap_pca, choice = "var", axes = 1, top = 10)
fviz_contrib(entrap_pca, choice = "var", axes = 2, top = 10)
fviz_contrib(entrap_pca, choice = "var", axes = 3, top = 10)
fviz_contrib(entrap_pca, choice = "var", axes = 4, top = 10)
...
fviz_contrib(mm_pca, choice = "var", axes = 1, top = 10)
fviz_contrib(mm_pca, choice = "var", axes = 2, top = 10)
fviz_contrib(mm_pca, choice = "var", axes = 3, top = 10)
fviz_contrib(mm_pca, choice = "var", axes = 4, top = 10)
...
```{r Multivariate skewness and kurtosis}
mardiaSkew(results_sem, use = "everything")
mardiaKurtosis(results_sem, use = "everything")
```
...
summary_test <- summary(fit_test, fit.measures=TRUE)
list(summary_test$coefficient, 
      round(1-(summary_test$deviance/summary_test$null.deviance), 2))
...
## SEM Path diagram
```{r SEM path diagram}
cols <- wes_palette(name = "SI Conceptualization in College Students", n = 4, type = "discrete")
