...
results_tr_sem1 <- dummy_cols(traindf_sem, select_columns = c(
    ...
    "age", "sexual", "gender_tgd", "sib_freq", "ed_any", "dx_mh", "meds_count",
    # environment
    "religios", "school2_type", "activ_no", "international", "residenc", "disrcim",
    # life events
    "ins_no", "gpa_sr", "binge", "psyhx", "fincur", "drugs_yn", "assault", "drug_mar",
    # defeat
    "aca_impa", "deprawsc", "anx_score",
...
SI_model_trM <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual_2 + sexual_3 + gender_tgd + meds_count +
...
SI_model_trM2 <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual_2 + sexual_3 + gender_tgd + assault +
                           ed_any + dx_mh + psyhx + fincur + drugs_yn + meds_count
...
```{r new_model_base}
## Dummy col SEM w/ continuous
results_tr_com1 <- dummy_cols(traindf_sem1, select_columns = c("si_type", "sexual", "school2_type", "residenc"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
#view(results_tr_com1)
...
SI_model_new<- '
                # measurement model
                    ...
                    diaenv =~  sib_freq + sexual_2 + sexual_3 + gender_tgd + ed_any + dx_mh + medther + discrim 
                   
                    le =~  psyhx + gpa_sr +  fincur + drugs_yn + assault + drug_mar + ins_no 
                    
                    def  =~ deprawsc + aca_impa + anx_score
                              
                    tsm =~ aaq_dum + flourish_dum + persist 
                    
                    entrap =~ percneed + dep_impa + gad7_impa
                    
                    mm =~ talk + mh_stigma + inf + belong1 + satisfied_overall

                # structural model - regressions
                
                    si ~ diaenv + le + def + def*tsm + entrap + entrap*mm
