...
data.v3$gender_noncis <- as.factor(data.v3$gender_noncis)
```
...
traindf_dia <- traindf.v12 %>% select(age, sexual, gender_noncis, finpast, sib_freq, race, ed_scoff, body_sr,
...
traindf_dia <- traindf.v12 %>% select(age, sexual, gender_noncis, finpast, sib_freq, race, body_sr,
...
results_dia <- dummy_cols(traindf_dia, select_columns = c("age", "sexual", "gender_noncis", "finpast", 
...
```{r clm_dia}
clm_dia <- traindf.v12 %>% select(si_type, age, sexual, gender_noncis, finpast, sib_freq, race, body_sr, dx_anx,
...
traindf_le <- traindf.v19 %>% select(ins_cover, gpa_sr, aca_impa, fincur, drug_mar, binge_fr,
...
```{r le_pca}
traindf_le <- traindf.v19 %>% select(ins_cover, gpa_sr, aca_impa, fincur, drug_mar, binge_fr, 
                                     assault_phys, assault_sex, assault_emo, psyhx, drugs_yn)
...
results_le <- dummy_cols(traindf_le, select_columns = c("ins_cover", "assault_phys", "assault_sex",
                                                        ...
                                                        "drug_mar", "fincur", "binge_fr", "drugs_yn"),
                         remove_first_dummy = TRUE)
...
```{r clm_LE}
clm_le <- traindf.v19 %>% select(si_type, ins_cover, gpa_sr, aca_impa, fincur, drug_mar, 
                                 assault_sex, assault_emo, assault_phys, binge_fr, psyhx, drugs_yn)
...
#                 aca_impa + fincur + binge_fr + drug_mar + psyhx + drugs_yn,
#               data = clm_le)
# summary(clm_le)
# coef(clm_le)
...
traindf_sem <- traindf.v28 %>% select(
  ...
  si_type,
  # diathesis
  age, sexual, gender_noncis, sib_freq, race, ed_any, dx_dep, dx_bi, dx_anx, dx_tr, dx_adhd, meds_count,
  # environment
  divers, satisfied_overall, talk, religios, discrim, school2_type, activ_yn, military,
  # life events
...
## Create dummy vars
```{r dummy_cols SEM}
results_sem <- dummy_cols(traindf_sem2, select_columns = c(
    # outcome var
    "si_type",
    # diathesis
    "age", "sexual", "gender_noncis", "sib_freq", "race", "ed_any", "dx_dep", "dx_bi", "dx_anx",
...
# test data
var_desc2 <- describe(traindf_sem2) %>% view()
...
## Variable plots
```{r var_plots}
traindf_sem2 <- as.data.frame(data.v9)
# diathesis
  # age
    histogram(~ age, data = data.v9, type = "density", panel = function(x, ...) {
    panel.histogram(x, ...)
  ...
  # age + SI type
    densityplot(~ age | si_type, data = data.v9)

  #sexual
    hist(data.v9$sexual)
    ggplot(mutate(data.v9, age = fct_infreq(age))) + geom_bar(aes(x = Hair))
    

hist(data.v9$sib_freq)


pie(data.v9$race)


hist(data.v9$med_count)

# boolean vars
hist(data.v9$gender_noncis)
hist(data.v9$ed_any)
hist(data.v9$dx_dep)
hist(data.v9$dx_bi)
hist(data.v9$dx_anx)
hist(data.v9$dx_trauma)
hist(data.v9$dx_adhd)
...
## Model correlations
```{r model_corr}
results_corr <- NA_preproc(results_sem)
...
save(results_sem, file = "traindf_sem.RData")
load("traindf_sem.RData")
write_xlsx(results_sem, path = "traindf_sem.xls", col_names = TRUE, format_headers = TRUE)

# save full var set for SEM
save(data.v9, file = "datav9.RData")
load("datav9.RData")
write_xlsx(data.v9, path = "datav9.xls", col_names = TRUE, format_headers = TRUE)
...
results_tr_sem <- as.matrix(results_sem)

fit <- cfa(SI_model_tr, data = results_sem,
...
## Discrimination
```{r discrim}
table(testdf.v13$discrim)
testdf.v13$discrim <- as.factor(testdf.v13$discrim)
```
...
## Military status
```{r military}
table(testdf.v14$military)
testdf.v14$military <- as.logical(testdf.v14$military)
```

## International student
```{r international}
table(testdf.v14$international)
...
testdf.v15 = subset(testdf.v15, select= -c(ins_1, ins_mh, ins_ade, ins_inade)) # clean up var
```
...
#   mutate(aaq_dum = ifelse(AAQ_tot > rs_df_arranged[1,1], TRUE, FALSE))
# 
# table(testdf.v22$aaq_dum)
```

## Achievement
n = 594  ** too small
```{r achieve4}
table(testdf.v22$achieve4)
```
...
```{r CORR TSM}
# Define TSM variable candidates for correlation matrix
testdf_tsm <- testdf.v23 %>% select(persist, anx_score, dep_secret, achieve4, aaq_tot)
...
results_tsm <- dummy_cols(testdf_tsm, select_columns = c("persist", "anx_score", "dep_secret", "achieve4", "aaq_tot"),
                         remove_first_dummy = TRUE)
...
## CLM TSM
```{r CLM TSM}
clm_tsm <- testdf.v23 %>% select(si_type, persist, anx_score, dep_secret, aaq_tot)
...
```{r Entrapment corr}
# Define entrapment variable candidates for correlation matrix
testdf_entrap <- testdf.v25 %>% select(dep_impa, gad7_impa, percneed_cur)
...
### EFA Entrapment
```{r CFA entrap}
testdf_entrap <- testdf.v25 %>% select(dep_impa, gad7_impa, percneed_cur)   # all continuous... treat
...
###  CLM Entrapment regression
```{r clm_entrap}
clm_entrap <- testdf.v25 %>% select(si_type, dep_impa, gad7_impa, percneed_cur)
...
** too small for inclusion in model
```{r achieve_1_14_18}
table(testdf.v26$achieve1)
table(testdf.v26$achieve14)
table(testdf.v26$achieve18)

# sample too small in the elective, remove
testdf.v22 = subset(testdf.v22, select= -c(achieve1, achieve14, achieve18)) # clean up var
```
...
## Friendship development
```{r friend_devel}
table(testdf.v26$friend_devel)

# sample too small in the elective, remove
testdf.v26 = subset(testdf.v26, select= -c(friend_devel)) # clean up var
```
...
```{r MM_corr}
# Define MM variable candidates for correlation matrix
testdf_mm <- testdf.v27 %>% select(inf, flourish_dum, belong1, mh_stigma, brs_dum)
...
results_mm <- dummy_cols(testdf_mm, select_columns = c("inf", "flourish_dum", "belong1", "mh_stigma", "brs_dum"),
                         remove_first_dummy = TRUE)
mm_pca <- PCA(results_mm, graph=FALSE)  # apply PCA
eigval_mm <- get_eigenvalue(mm_pca)
eigval_mm
...
Variables for consideration:  inf, flourish_dum, belong2, stig_pcv_2, stig_pcv_3, brs_dum
```{r CLM mm}
clm_mm <- testdf.v27 %>% select(si_type, inf, flourish, belong1, mh_stigma, brs_tot)
...
# loop to replicate row for all var
for (i in 1:nrow(missing_rates)) {
  missing_rates[i, 2] = sum(!is.na(testdf.v28[[i]]))
  missing_rates[i, 3] = missing_rates[i, 2] / nrow(testdf.v28)
}
# view matrix
view(missing_rates)
# save matrix
...
## Create dummy vars
```{r dummy_cols SEM}
results_sem <- dummy_cols(testdf_sem2, select_columns = c(
    # outcome var
    "si_type",
    # diathesis
    "age", "sexual", "gender_noncis", "sib_freq", "race", "ed_any", "dx_dep", "dx_bi", "dx_anx",
