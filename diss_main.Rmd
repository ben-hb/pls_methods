...
SI_model_new<- '
                # measurement model
                    ...
                    diaenv =~  medther + dx_mh + sib_freq + sexual_2 + sexual_3 + gender_tgd + ed_any
                               
                    le =~  psyhx + fincur + drugs_yn + assault + ins_no
                ...
                # structural model - regressions
                    ...
                    si ~ diaenv + le + deftrap + mmtsm*deftrap 
...
# Prep SEM data vars
traindf_sem2 <- traindf.v28 %>% select(
  # outcome var
  si_type,
  # diathesis
  age, sexual, gender_tgd, sib_freq, ed_any, dx_mh, meds_count, race,
  # environment
  religios, school2_type, activ_no, international, discrim, residenc, medther,
  # life events
  ins_no, gpa_sr, binge, psyhx, fincur, drugs_yn, assault, drug_mar,
  # defeat
  aca_impa, anx_type, dep_type, anx_score, deprawsc,
  # threats to self-moderation
  persist, aaq_dum,
  # entrapment
  dep_impa, gad7_impa,
  # motivational moderators
  inf, flourish_dum, belong1, satisfied_overall, mh_stigma, talk)  #mh_stigma, 

# create sem matrix 
traindf_sem2 <- traindf_sem2 %>% 
  # mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
...
## Dummy col SEM w/ continuous
results_tr_com1 <- dummy_cols(traindf_sem2, select_columns = c("si_type", "sexual", "race"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
#view(results_tr_com1)
...
SI_model_new<- '
                # measurement model
                    ...
                    prem =~    sib_freq + sexual_2 + sexual_3 + gender_tgd + race_2 + race_3 + race_4 +
                               race_5 + race_6 + race_7 + race_8 + race_9 +
                               ed_any + dx_mh + activ_no + ins_no + meds_count + medther +
                               gpa_sr + binge + psyhx + fincur + drugs_yn + assault + drug_mar
                          
                    deftrap =~ deprawsc + aca_impa + anx_score + dep_impa + gad7_impa
...
SI_model_new2<- '
                # measurement model
                    ...
                    diaenv =~  dx_mh + medther + sib_freq + sexual_2 + sexual_3 + gender_tgd + ed_any + discrim
                   
                    le =~  psyhx + gpa_sr + fincur + drugs_yn + assault + ins_no 
                    
                    deftrap =~ deprawsc + aca_impa + anx_score + dep_impa + gad7_impa
                              
                    mmtsm =~ aaq_dum + flourish_dum + inf + belong1 + persist 

                # structural model - regressions
                
                    si ~ diaenv + le + deftrap + deftrap*mmtsm

                #  residual covars '

fit_trainM5 <- sem(SI_model_new2, data=results_tr_com1, ordered=T)
summary_train5 <- summary(fit_trainM5, fit.measures=TRUE, standardized = TRUE, rsquare=T)
fitMeasures(fit_trainM5, fit.measures = "all")
parameterestimates(fit_trainM5, boot.ci.type = "bca.simple", standardized = TRUE) 

modificationindices(fit_trainM5, minimum.value = 20)
