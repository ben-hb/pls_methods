...
```{r train talk}
# traindf.v26$talk1_1[is.na(traindf.v26$talk1_1)] <- 0
# traindf.v26$talk1_2[is.na(traindf.v26$talk1_2)] <- 0
# traindf.v26$talk1_3[is.na(traindf.v26$talk1_3)] <- 0
# traindf.v26$talk1_4[is.na(traindf.v26$talk1_4)] <- 0
# traindf.v26$talk1_5[is.na(traindf.v26$talk1_5)] <- 0
# traindf.v26$talk1_6[is.na(traindf.v26$talk1_6)] <- 0
# traindf.v26$talk1_7[is.na(traindf.v26$talk1_7)] <- 0
# traindf.v26$talk1_8[is.na(traindf.v26$talk1_8)] <- 0
...
traindf.v26 <- traindf.v26 %>% 
  ...
  mutate(talk = case_when(talk == 1 ~ 1,
                          ...
                          talk == 4 ~ 4)) %>%
  filter(!is.na(talk))
traindf.v26$talk <- as.factor(traindf.v26$talk)
...
# TSM as a moderator
summary(lm(entrap~def+tsm+def*tsm, data=results_tr_com1))

# # ENTRAPMENT AS A MEDIATOR
fit.totaleffect2 <- clm(si~def, data=results_tr_com1)
summary(fit.totaleffect2)

fit.mediator2 <- lm(entrap~def, data=results_tr_com1)
summary(fit.mediator2)

fit.dv2 <- clm(si~def+entrap, data=results_tr_com1)
summary(fit.dv2)
...
```{r sub 18-19_frame}
test18.v28 <- test1.v28 %>%
  filter(age == 18 | age == 19)

# Prep SEM data vars
test18_sem <- test18.v28 %>% select(
  # outcome var
  si_type,
  # diathesis
  age, sexual, gender_tgd, sib_freq, ed_any, dx_mh, meds_count,
...
## Dummy col SEM w/ continuous
results_18_com <- dummy_cols(test18_sem, select_columns = c("si_type", "sexual", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)

# Under model
fit_testUn <- sem(SI_model_testM, data=results_18_com, ordered=T)
...
```{r 20-25 frame}
test20.v28 <- test1.v28 %>%
  filter(age == 20 | age == 21 | age == 22 | age == 23| age == 24 | age == 25)

# Prep SEM data vars
test20_sem <- test20.v28 %>% select(
  # outcome var
  si_type,
  # diathesis
  age, sexual, gender_tgd, sib_freq, ed_any, dx_mh, meds_count,
...
## Dummy col SEM w/ continuous
results_20_com <- dummy_cols(test20_sem, select_columns = c("si_type", "sexual", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)

#Upper model
fit_testUp <- sem(SI_model_testM, data=results_20_com, ordered=T)
