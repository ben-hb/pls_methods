...
data_sem <- data_sem %>% 
  ...
  dummy_cols(remove_first_dummy = TRUE) %>% 
  data.matrix()
data_sem   # check output

results_sem <- dummy_cols(data_sem, select_columns = c(
    # diathesis
    "age", "sexual", "gender_noncis", "finpast", "sib_freq", "race", "body_sr", "ed_any", "audit_tot", "dx_dep", "dx_bi",
    "dx_tr", "dx_adhd", "dx_pers", "meds_sti", "meds_dep", "meds_anx", "meds_mood", "meds_sle", "meds_count",
    # environment
...
## SEM data train
```{r SEM}
traindf.v29 <- NA_preproc(traindf.v29)
traindf.v29 <- as.data.frame(traindf.v29)

SI_model_tr <- '# measurement model
                dia_sem =~ age + sexual + gender_noncis + finpast + sib_freq + race + body_sr + ed_any + audit_tot + dx_dep + dx_bi + 
                           dx_tr + dx_adhd + dx_pers + meds_sti + meds_dep + meds_anx + meds_mood + meds_sle + meds_count
                env_sem =~ env_mh + ther_vis + divers + satisfied_overall + residenc + talk + religios + discrim + school2_type + 
                           activ_yn + military + international
                le_sem =~  ins_cover + gpa_sr + assault_phys + binge_fr + assault_sex + assault_emo + psyhx + fincur + drugs_yn + abuse_year
                def_sem =~ aca_impa + dep_mod + dep_sev
                tsm_sem =~ persist + anx_mod + anx_sev + dep_secret + AAQ_tot
                entrap_sem =~ dep_impa + gad7_impa + percneed_cur
                mm_sem =~ inf + flourish + belong1 + belong2 + belong8 + belong9 + stig_pcv_2 + stig_pcv_3 + BRS_tot
                si_sem =~ si_type
...
fit_train <- sem(SI_model_tr, data=data.v29)
summary(fit_train, fit.measures=TRUE, standardized = TRUE))

```

## SEM data test
```{r SEM test model}
testdf.v29 <- NA_preproc(testdf.v29)
testdf.v29 <- as.data.frame(testdf.v29)

SI_model_test <- ' # measurement model
...
fit_test <- sem(SI_model_test, data=testdf.v29)
