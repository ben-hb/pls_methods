...
ther_vis, school2, religios, residenc, firstgen
...
traindf.v13 <- traindf.v13 %>%
  ...
  mutate(religios = case_when(religios == 1 ~ 5, 
                              religios == 2 ~ 4,
                              religios == 3 ~ 3,
                              religios == 4 ~ 2,
                              religios == 5 ~ 1))
...
traindf.v14 <- traindf.v14 %>%
  mutate(no_medther = case_when(medther == 0 ~ 1,
                                medther == 1 ~ 0))
table(traindf.v14$no_medther)
```
...
traindf_prem <- traindf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       meds_count, religios, activ_no, school2_type, ins_no, residenc,
...
results_prem <- dummy_cols(traindf_prem, select_columns = c("age", "sexual", "gender_tgd", "sib_freq",
                                                            "race", "ed_any", "dx_mh", "meds_count", "religios",
...
traindf_def <- traindf.v21 %>% select(aca_impa, deprawsc, anx_score)
...
## CLM defeat
```{r train CLM defeat}
results2_def <- traindf.v21 %>% select(si_type, aca_impa, deprawsc, anx_score)
clm_def <- na.omit(results2_def)

# run defeat regression
clm_def <- clm(si_type ~ aca_impa + deprawsc + anx_score, data = clm_def)
...
```{r perneed}
table(traindf.v24$percneed, useNA = "always")
traindf.v24 <- traindf.v24 %>%
  mutate(percneed = case_when(percneed == 1 ~ 6,
                             percneed == 2 ~ 5,
                             percneed == 3 ~ 4,
                             percneed == 4 ~ 3,
                             percneed == 5 ~ 2,
                             percneed == 6 ~ 1)) %>%
  filter(!is.na(percneed))
...
traindf.v26 <- traindf.v26 %>% 
  ...
  mutate(talk = case_when((talk1_9 == 1 | talk1no == 1) ~ 1,                                    # no support
                          ...
                          (talk1_1 == 1 | talk1_6 == 1 | talk1_7 == 1 | talk1_8 == 1) ~ 0))     # formal and informal support,
...
SI_model_new<- '
                # measurement model
                    si =~ si_type_3 + si_type_2 + si_type_4
                    prem <~  medther + dx_mh + sib_freq + sexual_2 + sexual_3 + gender_tgd +
                             psyhx + fincur + drugs_yn + assault
                    deftrap =~ dep_type + percneed + anx_type
                    mmtsm =~ aaq_dum + flourish_dum + persist + inf
                    deftrap_c =~ deftrap - mean(deftrap)
                    prem_c =~ prem - mean(prem) 
                    
               # a path
                    prem_c ~ 1 + a1 * deftrap_c
                    prem_c ~ a2 * mmtsm
...
summary_train3 <- summary(fit_trainM3, fit.measures=T, standardized = T, rsquare=T)
fitMeasures(fit_trainM3, fit.measures = "all")
parameterestimates(fit_trainM3, boot.ci.type = "bca.simple", standardized = T) 

modificationindices(fit_trainM3, minimum.value = 20)

semPaths(fit_trainM3)
fit.totaleffect=lm(si~deftrap, results_tr_com1)
...
SI_model_new<- '
                # measurement model
                    ...
                    prem =~    sib_freq + sexual_2 + sexual_3 + gender_tgd +
                               ...
                               gpa_sr + binge + psyhx + fincur + drugs_yn + assault
                          
                    deftrap =~ dep_type + aca_impa + anx_type + dep_impa + gad7_impa
                              
                    mmtsm =~   aaq_dum + persist + flourish_dum + inf + belong1 + satisfied_overall + mh_stigma '

# results <- gemm(results_tr_com1[results_tr_com1], xvar = "prem", mvar = c("deftrap"), mymod = "mmtsm", yvar = "si", estMethod = "bootstrap")
...
# Pull file
# data.pls <- read_excel("~/Desktop/results_pls_tr.xlsx",
...
testdf.v10 <- testdf.v10 %>%
  ...
  mutate(ed_any = case_when(ed_any == 1 ~ 1,
                            ed_any == 0 ~ 0))
testdf.v10$ed_any <- as.logical(testdf.v10$ed_any)
...
testdf.v14 <- testdf.v13 %>%
  mutate(international = case_when(international == 1 ~ 0,
                                   international == 0 ~ 1)) %>%
  filter(!is.na(international))
...
SI_model_test1 <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual_2 + sexual_3 + gender_tgd + assault +
                           ed_any + dx_mh + meds_count + psyhx + fincur + drugs_yn 
...
```{r}
compareFit(fit_testUp, fit_testUn)
```
