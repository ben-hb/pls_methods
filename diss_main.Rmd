...
m_dia <- clm(si_type ~ (age + sexual + gender + finpast + sib_freq + race + disab_1 + ed_scoff + 
               ...
               body_sr + dx_dep + dx_bi + dx_psy + dx_adhd)^2, data = train_dia)
summary(m_dia)
...
data.v16 <- data.v14  # new dataframe for analysis
...
data.v16$drug_none[is.na(data.v16$drug_none)] <- 0
...
data_le <- data.v19 %>% select(ins_cover, abuse_year, gpa_sr, aca_impa, fincur, assault_phys, 
...
clm_le <- data.v19 %>% select(si_type, ins_cover, abuse_year, gpa_sr, aca_impa, fincur, assault_phys, 
                              binge_fr, assault_sex, assault_emo, deprawsc, psyhx, drug_type)
...
# residual plot
le_data$resid <- resids(m_le, method = c("jitter"),
     jitter.scale = c("response"))
le_data <- cbind(le_data, m_le[["fitted.values"]])
ggplot(le_data, aes(x = si_type, y = resid)) +
geom_point(color = "purple", alpha = 0.15) +
geom_smooth(method = "lm")
...
## Academic failure
```{r failed}
table(data.v20$failed, useNA = "always")
...
n= 8916 ** too small
```{r men_ill_ash}
table(data.v20$men_ill_ash, useNA = "always")
...
data_def <- data.v21 %>% select(si_type_num, aca_impa, deprawsc)
view(data_def)
...
## GAD7 total
```{r anx_score}
table(data.v22$anx_score, useNA = "always")
```

## Secretive MH
```{r dep_secret}
table(data.v22$dep_secret, useNA = "always")
```
...
table(data.v22$AAQ_tot, useNA = "always")
```

## Achievement
n = 594  ** too small
```{r achieve4}
table(data.v22$achieve4, useNA = "always")
```

## Persist
```{r persist}
table(data.v22$persist, useNA = "always")
```
...
## CLM TSM
```{r CLM TSM}
data.v23$persist <- as.factor(data.v23$persist)
data.v23$anx_score <- as.integer(data.v23$anx_score)
data.v23$dep_secret <- as.factor(data.v23$dep_secret)
data.v23$AAQ_tot <- as.integer(data.v23$AAQ_tot)

clm_tsm <- data.v23 %>% select(si_type, persist, anx_score, dep_secret, AAQ_tot)
...
# sure fx
tsm_data <- m_tsm[["model"]]

# residual plot
tsm_data$resid <- resids(tsm_data, method = c("jitter"),
      jitter.scale = c("response"))
tsm_data <- cbind(tsm_data, mod_tsm[["fitted.values"]])
ggplot(tsm_data, aes(x = si_type, y = resid)) +
geom_point(color = "purple", alpha = 0.15) +
geom_smooth(method = "lm")
...
ggplot(entrap_data, aes(x = si_type, y = resid)) + 
  geom_point(color = "purple", alpha = 0.15) + 
  geom_smooth(method = "lm")

entrap_data %>% group_by(si_type) %>% summarize(mean = mean(resid, na.rm = TRUE), sd = sd(resid, na.rm = TRUE))
entrap_data %>% filter(si_type == "Passive SI") %>% ggplot(aes(x = resid)) + geom_density(color = "magenta")
...
table(data.v26$belong2)
table(data.v26$belong8)
table(data.v26$belong9)
```

## Achievement
** too small for inclusion in model
```{r achieve_1_14_18}
table(data.v26$achieve1, useNA = "always")
table(data.v26$achieve14, useNA = "always")
table(data.v26$achieve18, useNA = "always")
...
## Flourishing Scale
```{r flourish}
table(data.v26$flourish, useNA = "always")
```
...
data.v26$inf_7[is.na(data.v26inf_7)] <- 0
...
```{r friend_devel}
table(data.v26$friend_devel, useNA = "always")
...
```{r stig_pcv_2}
table(data.v26$stig_pcv_2, useNA = "always")
```

## Perceived MH tx as negative
```{r stig_pcv_3}
table(data.v26$stig_pcv_3, useNA = "always")
```
...
data_mm <- data.v27 %>% select(inf, flourish, belong1, belong2, belong8, belong9, stig_pcv_2, stig_pcv_3, brs_tot)
data <- data_mm
sig <- 0.6
...
corr <- as.data.frame(as.table(corr))                   #turn into a 3-column table
corr <- na.omit(corr)                                   #remove the NA values from above 
mm_corr <- corr[order(-abs(corr$Freq)),]                   #sort by highest correlation
print(MM_corr)
...
clm_mm <- data.v27 %>% select(si_type, inf, flourish, belong1, belong2, belong8, belong9, stig_pcv_2, stig_pcv_3, brs_tot)
...
ggplot(mm_data, aes(x = si_type, y = resid)) + 
  geom_point(color = "purple", alpha = 0.15) + 
  geom_smooth(method = "lm")
...
for (i in 1:nrow(school2)) {   # i corresponds to rows, j to columns
  school2[i, 2 + j] <- sum(!is.na(data.v23[data.v28$school2 == school2[[i,1]], var_names[j]]))
}
}
...
#build table for absolute missingness based on school2
#create skeleton of missingness matrix
var <- count(data.v28, data.v23[var_names[k]])
var_ncol <- ncol(var)
...
set.seed(1000) 
split = sample.split(data.v29$school2, SplitRatio = 0.50)
 
# Create training and testing sets
traindf.v29 <- subset(data.v29, split == TRUE)
testdf.v29 <- subset(data.v29, split == FALSE)
...
fit_train <- sem(SI_model_tr, data=traindf.v29)
...
fit_train <- sem(model, data=data.v29)
summary(fit_train, fit.measures=TRUE)


fit_test <- sem(SI_model_tr, data=testdf.v29)
