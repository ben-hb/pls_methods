...
traindf.v11$meds_ther <- as.logical(traindf.v11$meds_ther)

table(traindf.v11$meds_ther, useNA = "always")
```
...
clm_env <- traindf.v15 %>% select(si_type, religios, residenc, activ_no, school2_type, international)
...
traindf_mm <- traindf.v27 %>% select(inf, flourish_dum, belong1, mh_stigma, satisfied_overall)
...
clm_mm <- traindf.v27 %>% select(si_type, inf, flourish_dum, belong1, satisfied_overall, mh_stigma)
clm_mm <- na.omit(clm_mm)

# run MM regression
clm_mm <- clm(si_type ~ inf + flourish_dum + belong1 + satisfied_overall + mh_stigma, data = clm_mm)
summary(clm_mm)
...
SI_model_tr1 <- '
                # measurement model
                  ...
                  prem =~  sib_freq + sexual_2 + sexual_3 + gender_tgd +
                           ed_any + dx_mh + activ_no + ins_no + meds_ther + gpa_sr + 
                           psyhx + drugs_yn + assault 
                          
                  def =~ dep_type + aca_impa + anx_type
                              
                  tsm =~ aaq_dum + persist

                  entrap =~ dep_impa + percneed

                  mm =~ flourish_dum + inf + satisfied_overall
...
SI_model_trM <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual_2 + sexual_3 + gender_tgd + meds_ther +
                           ed_any + dx_mh + activ_no + ins_no + 
                           gpa_sr + psyhx + drugs_yn + assault
...
SI_model_trM2 <- '
                # measurement model
                  ...
                  prem  =~ sib_freq + sexual_2 + sexual_3 + gender_tgd + assault +
                           ed_any + dx_mh + psyhx + drugs_yn + medther
...
testdf.v11 <- testdf.v11 %>%
  mutate(meds_ther = case_when(meds_any == 1 ~ 1,
                              ...
                              meds_any == 0 ~ 0)) %>%
  filter(!is.na(meds_ther))
testdf.v11$meds_ther <- as.logical(testdf.v11$meds_ther)

table(testdf.v11$meds_ther, useNA = "always")
```


## DIATHESIS ANALYSES
```{r test diathesis analyses}
testdf.v12 <- testdf.v11 # create new dataframe for summary stats. Will jump to v13 for environmental
```

### Diathesis correlation matrix
```{r test diathesis correlation}
...
### EFA diathesis
```{r test PCA diathesis}
# create dummy columns for EFA analysis
pca_dia <- data.matrix(testdf_dia)
results_dia <- dummy_cols(pca_dia, select_columns = c("age", "sexual", "gender_tgd", "finpast", 
                                                       "sib_freq", "race", "dx_mh", "ed_any", "meds_ther"), 
                          remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
### Environmental correlation
```{r test env correlation}
# Define environment vars for correlation matrix
testdf_env <- testdf.v15 %>% select(discrim, religios, residenc, activ_no, divers, 
                                      school2_type, military, international)
...
##EFA Environment
```{r test EFA Environment}
# PCA map with eigenvalues
testdf_env <- NA_preproc(testdf_env)
testdf_env <- as.matrix(testdf_env)
results_env <- dummy_cols(testdf_env, select_columns = c("religios", "residenc", "activ_no",
                                                         "school2_type", "international"),
                          remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
### CLM environmental regression
```{r test CLM environment}
# preparing dataframe for regression
clm_env <- testdf.v15 %>% select(si_type, religios, residenc, activ_no, school2_type, international)
clm_env <- NA_preproc(clm_env) %>% as.data.frame()
...
# Define life event var candidates for correlation matrix
testdf_le <- testdf.v19 %>% select(ins_no, gpa_sr, fincur, binge,
                                     assault, drugs_yn, drug_mar, psyhx)
...
## EFA LE
```{r test le_pca}
testdf_le <- testdf.v19 %>% select(ins_no, gpa_sr, fincur, assault,
                                     binge, drugs_yn, drug_mar, psyhx)
...
results_le <- dummy_cols(testdf_le, select_columns = c("ins_no", "gpa_sr", "fincur",
                                                        "binge", "drugs_yn", "drug_mar", 
                                                        "assault", "psyhx"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
```{r test clm_LE}
# prepare dataframe for regression
clm_le <- testdf.v19 %>% select(si_type, ins_no, gpa_sr, assault, binge, drugs_yn, 
                                 drug_mar, psyhx, fincur)
clm_le <- na.omit(clm_le)
...
```{r test results_prem}
# Define diathesis vars for correlation matrix
testdf_prem <- testdf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       meds_ther, religios, activ_no, school2_type, fincur, ins_no, residenc,
                                       gpa_sr, assault, drugs_yn, drug_mar, psyhx)
...
```{r test prem_pca}
# Define diathesis vars for correlation matrix
testdf_prem <- testdf.v19 %>% select(age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       meds_ther, religios, activ_no, school2_type, ins_no, residenc,
                                       gpa_sr, assault, drugs_yn, drug_mar, psyhx, fincur)
...
results_prem <- dummy_cols(testdf_prem, select_columns = c("age", "sexual", "gender_tgd", "sib_freq",
                                                            "race", "ed_any", "dx_mh", "meds_ther", "religios",
                                                            "activ_no", "school2_type", "ins_no", "residenc",
                                                            "gpa_sr", "assault", "drugs_yn","drug_mar", 
                                                            "psyhx", "fincur"),
...
## CLM Premotivation regression
```{r test clm_prem}
clm_prem <- testdf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                 religios, activ_no, school2_type, ins_no, residenc, meds_ther,
                                 gpa_sr, assault, drugs_yn, drug_mar, psyhx, fincur)
clm_prem <- na.omit(clm_prem)
...
```{r test defeat cor}
# Define defeat variable candidates for correlation matrix
testdf_def <- testdf.v21 %>% select(aca_impa, anx_type, dep_type)
...
results_def <- dummy_cols(testdf_def, select_columns = c("aca_impa", "depr_type", "anx_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
## TSM analyses
```{r test TSM var analyses}
testdf.v23 <- testdf.v22
```

## Threats to self-moderation correlation
```{r test tsm correlation}
# Define TSM variable candidates for correlation matrix
...
table(testdf.v24$dep_impa, useNA = "always")  # check output, NAs, skew
...
table(testdf.v24$percneed, useNA = "always") # check output, NAs
```
...
## ENTRAPMENT ANALYSES
```{r test entrapment analyses}
testdf.v25 <- testdf.v24
```

### Entrapment correlation
```{r test entrapment corr}
# Define entrapment variable candidates for correlation matrix
testdf_entrap <- testdf.v25 %>% select(dep_impa, gad7_impa, percneed)
...
###  CLM Entrapment regression
```{r test clm_entrap}
clm_entrap <- testdf.v25 %>% select(si_type, dep_impa, gad7_impa, percneed)
clm_entrap <- na.omit(clm_entrap)
...
```{r test MM_corr}
# Define MM variable candidates for correlation matrix
testdf_mm <- testdf.v27 %>% select(inf, flourish_dum, belong1, mh_stigma, satisfied_overall)
...
results_mm <- dummy_cols(testdf_mm, select_columns = c("flourish_dum", "belong1", "inf", 
                                                        "mh_stigma", "satisfied_overall"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
mm_pca <- PCA(results_mm, graph=FALSE)  # apply PCA
eigval_mm <- get_eigenvalue(mm_pca)
...
```{r test CLM mm}
# prepare data for regression
clm_mm <- testdf.v27 %>% select(si_type, inf, flourish_dum, belong1, satisfied_overall, mh_stigma)
clm_mm <- na.omit(clm_mm)
...
## Model w/ levels correlations
```{r test model_lev_corr}
results_corr <- NA_preproc(testdf_sem)
sig <- 0.4
...
SI_model_new2 <- '
                # measurement model
                    ...
                    prem =~    sib_freq + sexual_2 + sexual_3 + gender_tgd +
                               ed_any + dx_mh + medther + gpa_sr + psyhx + 
                               drugs_yn + assault
                          
                    deftrap =~ dep_type + anx_type + dep_impa
                              
                    mmtsm =~   aaq_dum + flourish_dum + inf

                 # structural model - regressions
                   si ~ prem + deftrap + mmtsm + deftrap*mmtsm '

fit_testM4 <- sem(SI_model_new2, data=results_test_com1, ordered=T)
...
# Under model
fit_testUn <- sem(SI_model_new2, data=results_18_com, ordered=T)
summary_testUn <- summary(fit_testUn, fit.measures=TRUE, standardized = TRUE)
fitMeasures(fit_testUn, fit.measures = "all")


# test new model
fit_test2_un <- sem(SI_model_new2, data=results_18_com, ordered=T)
...
#Upper model
fit_testUp <- sem(SI_model_new2, data=results_20_com, ordered=T)
summary_testUp <- summary(fit_testUp, fit.measures=TRUE, standardized = TRUE)
fitMeasures(fit_testUp, fit.measures = "all")

# test new model
fit_test2_upp <- sem(SI_model_new2, data=results_20_com, ordered=T)
