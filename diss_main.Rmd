...
results_env2 <- dummy_cols(data_env2, select_columns = c("si_type", "discrim", "religios", "env_mh", "residenc", "ther_vis",
                                                       ...
                                                       "ther_vis", "talk", "military", "international"),
                          remove_first_dummy = TRUE)
clm_env <- NA_preproc(results_env2)
clm_env <- as.data.frame(results_env2)

randomForest(si_type ~ discrim + satisfied_overall + ther_vis + talk + activ_yn + international + 
                ther_vis + divers + + military + school2_type + residenc + env_mh, data = clm_env)

## for plotting
colors = rainbow(length(unique(results_env2)))
names(colors) = unique(results_env2)
