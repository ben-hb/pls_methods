...
table(data.v10$audit_tot)

data.v10 = subset(data.v10, select= -c(audit_1:audit_10, audit_3_m, audit_3_f, audit_3_o)) # clean up vars/columns

```

## Eating disorder, any type
```{r ed_any}
table(data.v10$ed_any)
```
...
data.v13$discrim <- as.factor(data.v13$discrim)
```
...
data.v14$military <- as.logical(data.v14$military)
```

## Overall school satisfaction
```{r satisfied_overall}
table(data.v14$satisfied_overall)
data.v14$satisfied_overall <- as.factor(data.v14$satisfied_overall)
```
...
data.v16$fincur <- as.factor(data.v16$fincur)
```
...
data.v18$binge_fr <- as.factor(data.v18$binge_fr)
...
data.v20$aca_impa <- as.factor(data.v20$aca_impa)
```
...
data.v22$anx_score <- as.numeric(data.v22$anx_score)
...
data.v22$dep_secret <- as.factor(data.v22$dep_secret)
```
...
data.v22$aaq_tot <- as.numeric(data.v22$aaq_tot)
...
## Persist
```{r persist}
data.v22$persist <- as.factor(data.v22$persist)
...
data.v24$dep_impa <- as.factor(data.v24$dep_impa)
```

## Anxiety impact
```{r gad7_impa}
table(data.v24$gad7_impa)
data.v24$gad7_impa <- as.factor(data.v24$gad7_impa)
```

## Current needs
```{r perneed_cur}
table(data.v24$percneed_cur)
data.v24$percneed_cur <- as.factor(data.v24$percneed_cur)
```
...
data.v26$belong1 <- as.factor(data.v26$belong1)
...
table(data.v26$inf)
data.v26$inf <- as.factor(data.v26$inf)
...
school2_all <- data.frame(school2[, 1:2], props)

# view missingness matrix
view(school2_all)

# save matrix
write_xlsx(school2_all, path = "sch2_missing_all.xlsx", col_names = TRUE, format_headers = TRUE)
```

## Missingness of each overall variable
```{r Missingness of each overall var}
# Create dataframe to store data 
missing_rates_all <- data.frame("variable" = var_names,
           "total" = rep(999, length(var_names)),
           "prop_present" = rep(999, length(var_names)))

# loop to replicate row for all var
for (i in 1:nrow(missing_rates_all)) {
  missing_rates_all[i, 2] = sum(!is.na(data.v28[[i]]))
  missing_rates_all[i, 3] = missing_rates[i, 2] / nrow(data.v28)
}
# view matrix
view(missing_rates_all)
# save matrix
write_xlsx(missing_rates_all, path = "overall_missing_all.xlsx", col_names = TRUE, format_headers = TRUE)
```
...
save(missing_dfs, file = "missing_dfs_all.RData")
load("missing_dfs_all.RData")
write_xlsx(missing_dfs, path = "missing_all.xlsx", col_names = TRUE, format_headers = TRUE)
```
...
# xlsx output
data_sem3 <- as.data.frame(data_sem2)
save(data_sem3 , file = "data_nondummy_all.RData")
load("data_nondummy_all.RData")
write_xlsx(data_sem3, path = "data_nondummy_all.xls", col_names = TRUE, format_headers = TRUE)
```
...
```{r Var_sum}
# training data
var_desc_all <- describe(data_sem2) %>% view()

# xlsx output
save(var_desc_all, file = "var_desc_all.RData")
load("var_desc_all.RData")
write_xlsx(var_desc_all, path = "var_desc_all.xls", col_names = TRUE, format_headers = TRUE)
```
...
save(results_sem, file = "data_sem_all.RData")
load("data_sem_all.RData")
write_xlsx(results_sem, path = "data_sem_all.xls", col_names = TRUE, format_headers = TRUE)
```
...
fit_train$prediction <- predict(SI_model_test, newdata = data.v29, type = "response")
