...
# PREPARING SAMPLE FOR AGES 18 and 19
...
data_sem2_1819 <- data_sem_1819 %>% 
  mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
data_sem2_1819   # check output

# xlsx output
data_sem3_1819 <- as.data.frame(data_sem2_1819)
save(data_sem3_1819, file = "data_nondummy_1819.RData")
load("data_nondummy_1819.RData")
write_xlsx(data_sem3_1819, path = "data_nondummy_1819.xls", col_names = TRUE, format_headers = TRUE)
```

## Create dummy vars
```{r dummy_cols SEM}
results_sem_1819 <- dummy_cols(data_sem2_1819, select_columns = c(
    # outcome var
    "si_type",
    # diathesis
...
```{r Var_sum}
# data
var_desc_1819 <- describe(data_sem_1819) %>% view()

# xlsx output
save(var_desc_1819, file = "var_desc_1819.RData")
load("var_desc_1819.RData")
write_xlsx(var_desc_1819, path = "var_desc_1819.xls", col_names = TRUE, format_headers = TRUE)
```


## Model correlations
```{r model_corr}
results_corr_1819 <- NA_preproc(results_sem_1819)
results_corr_1819 <- as.data.frame(results_corr_1819)

# run a correlation and drop the insignificant ones
sig <- 0.8
corr <- cor(results_corr_1819, use = "pairwise.complete.obs")                               
corr[lower.tri(corr,diag=TRUE)] <- NA                   # prepare to drop duplicates and correlations of 1    
corr[corr %in% c(-1, 1)] <- NA                          # drop perfect correlations
corr <- as.data.frame(as.table(corr))                   #turn into a 3-column table
corr <- na.omit(corr)                                   #remove the NA values from above 
...
```{r save dataset}
results_sem_1819 <- as.data.frame(results_sem_1819)

# save full var set for SEM
save(results_sem_1819, file = "data_sem_1819.RData")
load("data_sem_1819.RData")
write_xlsx(results_sem_1819, path = "data_sem_1819.xls", col_names = TRUE, format_headers = TRUE)
```
...
# Model estimation
results_sem_1819 <- as.matrix(results_sem_1819)

fit <- cfa(SI_model_tr, data = results_sem_1819,
             ordered = c("age","si_type", "sib_freq","meds_count", "binge_fr", "deprawsc",
                         "anx_score", "aaq_tot", "dep_impa", "gad7_impa", "flourish", "brs_tot"))

fit_1819 <- sem(SI_model_1819, data=results_sem_1819)

summary_1819 <- summary(fit_1819, fit.measures=TRUE, standardized = TRUE)

list(summary_1819$coefficient,
...
fitmeasures(fit_1819)

# model diagnostics
fitted(fit_1819)
inspect(fit_1819, what="cor.all")
...
resid(fit_1819, "cor")
plot_matrix(residuals(fit_1819, type="cor")$cor)

# model prediction
fit_1819$prediction <- predict(SI_model_test, newdata = results_sem_1819, type = "response")

# distribution of the prediction score grouped by outcome
ggplot(fit_1819, aes(prediction, color = as.factor(left) ) ) +
geom_density( size = 1 ) +
...
modificationindices(fit_1819, minimum.value = 20)
fit_1819_2 <- update(fit_1819, add="nox ~ log_crim")
summary(fit_1819_2, fit.measures=TRUE)
