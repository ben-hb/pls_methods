...
### 'Sexual' transformed 
sexual identity: 1. Heterosexual, 2. Gay/Lesbian, 3. Bisexual, 4. Queer/non-binary, 5. Questioning, 6. Non-answers
```{r sexual regroup}
data.v5 <- data.v4 %>%
    filter(!is.na(sexual)) %>% 
  # aggregates all 'sexual' answers into one var
  mutate(
    sexual = case_when(sexual %in% c("Pansexual", "Queer", "Asexual/Demi") ~ 4, 
                       sexual %in% c("Questioning") ~ 5,
                       sexual %in% c("Gay", "Lesbian") ~ 2,
                       sexual == "Bisexual" ~ 3,
                       sexual == "Heterosexual" ~ 1,
                       sexual == "Other" ~ 4,
                       sexual == "Non-answers" ~ 6))        
...
data.v6$race[str_detect(data.v6$race_other_text, "[B|b]iracial")] <- 7
data.v6$race[str_detect(data.v6$race_other_text, "[B|b]i-racial")] <- 7
data.v6$race[str_detect(data.v6$race_other_text, "[M|m]ix")] <- 7
data.v6$race[str_detect(data.v6$race_other_text, "[H|h]alf")] <- 7
data.v6$race[str_detect(data.v6$race_other_text, "[I|i]ndo-[C|c]arribean")] <- 7
data.v6$race[str_detect(data.v6$race_other_text, "[M|m]ulti")] <- 7
data.v6$race[str_detect(data.v6$race_other_text, "[M|m]ulatto")] <- 7
data.v6$race[str_detect(data.v6$race_other_text, "[P|p]anda")] <- 7
data.v6$race[str_detect(data.v6$race_other_text, "[O|o]ther")] <- 7
data.v6$race[str_detect(data.v6$race_other_text, "[U|u]nkown")] <- 7
data.v6$race[str_detect(data.v6$race_other_text, "[A|a]mbiguous")] <- 7
data.v6$race[str_detect(data.v6$race_other_text, "[A|a]mbiguous")] <- 7
data.v6$race[data.v6$race_other_text %in% c("Sub Asian/East African", "two or more races", "Moroccan Mexican",
                                            ...
                                            "An American Mutt- a bit of everything somewhere in there")] <- 7 
  # Identifies as black and white as 'Other/Mixed'
data.v6$race[data.v6$race_other_text %in% c("Black/White","White/African American", "White/African American")] <- 7
  # Identifies as Asian and White as 'Other/Mixed'
data.v6$race[str_detect(data.v6$race_other_text, "[E|e]urasian")] <- 7
data.v6$race[data.v6$race_other_text %in% c("Japanese, Chinese, French, British, Greek", "Asian/White",
                                            ...
                                            "White & Asian", "Chinese and White")] <- 7
...
data.v6$race[data.v6$race_other_text %in% c("I am 1/4th Hispanic 3/4 German",  "Honduran, Irish", "White hispanic",
                                            ...
                                            "If I identify as puerto rican does that mean i get more scholarship money?")] <- 7
	 # Identifies as MR and White as 'Other/Mixed'
data.v6$race[data.v6$race_other_text %in% c("Half Middle Eastern half white", 
                                            "Half my family is white and the other middle eastern")] <- 7
...
# turn all NA/blank into -99

# Turn all NAs/blanks into -99
#results_mplus <- results_sem[is.na(results_sem)] <- "-99"
...
load("data_sem.RData")
write.csv(results_sem,"/Users/shawnasisler/Desktop/diss_main.csv", row.names = FALSE)

# save training data
save(traindf.v29, file = "traindf_diss.RData")
load("traindf_diss.RData")
write.csv(traindf.v29,"/Users/shawnasisler/Desktop/traindf_0608.csv", row.names = FALSE)

# save test data
save(testdf.v29, file = "testdf_diss.RData")
load("testdf_diss.RData")
write.csv(testdf.v29,"/Users/shawnasisler/Desktop/traindf_0608.csv", row.names = FALSE)
```
