---
title: "Sisler Full Data Description"
output:
...
# create si_type var
  ...
  mutate(si_activ = case_when(sui_plan == 1 ~ 1,
...
table(data.v11$meds_any, useNA = "always")
```
...
data.v20$dep_impa <- as.numeric(data.v20$dep_impa)

table(data.v20$dep_impa, useNA = "always")  # check output, NAs, skew
jarque.bera.test(data.v20$dep_impa)
```

## Anxiety impact
```{r train gad7_impa}
data.v20$gad7_impa <- as.numeric(data.v20$gad7_impa)
table(data.v20$gad7_impa, useNA = "always") # check output, skew, NAs
```
...
table(data.v20$percneed, useNA = "always") # check output, NAs
```
...
data_def <- data.v21 %>% select(aca_impa, anx_score, deprawsc, percneed, gad7_impa, dep_impa)
...
```{r train EFA_def}
data_def <- data.v21 %>% select(aca_impa, deprawsc, anx_score, percneed, gad7_impa, dep_impa)
...
results2_def <- data.v21 %>% select(si_type, aca_impa, deprawsc, anx_score, percneed, gad7_impa, dep_impa)
...
table(data.v22$belong1, useNA = "always")

data.v22$belong1 <- as.numeric(data.v22$belong1)
summary(data.v22$belong1)
jarque.bera.test(data.v22$belong1)

table(data.v22$belong2)  # elective question, but common
table(data.v22$belong8)  # nested x 2
table(data.v22$belong9)  # not as good of a proxy
```

## Flourishing Scale
```{r train flourish}
data.v22$flourish <- as.numeric(data.v22$flourish)

#  for loop for predictive cutoff point needed
rs_df <- data.frame("cutoff" = c(), "rs" = c())
for (i in 9:55) {
  flourish_bin <- ifelse(data.v22$flourish >= i, TRUE, FALSE)
  model = lm(data.v22$sui_idea ~ flourish_bin)
  rs_df <- rbind(rs_df, c(i, summary(model)$r.squared))
...
data.v23$flourish <- as.numeric(data.v23$flourish)
table(data.v23$flourish, useNA = "always")
```

## Informal Mental Health Support
```{r train inf}
data.v23$inf_1[is.na(data.v23$inf_1)] <- 0
data.v23$inf_2[is.na(data.v23$inf_2)] <- 0
data.v23$inf_3[is.na(data.v23$inf_3)] <- 0
data.v23$inf_4[is.na(data.v23$inf_4)] <- 0
data.v23$inf_5[is.na(data.v23$inf_5)] <- 0
data.v23$inf_6[is.na(data.v23$inf_6)] <- 0
data.v23$inf_7[is.na(data.v23$inf_7)] <- 0
...
data.v24$inf <- as.logical(data.v24$inf)

table(data.v24$inf, useNA ="always") # check output, skew
jarque.bera.test(data.v24$inf)
data.v24 = subset(data.v24, select= -c(inf_1:inf_help))  # clean up vars
```
...
table(data.v24$brs_tot)
data.v24$brs_tot <- as.numeric(data.v24$brs_tot)

#  for loop for predictive cutoff point needed
rs_df <- data.frame("cutoff" = c(), "rs" = c())
for (i in 6:24) {
  brs_bin <- ifelse(data.v24$brs_tot >= i, TRUE, FALSE)
  model = lm(data.v24$sui_idea ~ brs_bin)
  rs_df <- rbind(rs_df, c(i, summary(model)$r.squared))
}
...
data.v24 <- data.v24 %>%
  ...
  mutate(brs_dum = case_when(brs_dum == 1 ~ 1,
                             brs_dum == 0 ~ 0))
data.v24$brs_dum <- as.logical(data.v24$brs_dum)

# output
rs_df_arranged[1,1]
table(data.v24$brs_tot, useNA = "always")
table(data.v24$brs_dum, useNA = "always")
```

## Distress & social support
```{r train talk}
data.v24$talk1_1[is.na(data.v24$talk1_1)] <- 1
data.v24$talk1_2[is.na(data.v24$talk1_2)] <- 1
data.v24$talk1_3[is.na(data.v24$talk1_3)] <- 1
data.v24$talk1_4[is.na(data.v24$talk1_4)] <- 1
data.v24$talk1_5[is.na(data.v24$talk1_5)] <- 1
data.v24$talk1_6[is.na(data.v24$talk1_6)] <- 1
data.v24$talk1_7[is.na(data.v24$talk1_7)] <- 1
data.v24$talk1_8[is.na(data.v24$talk1_8)] <- 1
...
data.v25$talk <- as.logical(data.v25$talk)

table(data.v25$talk, useNA = "always") # check output

data.v25 = subset(data.v25, select= -c(talk1_1:talk1_9, talk1sig, talk1fr, 
...
```{r train stig_pcv_2}
# original likert
table(data.v25$stig_pcv_2)
table(data.v25$stig_pcv_3)
table(data.v25$stig_per_2)
table(data.v25$stig_per_3)
...
data.v26 <- data.v26 %>%
  mutate(ther = case_when(ther_cur == 1 ~ 1,
                          ...
                          ther_ever == 4 ~ 1,
                              ...
                              ther_ever == 0 ~ 0)) %>%
    filter(!is.na(ther))
data.v26$ther <- as.logical(data.v26$ther)

table(data.v26$ther, useNA = "always")
```
...
# training data
var_desc_full <- describe(data_sem)
view(var_desc_full)

# xlsx output
save(var_desc_full, file = "var_desc.RData")
load("var_desc.RData")
write_xlsx(var_desc_full, path = "var_desc_full.xls", col_names = TRUE, format_headers = TRUE)
```
...
```{r amended}
# Prep SEM data vars
data_sem_all <- data.v28 %>% select(
  # outcome var
  si_type, si_activ,
  # premotivation
  age, sexual, race, gender_tgd, sib_freq, ed_any, dx_mh, meds_any, finpast,
  religios, satisfied_overall, residenc, school2_type, international, undercls, dep_none, dep_mild, dep_mod, dep_modsev, dep_sev,
  anx_mod, anx_sev, anx_any,
...
# create sem matrix 
data_sem_all <- data.matrix(data_sem_all)
data_sem_all <- as.data.frame(data_sem_all)
view(data_sem_all)
  
## Dummy col SEM w/ continuous
# results_all_com1 <- dummy_cols(data_sem_all, select_columns = c("s),
#                          remove_first_dummy = TRUE, remove_selected_columns = TRUE)

# xlsx output
save(data_sem_all, file = "data_all_pls.RData")
load("data_all_pls.RData")
write_xlsx(data_sem_all, path = "data_all_pls.xlsx", col_names = TRUE, format_headers = TRUE)
```
...
# demographics
table(data.v26$age)
table(data.v26$si_type)
table(data.v26$gender_tgd)
table(data.v26$sexual)
table(data.v26$race)
table(data.v26$dx_mh)

# training data
var_desc_test <- describe(data_sem)
view(var_desc_test)
