...
data.v5 <- data.v4 %>%
  # aggregates all 'sexual' answers into one var
  mutate(sexual = ifelse(sexual == 1, 0, 1)) %>%
  filter(!is.na(sexual))
...
data.v13$satisfied_overall <- as.numeric(data.v13$satisfied_overall)

table(data.v13$satisfied_overall, useNA = "always")
jarque.bera.test(data.v13$satisfied_overall)
```
## Residence type
```{r train residenc}
...
clm_env <- data.v15 %>% select(si_type, religios, residenc, activ_no, school2_type, satisfied_overall)
...
data_def <- data.v21 %>% select(aca_impa, anx_score, deprawsc)
...
## EFA defeat
```{r train EFA_def}
data_def <- data.v21 %>% select(aca_impa, deprawsc, anx_score)
...
results_def <- dummy_cols(data_def, select_columns = c("aca_impa", "deprawsc", "anx_score"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
results2_def <- data.v21 %>% select(si_type, aca_impa, deprawsc, anx_score)
...
data_tsm <- data.v23 %>% select(persist, aaq_tot)
...
results_tsm <- dummy_cols(data_tsm, select_columns = c("persist", "aaq_tot"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
```{r train CLM TSM}
# prepare dataframe for regression
clm_tsm <- data.v23 %>% select(si_type, persist, aaq_tot)
...
data_mm <- data.v27 %>% select(inf, flourish, belong1, mh_stigma)
...
results_mm <- dummy_cols(data_mm, select_columns = c("flourish", "belong1", "inf", 
                                                        "mh_stigma"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
clm_mm <- data.v27 %>% select(si_type, inf, flourish, belong1, mh_stigma)
...
write_xlsx(results_tr_df1, path = "data_sem1.xls", col_names = TRUE, format_headers = TRUE)
```
...
# demographics
table(data.v28$age)
table(data.v28$si_type)
table(data.v28$gender_tgd)
table(data.v28$sexual)
table(data.v28$race)
table(data.v28$dx_mh)
nrow(data.v28)
