...
traindf_prem <- traindf.v15 %>% select(si_type, race, undercls, sexual, gender_tgd, sib_freq, ed_any, 
                                       dx_mh, international, meds_ther, religios, school2_type, 
...
### Moderators correlation
```{r train defeat cor}
# Define defeat variable candidates for correlation matrix
traindf_def <- traindf.v26 %>% select(mh_stigma, flourish, talk, brs_tot, inf, belong1, persist)
...
results_def <- dummy_cols(traindf_def, select_columns = c("mh_stigma", "flourish", "talk", 
                                                          "brs_tot", "inf", "belong1", "persist"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
```{r train CLM defeat}
# prepare regression analysis
results2_def <- traindf.v26 %>% select(si_type, mh_stigma, flourish, talk, brs_tot, inf, belong1, persist)
clm_def <- na.omit(results2_def)

# run defeat regression
clm_def <- glm(si_type ~ mh_stigma + flourish + talk + brs_tot + inf + belong1 + persist, 
               data = clm_def)
