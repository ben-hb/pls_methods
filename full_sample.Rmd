...
data_env <- data.v15 %>% select(discrim, religios, residenc, activ_no, divers, 
...
data_env <- NA_preproc(data_env)
data_env <- as.matrix(data_env)
results_env <- dummy_cols(data_env, select_columns = c("religios", "residenc", "activ_no",
                                                         "school2_type", "international"),
                          remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
data_le <- data.v19 %>% select(ins_no, gpa_sr, fincur, binge,
...
## EFA LE
```{r train le_pca}
data_le <- data.v19 %>% select(ins_no, gpa_sr, fincur, assault,
...
results_le <- dummy_cols(data_le, select_columns = c("ins_no", "gpa_sr", "fincur",
...
# Define diathesis vars for correlation matrix
data_prem <- data.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       meds_ther, religios, activ_no, school2_type, fincur, ins_no, residenc,
                                       gpa_sr, assault, drugs_yn, drug_mar, psyhx)

# Corr_simple function
corr_simple <- function(data=data_prem, sig=0.6){
  # convert data to numeric in order to run correlations 
  data_prem <- data_prem %>% mutate_if(is.character, as.factor)
  data_prem <-  data_prem %>% mutate_if(is.factor, as.numeric)
  # run a correlation and drop the insignificant ones
  corr <- cor(data_prem, use = "pairwise.complete.obs")                               
  corr[lower.tri(corr,diag=TRUE)] <- NA                   # prepare to drop duplicates and correlations of 1    
  corr[corr %in% c(-1, 1)] <- NA                          # drop perfect correlations
  corr <- as.data.frame(as.table(corr))                   # turn into a 3-column table
  corr <- na.omit(corr)                                   # remove the NA values from above 
...
```{r train prem_pca}
# Define diathesis vars for correlation matrix
data_prem <- data.v19 %>% select(age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       meds_ther, religios, activ_no, school2_type, ins_no, residenc,
                                       gpa_sr, assault, drugs_yn, drug_mar, psyhx, fincur)
data_prem <- na.omit(data_prem)
...
data_prem <- data_prem %>% 
  ...
  dummy_cols(remove_first_dummy = TRUE) %>% 
  data.matrix()
data_prem   # check output

results_prem <- dummy_cols(data_prem, select_columns = c("age", "sexual", "gender_tgd", "sib_freq",
...
data_def <- data.v21 %>% select(aca_impa, anx_type, dep_type)
...
data_entrap <- data.v25 %>% select(dep_impa, gad7_impa, percneed)
...
data_mm <- data.v27 %>% select(inf, flourish_dum, belong1, mh_stigma, satisfied_overall)
...
results_mm <- dummy_cols(data_mm, select_columns = c("flourish_dum", "belong1", "inf", 
...
# save matrix
write_xlsx(school2, path = "sch2_missing_full.xlsx", col_names = TRUE, format_headers = TRUE)
```
...
write_xlsx(missing_dfs, path = "missing_full.xlsx", col_names = TRUE, format_headers = TRUE)
```
...
view(data_sem)   # check output

# # xlsx output
data_sem_out <- as.data.frame(data_sem)
save(data_sem_out , file = "data_nondummy.RData")
load("data_nondummy.RData")
write_xlsx(data_sem_out, path = "data_nondummy_full.xls", col_names = TRUE, format_headers = TRUE)
```

## Create dummy vars
```{r train dummy_vars}
results_tr_sem1 <- dummy_cols(data_sem, select_columns = c(
    # outcome var
    "si_type",
    # diathesis
...
save(results_tr_df1, file = "data_sem1.RData")
load("data_sem1.RData")
write_xlsx(results_tr_df1, path = "data_sem1_full.xls", col_names = TRUE, format_headers = TRUE)
```
...
# training data
var_desc <- describe(data_sem)
view(var_desc)

# xlsx output
save(var_desc, file = "var_desc.RData")
load("var_desc.RData")
write_xlsx(var_desc, path = "var_desc_full.xls", col_names = TRUE, format_headers = TRUE)
```

## Model w/ levels correlations
```{r train model_lev_corr}
results_corr <- NA_preproc(data_sem)
sig <- 0.4
...
data_sem1 <- data_sem1 %>% 
  # mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
data_sem1 <- na.omit(data_sem1)
head(data_sem1)   # check output


data_sem2 <- as.data.frame(data_sem1)

# xlsx output
save(data_sem2, file = "data_sem2.RData")
load("data_sem2.RData")
write_xlsx(data_sem2, path = "data_sem2_full.xlsx", col_names = TRUE, format_headers = TRUE)
```

# SEM model with partial cont and dummy
```{r si_model_tr1}
## Dummy col SEM w/ continuous
results_tr_com1 <- dummy_cols(data_sem1, select_columns = c("si_type", "sexual"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
