...
traindf_prem <- traindf.v15 %>% select(si_type, age, race, undercls, sexual, gender, gender_tgd, sib_freq, ed_any, cli_TQ, cli_women, cli_LGB, social_sexid, days_work_paid, children_dep, body_sr, cli_nonwhite, cli_black, cli_his, cli_asian, dei_campus, ed_scoff, audit_tot, meds_count, cli_relig, dx_mh, international, meds_any, religios, school2_type, finpast, ins_no, residenc, hours_work_paid, children_dep, satisfied_overall)
...
```{r train prem_pca}
# Define diathesis vars for correlation matrix
traindf_prem <- traindf.v15 %>% select(age, race, sexual, gender, gender_tgd, sib_freq, ed_any, days_work_paid, children_dep, body_sr, dei_campus, ed_scoff, audit_tot, meds_count, dx_mh, international, meds_any, religios, school2_type, finpast, ins_no, residenc, children_dep, satisfied_overall)
...
results_prem <- dummy_cols(traindf_prem, select_columns = c("age", "race", "sexual", "gender", "gender_tgd", "sib_freq", "ed_any", "days_work_paid", "children_dep", "body_sr", "dei_campus", "ed_scoff", "audit_tot", "meds_count", "cli_relig", "dx_mh", "international", "meds_any", "religios", "school2_type", "finpast", "ins_no", "residenc", "children_dep", "satisfied_overall"),
...
glm_prem <- traindf.v15 %>% select(si_type, sexual, gender_tgd, sib_freq, race, ed_any, finpast,
                                   ...
                                   meds_any, undercls, days_work_paid, children_dep, satisfied_overall)
glm_prem <- na.omit(glm_prem)

# run regression
glm_prem <- glm(si_type ~ sexual + gender_tgd + sib_freq + race + ed_any + dx_mh + undercls + finpast + satisfied_overall +
                religios +  school2_type + international + residenc + meds_any + ins_no + days_work_paid + children_dep,
              data = glm_prem)
summary(glm_prem)
...
traindf_le <- traindf.v19 %>% select(gpa_sr, fincur, binge, assault, drugs_yn, drug_mar, psyhx, failed)
...
### PCA LE
```{r train le_pca}
traindf_le <- traindf.v19 %>% select(gpa_sr, fincur, assault, binge, drugs_yn, drug_mar, psyhx, cli_lowses, fin_comp1, isi_tot)
...
results_le <- dummy_cols(traindf_le, select_columns = c("gpa_sr", "fincur", "binge", "drugs_yn", "drug_mar", 
                                                        "assault", "psyhx", "cli_lowses", "fin_comp1", "isi_tot"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
glm_le <- traindf.v19 %>% select(si_type, gpa_sr, fincur, assault, binge, drugs_yn, drug_mar, psyhx, cli_lowses, fin_comp1, isi_tot)
glm_le <- na.omit(glm_le)

# run regression
glm_le <- glm(si_type ~ gpa_sr + fincur + assault + binge + drugs_yn + drug_mar + psyhx + cli_lowses + fin_comp1 + isi_tot,
              data = glm_le)
summary(glm_le)
coef(glm_le)
...
results_def <- dummy_cols(traindf_def, select_columns = c("aca_impa", "anx_score", "deprawsc", "percneed", "gad7_impa", "dep_impa", "dep_type", "anx_type", "aca_mh_impa", "aca_impa"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
results2_def <- traindf.v21 %>% select(si_type, aca_impa, anx_score, deprawsc, percneed, gad7_impa, dep_impa, dep_type, anx_type, aca_mh_impa, aca_impa)
glm_def <- na.omit(results2_def)

# run defeat regression
glm_def <- glm(si_type ~ aca_impa + anx_score + deprawsc + percneed + gad7_impa + dep_impa + dep_type + anx_type + aca_mh_impa + aca_impa, 
               data = glm_def)
...
testdf.v13 = subset(testdf.v13, select= -c(ins_1, ins_mh, ins_ade, ins_inade)) # clean up var
```
...
# trying to figure out how to take each school's average score on testdf.v14$inter_hy_sui and assign it to a student.... maybe testdf.v14$witness_4???? testdf.v14$cli_look??
...
```{r test prem_pca}
# Define diathesis vars for correlation matrix
testdf_prem <- testdf.v15 %>% select(sexual, age, gender_tgd, sib_freq, race, ed_any, dx_mh, 
...
results_prem <- dummy_cols(testdf_prem, select_columns = c("sexual", "age", "gender_tgd", "sib_freq", "finpast",
...
## Current SES comparison interaction effects
```{r rain fincur_interxn}
table(testdf.v16$fin_comp1)
table(testdf.v16$cli_lowses)
```
...
# Define life event var candidates for correlation matrix
testdf_le <- testdf.v19 %>% select(gpa_sr, fincur, binge, assault, drugs_yn, drug_mar, psyhx)
...
### PCA LE
```{r test le_pca}
testdf_le <- testdf.v19 %>% select(gpa_sr, fincur, assault, binge, drugs_yn, drug_mar, psyhx)
...
results_le <- dummy_cols(testdf_le, select_columns = c("gpa_sr", "fincur", "binge", "drugs_yn", "drug_mar", 
                                                        "assault", "psyhx"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
```{r test glm_LE}
# prepare dataframe for regression
glm_le <- testdf.v19 %>% select(si_type, gpa_sr, assault, binge, drugs_yn, 
...
```{r test MMTSM_corr}
# Define MM variable candidates for correlation matrix
testdf_mm <- testdf.v27 %>% select(inf, flourish, belong1, mh_stigma, talk, brs_tot, 
                                     satisfied_overall, ther, persist, aaq_tot)
...
### PCA MMTSM
```{r test PCA MMTSM}
testdf_mmtsm <- testdf.v27 %>% select(inf, flourish, belong1, talk, mh_stigma, persist, 
                                        satisfied_overall, aaq_tot, ther)
...
results_mmtsm <- dummy_cols(testdf_mmtsm, select_columns = c("flourish", "belong1", "inf", "ther",
                                                        "mh_stigma", "talk", "persist", "aaq_tot",
                                                        "satisfied_overall"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
mmtsm_pca <- PCA(results_mmtsm, graph=FALSE)  # apply PCA
...
```{r test GLM mmtsm}
# prepare data for regression
glm_mmtsm <- testdf.v27 %>% select(si_type, inf, flourish, belong1, talk, mh_stigma, persist, 
                                    satisfied_overall, ther, aaq_tot)
glm_mmtsm <- na.omit(glm_mmtsm)
...
testdf_sem1 <- testdf.v26 %>% select(
  ...
  si_type, si_activ,
  # premotivation
  age, race, undercls, sexual, gender, gender_tgd, sib_freq, ed_any, cli_TQ, cli_women, cli_LGB, social_sexid, days_work_paid, cli_relig,
  cli_nonwhite, cli_black, cli_his, cli_asian, dei_campus, ed_scoff, audit_tot, meds_count, cli_relig, dx_mh, international, meds_any, religios, 
  school2_type, finpast, residenc, children_dep,
  # life events
  gpa_sr, psyhx, fincur, drugs_yn, drug_mar, assault, binge, ins_no,  body_sr, 
  # defeat-entrapment
  deprawsc, anx_score, dep_impa, percneed,
  # threats to internal/external moderation
  persist, aaq_tot, flourish, talk, ther, satisfied_overall)  
