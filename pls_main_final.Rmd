...
table(traindf.v21$dep_secret, useNA = "always")  # check output, too many NAs
```
...
traindf_mm <- traindf.v27 %>% select(persist, belong1, belong2, belong8, flourish, inf, brs_tot, talk, mh_stigma, ther, aaq_tot, dep_secret)
...
### PCA MMTSM
```{r train PCA MMTSM}
traindf_mmtsm <- traindf.v27 %>% select(persist, belong1, belong2, belong8, flourish, inf, 
                                        brs_tot, talk, mh_stigma, ther, aaq_tot, dep_secret)
...
results_mmtsm <- dummy_cols(traindf_mmtsm, select_columns = c("persist", "belong1", "belong2", "belong8", 
                                                              "flourish", "inf", "brs_tot", "talk", "mh_stigma", 
                                                              "ther", "aaq_tot", "dep_secret"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
glm_mmtsm <- traindf.v27 %>% select(si_type, persist, belong1, belong2, belong8, flourish, inf, brs_tot, talk, mh_stigma, ther, aaq_tot, dep_secret)
glm_mmtsm <- na.omit(glm_mmtsm)

# run MM regression
glm_mmtsm <- glm(si_type ~ persist + belong1 + belong2 + belong8 + flourish + inf + brs_tot + talk + mh_stigma + ther + aaq_tot + dep_secret, 
              data = glm_mmtsm)
...
glm_all <- traindf.v27 %>% select(
  ...
  si_type,
  # premotivation
  age, race, sexual, gender, gender_tgd, sib_freq, school2_type, env_mh,
  international, ins_no, satisfied_overall, residenc, religios, meds_count, dx_mh, 
  meds_any, ed_any, finpast, body_sr, days_work_paid, children_dep, discrim,
  # life events
  gpa_sr, fincur, binge, assault, drugs_yn, drug_mar, psyhx, failed,   # missing isi_tot
  # defeat-entrapment
  aca_impa, anx_score, deprawsc, percneed, gad7_impa, dep_impa, dep_type, anx_type, aca_mh_impa, aca_impa,
  # threats to internal/external moderation
  persist, belong1, belong2, belong8, flourish, inf, brs_tot, talk, mh_stigma, ther, aaq_tot, dep_secret)
