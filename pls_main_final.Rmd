...
traindf_le <- traindf.v19 %>% select(gpa_sr, fincur, binge, assault, drugs_yn, drug_mar, psyhx, failed)   # still missing isi_tot
...
### PCA LE
```{r train le_pca}
traindf_le <- traindf.v19 %>% select(gpa_sr, fincur, binge, assault, drugs_yn, drug_mar, psyhx, failed, discrim)   # missing isi_tot
...
results_le <- dummy_cols(traindf_le, select_columns = c("gpa_sr", "fincur", "binge", "assault", "drugs_yn", "drug_mar", "psyhx", "failed", "discrim"),      # missing isi_tot
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
glm_le <- traindf.v19 %>% select(si_type, gpa_sr, fincur, binge, assault, drugs_yn, drug_mar, psyhx, failed, discrim)  # missing isi_tot
glm_le <- na.omit(glm_le)

# run regression
glm_le <- glm(si_type ~ gpa_sr + fincur + binge + assault + drugs_yn + drug_mar + psyhx + failed + discrim,
              data = glm_le)
summary(glm_le)
coef(glm_le)
