...
data.v1 <- data.raw %>%
  ...
  select(-c(                         # narrowing columns numbers to reduce output matrices
    ...
    friend_re:friend_religid, re_find:time_activ,
    #  SAT-related
...
# interaction effect
table(data.v5$cli_LGB)
table(data.v5$social_sexid)
...
data.v5 <- data.v5 %>%
  mutate(
    hours_work_paid = case_when(hours_work_paid == 0 ~ 0,        # no paid work
...
table(data.v5$hours_work_paid, useNA = "always") # tabulate and check output
```

## Child dependents
```{r children}
data.v5 <- data.v5 %>%
  mutate(children_dep = ifelse(children_dep > 1, 1, 0))

table(data.v5$children_dep)
```
...
table(data.v6$social_re)
table(data.v6$cli_nonwhite)
table(data.v6$clo_black)
table(data.v6$cli_ainaan)
table(data.v6$cli_asian)
table(data.v6$cli_sai)
table(data.v6$cli_his)
table(data.v6$cli_mides)
table(data.v6$cli_pi)
table(data.v6$cli_white)
```
...
data.v6 <- data.v6 %>%   # fix directionality
  mutate(cli_LGB  = case_when(cli_LGB  == 1 ~ 5,   # hostile
                              cli_LGB  == 2 ~ 4,
                              cli_LGB  == 3 ~ 3,
                              cli_LGB  == 4 ~ 2,
                              cli_LGB  == 5 ~ 1))  # welcoming

data.v6 <- data.v6 %>%   # fix directionality
  mutate(cli_TQ   = case_when(cli_TQ   == 1 ~ 5,   # hostile
                              cli_TQ   == 2 ~ 4,
                              cli_TQ   == 3 ~ 3,
                              cli_TQ   == 4 ~ 2,
                              cli_TQ   == 5 ~ 1))  # welcoming

data.v6 <- data.v6 %>%   # fix directionality
  mutate(cli_immigrant  = case_when(cli_immigrant  == 1 ~ 5,   # hostile
                              cli_immigrant  == 2 ~ 4,
                              cli_immigrant  == 3 ~ 3,
                              cli_immigrant  == 4 ~ 2,
                              cli_immigrant  == 5 ~ 1))  # welcoming

  
  filter(!is.na(dei_campus))  # filter for NAs
#data.v7$sib_freq <- as.logical(data.v7$sib_freq)  # move frequency to numeric class

table(data.v6$sib_freq, useNA = "always")  # use for NA changes
data.v6 = subset(data.v6, select= -c(sib_none:sib_oth)) # clean up vars/columns
```
...
table(traindf.v12$discrim, useNA = "always") # check output (lots of NAs)
```
...
# interaction effects of religion
table(traindf.v13$social_religid)
data.v6 <- data.v6 %>%   # fix directionality
...
### school culture for mental health

```{r train school2}
# insert each school as a dichotomous var --- how can we give each school a set MH upstander/bystander score???

traindf.v14 <- traindf.v14 %>%
  mutate(school_MH_culture = case_when(school2 == "AlbertaCAD" ~ , 1, 0)) %
  mutate(var_Boston = ifelse((school2 == "Boston"), 1, 0)) %>%
  mutate(var_CIA = ifelse((school2 == "CIA"), 1, 0)) %>%
  mutate(var_CalPoly = ifelse((school2 == "CalPoly"), 1, 0)) %>%
  mutate(var_ColoradoCollege = ifelse((school2 == "ColoradoCollege"), 1, 0)) %>%
...
## Welcome v. unwelcome -- where did this variable go????
```{r test exp_community}
traindf.v14 <- traindf.v14 %>%
  mutate(community = case_when(exp_community == 1 ~ 1,
                               exp_community == 2 ~ 1,
                               exp_community == 3 ~ 2,
                               exp_community == 4 ~ 3,
                               exp_community == 4 ~ 3,
...
traindf.v14$community <- as.numeric(traindf.v14$community)
table(traindf.v14$community, useNA = "always")  # check output (excessive NAs)
```

## Community -- where did this variable go????
```{r test exp_community}
traindf.v14$exp_community <- as.numeric(traindf.v14$exp_community)
table(traindf.v14$exp_community, useNA = "always")  # check output (excessive NAs)
```
...
traindf_prem <- traindf.v15 %>% select(si_type, race, undercls, sexual, gender_tgd, sib_freq, ed_any, 
                                       ...
                                       ins_no, residenc, hours_work_paid, children_dep)
...
```{r train prem_pca}
# Define diathesis vars for correlation matrix
traindf_prem <- traindf.v15 %>% select(sexual, age, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       international, finpast, meds_any, religios, school2_type, 
                                       residenc, undercls, ins_no, hours_work_paid, children_dep)
...
results_prem <- dummy_cols(traindf_prem, select_columns = c("sexual", "age", "gender_tgd", "sib_freq", "finpast",
                                                            ...
                                                            "religios", "international", "ins_no",
                                                            "school2_type", "residenc", "undercls", "hours_work_paid", "children_dep"),
...
glm_prem <- traindf.v15 %>% select(si_type, sexual, gender_tgd, sib_freq, race, ed_any, finpast,
                                   ...
                                   meds_any, undercls, hours_work_paid, children_dep)
glm_prem <- na.omit(glm_prem)

# run regression
glm_prem <- glm(si_type ~ sexual + gender_tgd + sib_freq + race + ed_any + dx_mh + undercls + finpast +
                religios +  school2_type + international + residenc + meds_any + ins_no + hours_work_paid + children_dep,
              data = glm_prem)
summary(glm_prem)
...
## Current SES comparison
```{r train fincur}
table(traindf.v16$afford_school)
table(traindf.v16$fin_comp1)
table(traindf.v16$fin_comp2)

##Interaction effect
table(traindf.v16$cli_lowses)
```
...
traindf_le <- traindf.v19 %>% select(gpa_sr, fincur, binge, assault, drugs_yn, drug_mar, psyhx)
...
traindf.v22$satisfied_overall <- as.numeric(traindf.v22$satisfied_overall)

table(traindf.v22$satisfied_overall, useNA = "always")
jarque.bera.test(traindf.v22$satisfied_overall)
```
...
traindf.v24 <- traindf.v24 %>% 
  ...
  mutate(agl = case_when((talk1_9 == 1 | talk1no == 1) ~ 1,                                    # no support
...
traindf_mm <- traindf.v27 %>% select(inf, flourish, belong1, mh_stigma, talk, brs_tot, 
                                     satisfied_overall, ther, persist, aaq_tot)
...
### PCA MMTSM
```{r train PCA MMTSM}
traindf_mmtsm <- traindf.v27 %>% select(inf, flourish, belong1, talk, mh_stigma, persist, 
                                        satisfied_overall, aaq_tot, ther)
...
glm_mmtsm <- traindf.v27 %>% select(si_type, inf, flourish, belong1, talk, mh_stigma, persist, 
                                    satisfied_overall, ther, aaq_tot)
glm_mmtsm <- na.omit(glm_mmtsm)

# run MM regression
glm_mmtsm <- glm(si_type ~ inf + flourish + belong1 + mh_stigma + talk + persist + 
                   satisfied_overall + aaq_tot + ther, 
              data = glm_mmtsm)
...
glm_all <- traindf.v27 %>% select(
  ...
  religios, residenc, school2_type, international, undercls,
  # life events
  ...
  inf, flourish, belong1, talk, mh_stigma, persist, satisfied_overall, aaq_tot, ther)
...
var_names <- c(
  ...
  "deprawsc", "anx_score", "dep_impa", "percneed",
  # threats to internal/external moderation
  "persist", "aaq_tot", "flourish", "satisfied_overall", "talk", "ther")  
...
jarque.bera.test(testdf.v14$ins_no)
...
## Community
```{r test exp_community}
testdf.v14$exp_community <- as.numeric(testdf.v14$exp_community)
table(testdf.v14$exp_community, useNA = "always")  # check output (excessive NAs)
```
...
jarque.bera.test(testdf.v22$satisfied_overall)
```
...
# training v. testing
wilcox.test(testdf.v26B$age,traindf.v28B$age, alternative = "two.sided") # W = 15974521, p-value = 0.5965
wilcox.test(testdf.v26B$si_type,traindf.v28B$si_type, alternative = "two.sided") # W = 16274859, p-value = 0.05296
wilcox.test(testdf.v26B$gender_tgd,traindf.v28B$gender_tgd, alternative = "two.sided") # W = 16038848, p-value = 0.6233
wilcox.test(testdf.v26B$sexual, traindf.v28B$sexual, alternative = "two.sided") # W = 16083640, p-value = 0.891
wilcox.test(testdf.v26B$race, traindf.v28B$race, alternative = "two.sided") # W = 16010644, p-value = 0.7108
wilcox.test(testdf.v26B$dx_mh, traindf.v28B$dx_mh, alternative = "two.sided") # W = 16325040, p-value = 0.07795
...
wilcox.test(test20.v26B$race, test18.v26B$race, alternative = "two.sided") #  = 3492851, p-value = 0.5591
