...
# possible interaction with thin_good?
table(data.v5$thin_good)
```

## interaction effect for subjective weight -- something isnt working here
``` {r wcs}
# set all NAs in activities equal to 0
data.v5$wcs_1_f[is.na(data.v5$wcs_1_f)] <- 0
data.v5$wcs_1_m[is.na(data.v5$wcs_1_m)] <- 0
data.v5$wcs_1_m[is.na(data.v5$wcs_1_o)] <- 0

data.v5$wcs_1_f <- as.numeric(data.v5$wcs_1_f)
data.v5$wcs_1_f <- as.numeric(data.v5$wcs_1_m)
data.v5$wcs_1_f <- as.numeric(data.v5$wcs_1_o)

## WCS --Combine wcs_1_f, wcs_1_m, wcs_1_o to create 'wcs_1'
data.v5$wcs_1 = data.v5$wcs_1_f + data.v5$wcs_1_m + data.v5$wcs_1_o
table(data.v5$wcs_1)

# total WCS score 
data.v5 <- data.v5 %>%
  mutate(wcs_tot =sum(wcs_1 + wcs_2 + wcs_3 + $wcs_4 + wcs_5))   # not sure what to do with the NAs here

table(data.v5$wsc, useNA = "always")
...
# cant get this to work... trying to do a row-wise average school for subjective DEI environment....
data.v6 <- data.v6 %>%
  mutate(dei_campus = mean(hostile_friendly, uncoop_coop, notwelc_welc, disresp_resp))
...
### Suicide culture in school - interaction effect
```{r train school_sui}

# trying to figure out how to take each school's average score on traindf.v14$inter_hy_sui and assign it to a student.... maybe traindf.v14$witness_4???? traindf.v14$cli_look??
