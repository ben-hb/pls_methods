...
jarque.bera.test(traindf.v11$meds_count)
```
...
results_dia <- dummy_cols(traindf_dia, select_columns = c("age", "sexual", "gender_tgd", "finpast", 
                                                       "sib_freq", "race", "dx_mh", "ed_any", "meds_count"), 
...
jarque.bera.test(traindf.v13$religios)
```
...
table(traindf.v13$residenc, useNA = "always")
jarque.bera.test(traindf.v13$residenc)
```
...
```{r military}
traindf.v14 <- traindf.v14 %>%
  mutate(military = case_when(military == 1 ~ 1,
                              military == 0 ~ 0)) %>%
...
jarque.bera.test(traindf.v14$activ_yn)
...
results_env <- dummy_cols(traindf_env, select_columns = c("religios", "residenc", "activ_yn",
                                                         "school2_type", "talk", "military", "international"),
                          remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
clm_env2 <- clm(si_type ~ religios + residenc + activ_yn + school2_type + international, 
                data = clm_env)
summary(clm_env2)
coef(clm_env2)
...
jarque.bera.test(traindf.v16$ins_cover)
...
traindf_prem <- traindf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       meds_count, residenc, religios, activ_yn, school2_type, ins_cover, military,
                                       gpa_sr, fincur, assault, binge, drugs_yn, drug_mar, psyhx, international)
...
traindf.v20$anx_score <- as.numeric(traindf.v20$anx_score)
jarque.bera.test(traindf.v20$anx_score)
...
jarque.bera.test(traindf.v24$gad7_impa)
```
...
traindf_mm <- traindf.v27 %>% select(inf, flourish_dum, belong1, satisfied_overall)
...
results_mm <- dummy_cols(traindf_mm, select_columns = c("flourish_dum", "belong1", "inf", "satisfied_overall"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
```{r CLM mm}
clm_mm <- traindf.v27 %>% select(si_type, inf, flourish_dum, belong1, satisfied_overall)
clm_mm <- na.omit(clm_mm)

# run MM regression
clm_mm <- clm(si_type ~ inf + flourish_dum + belong1 + satisfied_overall, data = clm_mm)
...
var_names <- c(
  # diathesis
  "age", "sexual", "gender_tgd", "sib_freq", "ed_any", "dx_mh", "meds_count",
  # environment
  "religios", "school2_type", "activ_yn", "international", "military", "residenc",
  # life events
  ...
  "dep_impa", "gad7_impa",
  # motivational moderators
  "inf", "flourish_dum", "belong1", "satisfied_overall")  
...
traindf_sem1 <- traindf.v28 %>% select(
  ...
  si_type,
  # diathesis
  sexual, gender_tgd, sib_freq, ed_any, dx_mh, meds_count,
  # environment
  religios, school2_type, activ_yn, international,
  # life events
  ins_cover, gpa_sr, binge, psyhx, fincur, drugs_yn, assault, drug_mar,
  # defeat
...
results_tr_com1 <- dummy_cols(traindf_sem1, select_columns = c("si_type", "sexual", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
SI_model_tr1 <- '
                # measurement model
                  ...
                  prem  =~ sexual_2 + sexual_3 + gender_tgd + sib_freq + 
                           ed_any + dx_mh + meds_count + religios + international +
                           school2_type_2 + school2_type_3 + activ_yn + ins_cover + 
...
nullRMSEA(fit_train1, scaled = FALSE, silent = FALSE)
...
summary_train <- summary(fit_trainM, fit.measures=TRUE, standardized = TRUE)
...
# mod_fit_tr <- cfa(SI_model_tr1, data = results_tr_com1)
# mi <- modindices(mod_fit_tr, sort = TRUE, maximum.number = 10)
# View(mi)
# mi[mi$op == "=~",] %>% View()
...
```{r mediation}
# TSM as a mediator?
fit.totaleffect=lm(entrap~def, results_tr_com1)
summary(fit.totaleffect)
fit.mediator <- lm(tsm~def, results_tr_com1)
summary(fit.mediator)
fit.dv <- lm(entrap~def+tsm, results_tr_com1)
summary(fit.dv)
...
fit.totaleffect2 <- lm(si~def, results_tr_com1)
summary(fit.totaleffect2)

fit.mediator2 <- lm(entrap~def, results_tr_com1)
summary(fit.mediator2)

fit.dv2 <- lm(si~def+entrap, results_tr_com1)
summary(fit.dv2)
...
fit.totaleffect3 <- lm(si~entrap, results_tr_com1)
summary(fit.totaleffect3)

fit.mediator3 <- lm(mm~entrap, results_tr_com1)
summary(fit.mediator3)

fit.dv3 <- lm(si~entrap+mm, results_tr_com1)
summary(fit.dv3)
...
```{r divide_test}
testdf.v10 <- as.data.frame(testdf.v10)

set.seed(500) 
split = sample.split(testdf.v10, SplitRatio = 0.50)
 
# Create training and testing sets
test1.v10 <- subset(testdf.v10, split == TRUE)
sub.v10 <- subset(testdf.v10, split == FALSE)
...
test1.v11 <- test1.v10 %>%
  ...
  mutate(ed_any = case_when(ed_any == 1 ~ 1,
                            ed_any == 0 ~ 0))
test1.v11$ed_any <- as.logical(test1.v11$ed_any)

table(test1.v11$ed_any, useNA = "always")
jarque.bera.test(test1.v11$ed_any)
```
...
to see what is associated (i.e. the count is meant to find the common meds in suicidality)
```{r med_count}
test1.v11$meds_1 <- as.numeric(test1.v11$meds_1)
test1.v11$meds_2 <- as.numeric(test1.v11$meds_2)
test1.v11$meds_3 <- as.numeric(test1.v11$meds_3)
test1.v11$meds_4 <- as.numeric(test1.v11$meds_4)
test1.v11$meds_5 <- as.numeric(test1.v11$meds_5)
test1.v11$meds_6 <- as.numeric(test1.v11$meds_6)
test1.v11$meds_7 <- as.numeric(test1.v11$meds_7)
test1.v11$meds_8 <- as.numeric(test1.v11$meds_8)

test1.v11$meds_1[is.na(test1.v11$meds_1)] <- 0
test1.v11$meds_2[is.na(test1.v11$meds_2)] <- 0
test1.v11$meds_3[is.na(test1.v11$meds_3)] <- 0
test1.v11$meds_4[is.na(test1.v11$meds_4)] <- 0
test1.v11$meds_5[is.na(test1.v11$meds_5)] <- 0
test1.v11$meds_6[is.na(test1.v11$meds_6)] <- 0
test1.v11$meds_7[is.na(test1.v11$meds_7)] <- 0
...
test1.v11 <- test1.v11 %>%
  ...
  mutate(meds_count = case_when(meds_count == 0 ~ 0,
                                ...
                                meds_count == 5 ~ 3,
                                meds_count == 6 ~ 3))
test1.v11$meds_count <- as.numeric(test1.v11$meds_count)

table(test1.v11$meds_count, useNA = "always")  # check output
test1.v11 = subset(test1.v11, select= -c(meds_1:meds_7)) # clean up vars/columns
hist(test1.v11$meds_count)
jarque.bera.test(test1.v11$meds_count)
```
...
test1.v13 <- test1.v11 %>%
  filter(!is.na(religios))
test1.v13$religios <- as.numeric(test1.v13$religios)

table(test1.v13$religios, useNA = "always")       # Check output
jarque.bera.test(test1.v13$religios)
```
...
table(test1.v13$residenc, useNA = "always")
jarque.bera.test(test1.v13$residenc)
```
...
test1.v14 <- test1.v13 %>%
  mutate(international = case_when(international == 1 ~ 1,
                                   international == 0 ~ 0)) %>%
  filter(!is.na(international))
test1.v14$international <- as.logical(test1.v14$international)

table(test1.v14$international, useNA = "always")
```

## Activity Involvement
```{r Activities}
# set all NAs in actvities equal to 0
test1.v14$activ_ac[is.na(test1.v14$activ_ac)] <- 0
test1.v14$activ_athc[is.na(test1.v14$activ_athc)] <- 0
test1.v14$activ_athv[is.na(test1.v14$activ_athv)] <- 0
test1.v14$activ_athi[is.na(test1.v14$activ_athi)] <- 0
test1.v14$activ_cs[is.na(test1.v14$activ_cs)] <- 0
test1.v14$activ_cu[is.na(test1.v14$activ_cu)] <- 0
test1.v14$activ_da[is.na(test1.v14$activ_da)] <- 0
test1.v14$activ_fs[is.na(test1.v14$activ_fs)] <- 0
test1.v14$activ_gs[is.na(test1.v14$activ_gs)] <- 0
test1.v14$activ_gov[is.na(test1.v14$activ_gov)] <- 0
test1.v14$activ_mp[is.na(test1.v14$activ_md)] <- 0
test1.v14$activ_md[is.na(test1.v14$activ_none)] <- 0
test1.v14$activ_soc[is.na(test1.v14$activ_soc)] <- 0
test1.v14$activ_rel[is.na(test1.v14$activ_rel)] <- 0
test1.v14$activ_art[is.na(test1.v14$activ_art)] <- 0
test1.v14$activ_other[is.na(test1.v14$activ_other)] <- 0
...
test1.v14 <- test1.v14 %>% 
  mutate(activ_yn = case_when(activ_none == 1 ~ 0,
                              ...
                              activ_other == 1 ~ 1)) %>%
  filter(!is.na(activ_yn))
test1.v14$activ_yn <- as.logical(test1.v14$activ_yn)

table(test1.v14$activ_yn, useNA = "always")     # tabulate and check output
jarque.bera.test(test1.v14$activ_yn)
#test1.v14 = subset(test1.v14, select= -c(activ_none, activ_ac:activ_other))    # clean up vars
```
...
# Binary variable for school type
table(test1.v14$school2_4yr)
table(test1.v14$school2_cc)
table(test1.v14$school2_ad)
...
test1.v14 <- test1.v14 %>%
  mutate(school2_type = case_when(school2_4yr == TRUE ~ 1,
                                  ...
                                  school2_ad == TRUE ~ 3)) %>%
  filter(!is.na(school2_type))
test1.v14$school2_type <- as.factor(test1.v14$school2_type)

table(test1.v14$school2_type, useNA = "always") # check output

# # Clean-up school2 var
# test1.v14 = subset(test1.v14, select= -c(var_Boston, var_CalPoly, var_ColoradoCollege, var_ColoradoMountain,
#                                        var_Delaware, var_DetroitMercy, var_Emerson, var_GeorgeMason,
#                                        var_GeorgiaTech, var_Kalamazoo, var_Kansas, var_Merrimack,
#                                        var_MichiganState, var_MontclairState, var_NevadaReno,
#                                        var_PennCollegeTech, var_RhodeIsland, var_Rollins, var_Sewanee, var_Smith,
...
test1.v16 <- test1.v14 %>% 
  mutate(ins_cover = case_when((ins_1 == 1 | ins_mh == 4 | ins_mh == 5 | ins_ade == 3) ~ 0,   # inadequate coverage
                               (ins_1 == 0 | ins_mh == 1 | ins_mh == 2 | ins_ade == 2) ~ 1)) %>% # adequate MH coverage
  filter(!is.na(ins_cover))   
test1.v16$ins_cover <- as.logical(test1.v16$ins_cover)
table(test1.v16$ins_cover, useNA = "always")   # check output
jarque.bera.test(test1.v16$ins_cover)

#test1.v16 = subset(test1.v16, select= -c(ins_1, ins_mh, ins_ade, ins_inade)) # clean up var
```
...
```{r drugs_yn}
## add summary of each
test1.v16$drug_coc[is.na(test1.v16$drug_coc)] <- 0
test1.v16$drug_her[is.na(test1.v16$drug_her)] <- 0
test1.v16$drug_met[is.na(test1.v16$drug_met)] <- 0
test1.v16$drug_stim[is.na(test1.v16$drug_stim)] <- 0
test1.v16$drug_ect[is.na(test1.v16$drug_ect)] <- 0
test1.v16$drug_opi[is.na(test1.v16$drug_opi)] <- 0
test1.v16$drug_other[is.na(test1.v16$drug_other)] <- 0

table(test1.v16$drug_coc)
table(test1.v16$drug_her)  # too small
table(test1.v16$drug_met)  # too small
table(test1.v16$drug_stim)
table(test1.v16$drug_ect)  # too small
table(test1.v16$drug_opi)   # too small
table(test1.v16$drug_other)  # too vague, small
...
test1.v16 <- test1.v16 %>%
  mutate(drugs_yn = case_when(drug_coc == 1 ~ 1,
                              ...
                              drug_opi == 1 ~ 1,
                              drug_other == 1 ~ 1))
test1.v16$drugs_yn[is.na(test1.v16$drugs_yn)] <- 0
test1.v16$drugs_yn <- as.logical(test1.v16$drugs_yn)

table(test1.v16$drugs_yn, useNA = "always")
jarque.bera.test(test1.v16$drugs_yn)
```

```{r test drug_mar}
test1.v16$drug_mar[is.na(test1.v16$drug_mar)] <- 0
table(test1.v16$drug_mar)  
...
test1.v16$drug_mar <- as.logical(test1.v16$drug_mar)

table(test1.v16$drug_mar, useNA = "always")
jarque.bera.test(test1.v16$drug_mar)
```
...
test1.v16 <- test1.v16 %>%
  ...
  mutate(fincur = case_when(fincur == 3 ~ 0,  # sometimes
                            ...
                            fincur == 1 ~ 1,  # always or often stressful
                            fincur == 2 ~ 1))
test1.v16$fincur <- as.logical(test1.v16$fincur)

table(test1.v16$fincur, useNA = "always")
jarque.bera.test(test1.v16$fincur)
```

## Abuse or assault, in the last year only
```{r assault}
table(test1.v16$abuse_life)
table(test1.v16$abuse_recent)
...
test1.v17 <- test1.v16 %>%
  mutate(abuse_year = case_when(abuse_recent == 1 ~ 1,
                                ...
                                abuse_life == 0 ~ 0,
                                is.na(abuse_life) ~ 0))
table(test1.v17$abuse_year)   # check output
...
test1.v17 <- test1.v17 %>%
  ...
  mutate(assault = case_when(assault == 1 ~ 1,
                             assault == 0 ~ 0,
                             is.na(assault) ~ 0))
test1.v17$assault <- as.logical(test1.v17$assault)

# tabulate and check output
table(test1.v17$assault, useNA = "always") 
jarque.bera.test(test1.v17$assault)
                                         
test1.v17 = subset(test1.v17, select= -c(abuse_life, abuse_recent)) # clean up var
```

## Binging frequency
```{r binge_fr}
# set all binge_fr values equal to 0
test1.v17$binge_fr_f[is.na(test1.v17$binge_fr_f)] <- 0
test1.v17$binge_fr_m[is.na(test1.v17$binge_fr_m)] <- 0
test1.v17$binge_fr_o[is.na(test1.v17$binge_fr_o)] <- 0

# create one binge var
test1.v18 <- test1.v17 %>% 
  mutate(binge_fr = as.numeric(binge_fr_f) + as.numeric(binge_fr_m) + as.numeric(binge_fr_o))
table(test1.v18$binge_fr)

test1.v18 <- test1.v18 %>%
  mutate(binge = case_when(binge_fr >= 4 ~ 1,
                           binge_fr < 4 ~ 0)) %>%
  filter(!is.na(binge))
test1.v18$binge <-as.logical(test1.v18$binge)

table(test1.v18$binge, useNA = "always") # check output
jarque.bera.test(test1.v18$binge)

test1.v18 = subset(test1.v18, select= -c(binge_fr_f, binge_fr_m, binge_fr_o)) # clean up var
```
...
test1.v18 <- test1.v18 %>% 
  mutate(psyhx = case_when((prov_pes == 1 | prov_inp == 1 | prov_par == 1) ~ 1, 
                           ...
                           ther_ever == 0 ~ 0)) %>%
  filter(!is.na(psyhx))
test1.v18$psyhx <- as.logical(test1.v18$psyhx)

table(test1.v18$psyhx, useNA = "always")   # check output
jarque.bera.test(test1.v18$psyhx)

test1.v18 = subset(test1.v18, select= -c(prov_pes, prov_inp, prov_par)) # clean up var
```
...
test1.v18 <- test1.v18 %>%
  mutate(gpa_sr = case_when(gpa_sr == 0 ~ 1,   # A+
                            ...
                            gpa_sr == 10 ~ 5,  # Don't know
                            is.na(gpa_sr) ~ 5))
test1.v18$gpa_sr <- as.numeric(test1.v18$gpa_sr)

table(test1.v18$gpa_sr, useNA = "always")
jarque.bera.test(test1.v18$gpa_sr)
```

## PHQ risk/severity
```{r deprawsc}
table(test1.v18$deprawsc)
test1.v18$deprawsc <- as.numeric(test1.v18$deprawsc)
...
test1.v20 <- test1.v18 %>%
  ...
  mutate(dep_na = case_when(is.na(deprawsc) ~ 1,
                            !is.na(deprawsc) ~ 0))
test1.v20$deprawsc <- as.numeric(test1.v20$deprawsc)

# Check outputs
table(test1.v20$dep_none)
table(test1.v20$dep_mild)
table(test1.v20$dep_mod)
table(test1.v20$dep_modsev)
table(test1.v20$dep_sev)
table(test1.v20$dep_na)
...
table(test1.v20$dep_type, useNA = "always")
```

## GAD7 total
```{r anx_score}
test1.v20$anx_score <- as.numeric(test1.v20$anx_score)
...
test1.v20 <- test1.v20 %>%
  ...
  mutate(anx_type = case_when(anx_score > 14 ~ 4,
                              ...
                              anx_score < 15 ~ 3))                    # anx severe
test1.v20$anx_score <- as.numeric(test1.v20$anx_score)
jarque.bera.test(test1.v20$anx_score)

table(test1.v20$anx_type, useNA = "always") # check output
```
...
test1.v20 <- test1.v20 %>%
  ...
  mutate(aca_impa = case_when(aca_impa == 1 ~ 1,
                              ...
                              aca_impa == 3 ~ 3,
                              aca_impa == 4 ~ 4)) 
test1.v20$aca_impa <- as.numeric(test1.v20$aca_impa)

table(test1.v20$aca_impa, useNA = "always")
jarque.bera.test(test1.v20$aca_impa)
```
...
table(test1.v22$aaq_tot)

#  for loop for predictive cutoff point needed
 rs_df <- data.frame("cutoff" = c(), "rs" = c())
 for (i in 8:48) {
   aaq_bin <- ifelse(test1.v22$aaq_tot >= i, TRUE, FALSE)
   model = lm(test1.v22$sui_idea ~ aaq_bin)
   rs_df <- rbind(rs_df, c(i, summary(model)$r.squared))
...
# deem 'rs_df_arranged[1,1]' as SI cutoff
 test1.v22 <- test1.v22 %>%
   ...
   mutate(aaq_dum = case_when(aaq_dum == 1 ~ 1,
                              aaq_dum == 0 ~ 0)) %>%
   filter(!is.na(aaq_dum))
test1.v22$aaq_dum <- as.logical(test1.v22$aaq_dum)

table(test1.v22$aaq_dum, useNA = "always")
```
...
test1.v22 <- test1.v22 %>%
  mutate(persist = case_when(persist == 1 ~ 1,   # confident
                             ...
                             persist == 5 ~ 5, 
                             persist == 6 ~ 6))  # not
test1.v22$persist <- as.numeric(test1.v22$persist)

table(test1.v22$persist, useNA = "always") # check output
jarque.bera.test(test1.v22$persist)
```
...
test1.v24 <- test1.v22 %>%
  mutate(dep_impa = case_when(dep_impa == 1 ~ 1,
                              ...
                              dep_impa == 4 ~ 4)) %>%
  filter(!is.na(dep_impa))
test1.v24$dep_impa <- as.numeric(test1.v24$dep_impa)

table(test1.v24$dep_impa, useNA = "always")
jarque.bera.test(test1.v24$dep_impa)
```
...
test1.v24 <- test1.v24 %>%
  mutate(gad7_impa = case_when(gad7_impa == 1 ~ 1,
                               ...
                               gad7_impa == 4 ~ 4)) %>%
  filter(!is.na(gad7_impa))
test1.v24$gad7_impa <- as.numeric(test1.v24$gad7_impa)

table(test1.v24$gad7_impa, useNA = "always")
jarque.bera.test(test1.v24$gad7_impa)
```
...
test1.v26 <- test1.v24 %>%
  mutate(belong1 = case_when(belong1 == 1 ~ 1,
                             ...
                             belong1 == 6 ~ 6)) %>%
  filter(!is.na(belong1))
table(test1.v26$belong1, useNA = "always")

test1.v26$belong1 <- as.numeric(test1.v26$belong1)
summary(test1.v26$belong1)
jarque.bera.test(test1.v26$belong1)

table(test1.v26$belong2)  # elective question, but common
table(test1.v26$belong8)  # nested x 2
table(test1.v26$belong9)  # not as good of a proxy
```
...
test1.v26 <- test1.v26 %>%
  mutate(satisfied_overall = case_when(satisfied_overall == 1 ~ 1,
                                       ...
                                       satisfied_overall == 6 ~ 6)) %>% 
  filter(!is.na(satisfied_overall))
test1.v26$satisfied_overall <- as.numeric(test1.v26$satisfied_overall)

table(test1.v26$satisfied_overall, useNA = "always")
jarque.bera.test(test1.v26$satisfied_overall)
```

## Flourishing Scale
```{r flourish}
test1.v26$flourish <- as.numeric(test1.v26$flourish)
table(test1.v26$flourish)


#  for loop for predictive cutoff point needed
rs_df <- data.frame("cutoff" = c(), "rs" = c())
for (i in 9:55) {
  flourish_bin <- ifelse(test1.v26$flourish >= i, TRUE, FALSE)
  model = lm(test1.v26$sui_idea ~ flourish_bin)
  rs_df <- rbind(rs_df, c(i, summary(model)$r.squared))
...
test1.v26 <- test1.v26 %>%
  mutate(flourish_dum = case_when(flourish > 50 ~ 5,
                                  ...
                                  flourish < 14 ~ 1)) %>%
  filter(!is.na(flourish_dum))
test1.v26$flourish_dum <- as.numeric(test1.v26$flourish_dum)

table(test1.v26$flourish_dum, useNA = "always")
```

## Informal Mental Health Support
```{r inf}
test1.v26$inf_1[is.na(test1.v26$inf_1)] <- 0
test1.v26$inf_2[is.na(test1.v26$inf_2)] <- 0
test1.v26$inf_3[is.na(test1.v26$inf_3)] <- 0
test1.v26$inf_4[is.na(test1.v26$inf_4)] <- 0
test1.v26$inf_5[is.na(test1.v26$inf_5)] <- 0
test1.v26$inf_6[is.na(test1.v26$inf_6)] <- 0
test1.v26$inf_7[is.na(test1.v26$inf_7)] <- 0
...
test1.v26 <- test1.v26 %>% 
  ...
  mutate(inf = case_when(inf_8 == 1 ~ 1,
                         ...
                         inf_7 == 1 ~ 0)) %>%
  filter(!is.na(inf))
test1.v26$inf <- as.logical(test1.v26$inf)
table(test1.v26$inf, useNA ="always")
jarque.bera.test(test1.v26$inf)

# clean up vars
#test1.v26 = subset(test1.v26, select= -c(inf_1:inf_help))
```
...
table(test1.v26$brs_tot)
test1.v26$brs_tot <- as.numeric(test1.v26$brs_tot)

table(is.na(test1.v26$brs_tot))

#  for loop for predictive cutoff point needed
rs_df <- data.frame("cutoff" = c(), "rs" = c())
for (i in 6:24) {
  brs_bin <- ifelse(test1.v26$brs_tot >= i, TRUE, FALSE)
  model = lm(test1.v26$sui_idea ~ brs_bin)
  rs_df <- rbind(rs_df, c(i, summary(model)$r.squared))
}
...
test1.v26 <- test1.v26 %>%
  ...
  mutate(brs_dum = case_when(brs_dum == 1 ~ 1,
                             brs_dum == 0 ~ 0)) %>%
  filter(!is.na(brs_dum))
test1.v26$brs_dum <- as.logical(test1.v26$brs_dum)

# output
rs_df_arranged[1,1]
table(test1.v26$brs_tot, useNA = "always")
table(test1.v26$brs_dum, useNA = "always")
```
...
# build table for absolute missingness based on school2
school2 <- count(test1.v28, school2)
school2_ncol <- ncol(school2)
...
for (j in 1:(length(var_names))) {
for (i in 1:nrow(school2)) {   # i corresponds to rows, j to columns
  school2[i, 2 + j] <- sum(!is.na(test1.v28[test1.v28$school2 == school2[[i,1]], var_names[j]]))
}
}
# add percentage in each column
school2_m <- as.matrix(school2[-1]) 
...
test1_sem <- test1_sem %>% 
  # mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
# test1_sem <- na.omit(test1_sem)
view(test1_sem)   # check output

# # xlsx output
test1_sem_out <- as.data.frame(test1_sem)
save(test1_sem_out , file = "test1_nondummy.RData")
load("test1_nondummy.RData")
write_xlsx(test1_sem_out, path = "test1_nondummy.xls", col_names = TRUE, format_headers = TRUE)
```
...
# create sem matrix 
test1_sem1 <- test1_sem1 %>% 
  # mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
test1_sem1 <- na.omit(test1_sem1)
head(test1_sem1)   # check output
```

# SEM model with partial cont and dummy
```{r si_model_tr1}
## Dummy col SEM w/ continuous
results_test_com1 <- dummy_cols(test1_sem1, select_columns = c("si_type", "sexual", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
#view(results_tr_com1)
...
fit_testM <- sem(SI_model_testM, data=results_test_com1, ordered=T)
summary_test <- summary(fit_testM, fit.measures=TRUE, standardized = TRUE)
...
sub.v11 <- sub.v11 %>%
  ...
  mutate(ed_any = case_when(ed_any == 1 ~ 1,
                            ed_any == 0 ~ 0))
sub.v11$ed_any <- as.logical(sub.v11$ed_any)

table(sub.v11$ed_any, useNA = "always")
jarque.bera.test(sub.v11$ed_any)
```
...
to see what is associated (i.e. the count is meant to find the common meds in suicidality)
```{r med_count}
sub.v11$meds_1 <- as.numeric(sub.v11$meds_1)
sub.v11$meds_2 <- as.numeric(sub.v11$meds_2)
sub.v11$meds_3 <- as.numeric(sub.v11$meds_3)
sub.v11$meds_4 <- as.numeric(sub.v11$meds_4)
sub.v11$meds_5 <- as.numeric(sub.v11$meds_5)
sub.v11$meds_6 <- as.numeric(sub.v11$meds_6)
sub.v11$meds_7 <- as.numeric(sub.v11$meds_7)
sub.v11$meds_8 <- as.numeric(sub.v11$meds_8)

sub.v11$meds_1[is.na(sub.v11$meds_1)] <- 0
sub.v11$meds_2[is.na(sub.v11$meds_2)] <- 0
sub.v11$meds_3[is.na(sub.v11$meds_3)] <- 0
sub.v11$meds_4[is.na(sub.v11$meds_4)] <- 0
sub.v11$meds_5[is.na(sub.v11$meds_5)] <- 0
sub.v11$meds_6[is.na(sub.v11$meds_6)] <- 0
sub.v11$meds_7[is.na(sub.v11$meds_7)] <- 0
...
sub.v11 <- sub.v11 %>%
  ...
  mutate(meds_count = case_when(meds_count == 0 ~ 0,
                                ...
                                meds_count == 5 ~ 3,
                                meds_count == 6 ~ 3))
sub.v11$meds_count <- as.numeric(sub.v11$meds_count)

table(sub.v11$meds_count, useNA = "always")  # check output
sub.v11 = subset(sub.v11, select= -c(meds_1:meds_7)) # clean up vars/columns
hist(sub.v11$meds_count)
jarque.bera.test(sub.v11$meds_count)
```
...
sub.v13 <- sub.v11 %>%
  filter(!is.na(religios))
sub.v13$religios <- as.numeric(sub.v13$religios)

table(sub.v13$religios, useNA = "always")       # Check output
jarque.bera.test(sub.v13$religios)
```
...
table(sub.v13$residenc, useNA = "always")
jarque.bera.test(sub.v13$residenc)
```
...
sub.v14 <- sub.v13 %>%
  mutate(international = case_when(international == 1 ~ 1,
                                   international == 0 ~ 0)) %>%
  filter(!is.na(international))
sub.v14$international <- as.logical(sub.v14$international)

table(sub.v14$international, useNA = "always")
```

## Activity Involvement
```{r Activities}
# set all NAs in actvities equal to 0
sub.v14$activ_ac[is.na(sub.v14$activ_ac)] <- 0
sub.v14$activ_athc[is.na(sub.v14$activ_athc)] <- 0
sub.v14$activ_athv[is.na(sub.v14$activ_athv)] <- 0
sub.v14$activ_athi[is.na(sub.v14$activ_athi)] <- 0
sub.v14$activ_cs[is.na(sub.v14$activ_cs)] <- 0
sub.v14$activ_cu[is.na(sub.v14$activ_cu)] <- 0
sub.v14$activ_da[is.na(sub.v14$activ_da)] <- 0
sub.v14$activ_fs[is.na(sub.v14$activ_fs)] <- 0
sub.v14$activ_gs[is.na(sub.v14$activ_gs)] <- 0
sub.v14$activ_gov[is.na(sub.v14$activ_gov)] <- 0
sub.v14$activ_mp[is.na(sub.v14$activ_md)] <- 0
sub.v14$activ_md[is.na(sub.v14$activ_none)] <- 0
sub.v14$activ_soc[is.na(sub.v14$activ_soc)] <- 0
sub.v14$activ_rel[is.na(sub.v14$activ_rel)] <- 0
sub.v14$activ_art[is.na(sub.v14$activ_art)] <- 0
sub.v14$activ_other[is.na(sub.v14$activ_other)] <- 0
...
sub.v14 <- sub.v14 %>% 
  mutate(activ_yn = case_when(activ_none == 1 ~ 0,
                              ...
                              activ_other == 1 ~ 1)) %>%
  filter(!is.na(activ_yn))
sub.v14$activ_yn <- as.logical(sub.v14$activ_yn)

table(sub.v14$activ_yn, useNA = "always")     # tabulate and check output
jarque.bera.test(sub.v14$activ_yn)
#sub.v14 = subset(sub.v14, select= -c(activ_none, activ_ac:activ_other))    # clean up vars
```
...
# Binary variable for school type
table(sub.v14$school2_4yr)
table(sub.v14$school2_cc)
table(sub.v14$school2_ad)
...
sub.v14 <- sub.v14 %>%
  mutate(school2_type = case_when(school2_4yr == TRUE ~ 1,
                                  ...
                                  school2_ad == TRUE ~ 3)) %>%
  filter(!is.na(school2_type))
sub.v14$school2_type <- as.factor(sub.v14$school2_type)

table(sub.v14$school2_type, useNA = "always") # check output
```
...
sub.v16 <- sub.v14 %>% 
  mutate(ins_cover = case_when((ins_1 == 1 | ins_mh == 4 | ins_mh == 5 | ins_ade == 3) ~ 0,   # inadequate coverage
                               (ins_1 == 0 | ins_mh == 1 | ins_mh == 2 | ins_ade == 2) ~ 1)) %>% # adequate MH coverage
  filter(!is.na(ins_cover))   
sub.v16$ins_cover <- as.logical(sub.v16$ins_cover)
table(sub.v16$ins_cover, useNA = "always")   # check output
jarque.bera.test(sub.v16$ins_cover)

#sub.v16 = subset(sub.v16, select= -c(ins_1, ins_mh, ins_ade, ins_inade)) # clean up var
```
...
```{r drugs_yn}
## add summary of each
sub.v16$drug_coc[is.na(sub.v16$drug_coc)] <- 0
sub.v16$drug_her[is.na(sub.v16$drug_her)] <- 0
sub.v16$drug_met[is.na(sub.v16$drug_met)] <- 0
sub.v16$drug_stim[is.na(sub.v16$drug_stim)] <- 0
sub.v16$drug_ect[is.na(sub.v16$drug_ect)] <- 0
sub.v16$drug_opi[is.na(sub.v16$drug_opi)] <- 0
sub.v16$drug_other[is.na(sub.v16$drug_other)] <- 0

table(sub.v16$drug_coc)
table(sub.v16$drug_her)  # too small
table(sub.v16$drug_met)  # too small
table(sub.v16$drug_stim)
table(sub.v16$drug_ect)  # too small
table(sub.v16$drug_opi)   # too small
table(sub.v16$drug_other)  # too vague, small
...
sub.v16 <- sub.v16 %>%
  mutate(drugs_yn = case_when(drug_coc == 1 ~ 1,
                              ...
                              drug_opi == 1 ~ 1,
                              drug_other == 1 ~ 1))
sub.v16$drugs_yn[is.na(sub.v16$drugs_yn)] <- 0
sub.v16$drugs_yn <- as.logical(sub.v16$drugs_yn)

table(sub.v16$drugs_yn, useNA = "always")
jarque.bera.test(sub.v16$drugs_yn)
```

## Marijuana use
```{r test drug_mar}
sub.v16$drug_mar[is.na(sub.v16$drug_mar)] <- 0
table(sub.v16$drug_mar)  
...
sub.v16$drug_mar <- as.logical(sub.v16$drug_mar)

table(sub.v16$drug_mar, useNA = "always")
jarque.bera.test(sub.v16$drug_mar)
```
...
sub.v16 <- sub.v16 %>%
  ...
  mutate(fincur = case_when(fincur == 3 ~ 0,  # sometimes
                            ...
                            fincur == 1 ~ 1,  # always or often stressful
                            fincur == 2 ~ 1))
sub.v16$fincur <- as.logical(sub.v16$fincur)

table(sub.v16$fincur, useNA = "always")
jarque.bera.test(sub.v16$fincur)
```

## Abuse or assault, in the last year only
```{r assault}
table(sub.v16$abuse_life)
table(sub.v16$abuse_recent)
...
sub.v17 <- sub.v16 %>%
  mutate(abuse_year = case_when(abuse_recent == 1 ~ 1,
                                ...
                                abuse_life == 0 ~ 0,
                                is.na(abuse_life) ~ 0))
table(sub.v17$abuse_year)   # check output
...
sub.v17 <- sub.v17 %>%
  ...
  mutate(assault = case_when(assault == 1 ~ 1,
                             assault == 0 ~ 0,
                             is.na(assault) ~ 0))
sub.v17$assault <- as.logical(sub.v17$assault)

# tabulate and check output
table(sub.v17$assault, useNA = "always") 
jarque.bera.test(sub.v17$assault)
                                         
sub.v17 = subset(sub.v17, select= -c(abuse_life, abuse_recent)) # clean up var
```

## Binging frequency
```{r binge_fr}
# set all binge_fr values equal to 0
sub.v17$binge_fr_f[is.na(sub.v17$binge_fr_f)] <- 0
sub.v17$binge_fr_m[is.na(sub.v17$binge_fr_m)] <- 0
sub.v17$binge_fr_o[is.na(sub.v17$binge_fr_o)] <- 0

# create one binge var
sub.v18 <- sub.v17 %>% 
  mutate(binge_fr = as.numeric(binge_fr_f) + as.numeric(binge_fr_m) + as.numeric(binge_fr_o))
table(sub.v18$binge_fr)

sub.v18 <- sub.v18 %>%
  mutate(binge = case_when(binge_fr >= 4 ~ 1,
                           binge_fr < 4 ~ 0)) %>%
  filter(!is.na(binge))
sub.v18$binge <-as.logical(sub.v18$binge)

table(sub.v18$binge, useNA = "always") # check output
jarque.bera.test(sub.v18$binge)

sub.v18 = subset(sub.v18, select= -c(binge_fr_f, binge_fr_m, binge_fr_o)) # clean up var
```
...
sub.v18 <- sub.v18 %>% 
  mutate(psyhx = case_when((prov_pes == 1 | prov_inp == 1 | prov_par == 1) ~ 1, 
                           ...
                           ther_ever == 0 ~ 0)) %>%
  filter(!is.na(psyhx))
sub.v18$psyhx <- as.logical(sub.v18$psyhx)

table(sub.v18$psyhx, useNA = "always")   # check output
jarque.bera.test(sub.v18$psyhx)

sub.v18 = subset(sub.v18, select= -c(prov_pes, prov_inp, prov_par)) # clean up var
```
...
sub.v18 <- sub.v18 %>%
  mutate(gpa_sr = case_when(gpa_sr == 0 ~ 1,   # A+
                            ...
                            gpa_sr == 10 ~ 5,  # Don't know
                            is.na(gpa_sr) ~ 5))
sub.v18$gpa_sr <- as.numeric(sub.v18$gpa_sr)

table(sub.v18$gpa_sr, useNA = "always")
jarque.bera.test(sub.v18$gpa_sr)
```

## PHQ risk/severity
```{r deprawsc}
table(sub.v18$deprawsc)
sub.v18$deprawsc <- as.numeric(sub.v18$deprawsc)
...
sub.v20 <- sub.v18 %>%
  ...
  mutate(dep_na = case_when(is.na(deprawsc) ~ 1,
                            !is.na(deprawsc) ~ 0))
sub.v20$deprawsc <- as.numeric(sub.v20$deprawsc)

# Check outputs
table(sub.v20$dep_none)
table(sub.v20$dep_mild)
table(sub.v20$dep_mod)
table(sub.v20$dep_modsev)
table(sub.v20$dep_sev)
table(sub.v20$dep_na)
...
table(sub.v20$dep_type, useNA = "always")
```

## GAD7 total
```{r anx_score}
sub.v20$anx_score <- as.numeric(sub.v20$anx_score)
...
sub.v20 <- sub.v20 %>%
  ...
  mutate(anx_type = case_when(anx_score > 14 ~ 4,
                              ...
                              anx_score < 15 ~ 3))                    # anx severe
sub.v20$anx_score <- as.numeric(sub.v20$anx_score)
jarque.bera.test(sub.v20$anx_score)

table(sub.v20$anx_type, useNA = "always") # check output
```
...
sub.v20 <- sub.v20 %>%
  ...
  mutate(aca_impa = case_when(aca_impa == 1 ~ 1,
                              ...
                              aca_impa == 3 ~ 3,
                              aca_impa == 4 ~ 4)) 
sub.v20$aca_impa <- as.numeric(sub.v20$aca_impa)

table(sub.v20$aca_impa, useNA = "always")
jarque.bera.test(sub.v20$aca_impa)
```
...
table(sub.v22$aaq_tot)

#  for loop for predictive cutoff point needed
 rs_df <- data.frame("cutoff" = c(), "rs" = c())
 for (i in 8:48) {
   aaq_bin <- ifelse(sub.v22$aaq_tot >= i, TRUE, FALSE)
   model = lm(sub.v22$sui_idea ~ aaq_bin)
   rs_df <- rbind(rs_df, c(i, summary(model)$r.squared))
...
# deem 'rs_df_arranged[1,1]' as SI cutoff
 sub.v22 <- sub.v22 %>%
   ...
   mutate(aaq_dum = case_when(aaq_dum == 1 ~ 1,
                              aaq_dum == 0 ~ 0)) %>%
   filter(!is.na(aaq_dum))
sub.v22$aaq_dum <- as.logical(sub.v22$aaq_dum)

table(sub.v22$aaq_dum, useNA = "always")
```
...
sub.v22 <- sub.v22 %>%
  mutate(persist = case_when(persist == 1 ~ 1,   # confident
                             ...
                             persist == 5 ~ 5, 
                             persist == 6 ~ 6))  # not
sub.v22$persist <- as.numeric(sub.v22$persist)

table(sub.v22$persist, useNA = "always") # check output
jarque.bera.test(sub.v22$persist)
```
...
sub.v24 <- sub.v22 %>%
  mutate(dep_impa = case_when(dep_impa == 1 ~ 1,
                              ...
                              dep_impa == 4 ~ 4)) %>%
  filter(!is.na(dep_impa))
sub.v24$dep_impa <- as.numeric(sub.v24$dep_impa)

table(sub.v24$dep_impa, useNA = "always")
jarque.bera.test(sub.v24$dep_impa)
```
...
sub.v24 <- sub.v24 %>%
  mutate(gad7_impa = case_when(gad7_impa == 1 ~ 1,
                               ...
                               gad7_impa == 4 ~ 4)) %>%
  filter(!is.na(gad7_impa))
sub.v24$gad7_impa <- as.numeric(sub.v24$gad7_impa)

table(sub.v24$gad7_impa, useNA = "always")
jarque.bera.test(sub.v24$gad7_impa)
```
...
sub.v24 <- sub.v24 %>%
  mutate(percneed_cur = case_when(percneed_cur == 1 ~ 1,
                                 ...
                                 percneed_cur == 5 ~ 5,
                                 percneed_cur == 6 ~ 6))
sub.v24$percneed_cur <- as.numeric(sub.v24$percneed_cur)
table(sub.v24$percneed_cur, useNA = "always")
```
...
sub.v26 <- sub.v24 %>%
  mutate(belong1 = case_when(belong1 == 1 ~ 1,
                             ...
                             belong1 == 6 ~ 6)) %>%
  filter(!is.na(belong1))
table(sub.v26$belong1, useNA = "always")

sub.v26$belong1 <- as.numeric(sub.v26$belong1)
summary(sub.v26$belong1)
jarque.bera.test(sub.v26$belong1)

table(sub.v26$belong2)  # elective question, but common
table(sub.v26$belong8)  # nested x 2
table(sub.v26$belong9)  # not as good of a proxy
```
...
sub.v26 <- sub.v26 %>%
  mutate(satisfied_overall = case_when(satisfied_overall == 1 ~ 1,
                                       ...
                                       satisfied_overall == 6 ~ 6)) %>% 
  filter(!is.na(satisfied_overall))
sub.v26$satisfied_overall <- as.numeric(sub.v26$satisfied_overall)

table(sub.v26$satisfied_overall, useNA = "always")
jarque.bera.test(sub.v26$satisfied_overall)
```

## Flourishing Scale
```{r flourish}
sub.v26$flourish <- as.numeric(sub.v26$flourish)
table(sub.v26$flourish)


#  for loop for predictive cutoff point needed
rs_df <- data.frame("cutoff" = c(), "rs" = c())
for (i in 9:55) {
  flourish_bin <- ifelse(sub.v26$flourish >= i, TRUE, FALSE)
  model = lm(sub.v26$sui_idea ~ flourish_bin)
  rs_df <- rbind(rs_df, c(i, summary(model)$r.squared))
...
sub.v26 <- sub.v26 %>%
  mutate(flourish_dum = case_when(flourish > 50 ~ 5,
                                  ...
                                  flourish < 14 ~ 1)) %>%
  filter(!is.na(flourish_dum))
sub.v26$flourish_dum <- as.numeric(sub.v26$flourish_dum)

table(sub.v26$flourish_dum, useNA = "always")
```

## Informal Mental Health Support
```{r inf}
sub.v26$inf_1[is.na(sub.v26$inf_1)] <- 0
sub.v26$inf_2[is.na(sub.v26$inf_2)] <- 0
sub.v26$inf_3[is.na(sub.v26$inf_3)] <- 0
sub.v26$inf_4[is.na(sub.v26$inf_4)] <- 0
sub.v26$inf_5[is.na(sub.v26$inf_5)] <- 0
sub.v26$inf_6[is.na(sub.v26$inf_6)] <- 0
sub.v26$inf_7[is.na(sub.v26$inf_7)] <- 0
...
sub.v26 <- sub.v26 %>% 
  ...
  mutate(inf = case_when(inf_8 == 1 ~ 1,
                         ...
                         inf_7 == 1 ~ 0)) %>%
  filter(!is.na(inf))
sub.v26$inf <- as.logical(sub.v26$inf)
table(sub.v26$inf, useNA ="always")
jarque.bera.test(sub.v26$inf)

# clean up vars
#sub.v26 = subset(sub.v26, select= -c(inf_1:inf_help))
```
...
table(sub.v26$brs_tot)
sub.v26$brs_tot <- as.numeric(sub.v26$brs_tot)

table(is.na(sub.v26$brs_tot))

#  for loop for predictive cutoff point needed
rs_df <- data.frame("cutoff" = c(), "rs" = c())
for (i in 6:24) {
  brs_bin <- ifelse(sub.v26$brs_tot >= i, TRUE, FALSE)
  model = lm(sub.v26$sui_idea ~ brs_bin)
  rs_df <- rbind(rs_df, c(i, summary(model)$r.squared))
}
...
sub.v26 <- sub.v26 %>%
  ...
  mutate(brs_dum = case_when(brs_dum == 1 ~ 1,
                             brs_dum == 0 ~ 0)) %>%
  filter(!is.na(brs_dum))
sub.v26$brs_dum <- as.logical(sub.v26$brs_dum)

# output
rs_df_arranged[1,1]
table(sub.v26$brs_tot, useNA = "always")
table(sub.v26$brs_dum, useNA = "always")
```
...
for (j in 1:(length(var_names))) {
for (i in 1:nrow(school2)) {   # i corresponds to rows, j to columns
  school2[i, 2 + j] <- sum(!is.na(sub.v28[sub.v28$school2 == school2[[i,1]], var_names[j]]))
}
}
# add percentage in each column
school2_m <- as.matrix(school2[-1]) 
...
#create skeleton of missingness matrix
var <- count(sub.v28, sub.v28[var_names[k]])
var_ncol <- ncol(var)
...
for (j in 1:(length(var_names))) {
for (i in 1:nrow(var)) {   # i corresponds to rows, j to columns
  var[i, 2 + j] <- sum(!is.na(sub.v28[sub.v28[[var_names[k]]] == var[[i,1]], var_names[j]]))
}
}
# add percentage in each column
var_m <- as.matrix(var[-1]) 
...
sub_sem <- sub_sem %>% 
  # mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
# sub_sem <- na.omit(sub_sem)
view(sub_sem)   # check output

# # xlsx output
sub_sem_out <- as.data.frame(sub_sem)
save(sub_sem_out , file = "sub_nondummy.RData")
load("sub_nondummy.RData")
write_xlsx(sub_sem_out, path = "sub_nondummy.xls", col_names = TRUE, format_headers = TRUE)
```
...
sub_sem1 <- sub_sem1 %>% 
  # mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
sub_sem1 <- na.omit(sub_sem1)
head(sub_sem1)   # check output
```

# SEM model with partial cont and dummy
```{r si_model_tr1}
## Dummy col SEM w/ continuous
results_test_com1 <- dummy_cols(sub_sem1, select_columns = c("si_type", "sexual", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
#view(results_tr_com1)
...
fit_sub <- sem(SI_model_tr1, data=results_test_com1, ordered=T)
summary_test <- summary(fit_sub, fit.measures=TRUE, standardized = TRUE)

    ## running NULL model, baseline model RMSEA
nullRMSEA(fit_sub, scaled = FALSE, silent = FALSE)

    ## modification indices
modificationindices(fit_sub, minimum.value = 20)
...
```{r si_model_tr2}
## Dummy col SEM w/ continuous
results_tr_com1 <- dummy_cols(sub_sem1, select_columns = c("si_type", "sexual", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
#view(results_tr_com1)
...
fit_testM <- sem(SI_model_trM, data=results_tr_com1, ordered=T)
summary_test <- summary(fit_testM, fit.measures=TRUE, standardized = TRUE)
