...
jarque.bera.test(testdf.v11$ed_any)
```
...
jarque.bera.test(testdf.v13$religios)
```
...
table(testdf.v13$residenc, useNA = "always")
jarque.bera.test(testdf.v13$residenc)
```
...
jarque.bera.test(testdf.v14$activ_yn)
...
jarque.bera.test(testdf.v16$ins_cover)
...
testdf.v20 <- testdf.v20 %>%
  ...
  mutate(anx_type = case_when(anx_score > 14 ~ 4,
                              ...
                              anx_score < 15 ~ 3))                    # anx severe
testdf.v20$anx_score <- as.numeric(testdf.v20$anx_score)
jarque.bera.test(testdf.v20$anx_score)
...
## Create dummy vars
```{r dummy_vars}
results_tr_sem1 <- dummy_cols(testdf_sem, select_columns = c(
    # outcome var
    "si_type",
    # diathesis
    "sexual", "gender_tgd", "sib_freq", "ed_any", "dx_mh", "meds_count",
...
#save full var set for SEM
save(results_tr_df1, file = "testdf_sem1.RData")
load("testdf_sem1.RData")
write_xlsx(results_tr_df1, path = "testdf_sem1.xls", col_names = TRUE, format_headers = TRUE)
```
...
```{r si_model_tr1}
## Dummy col SEM w/ continuous
results_test_com1 <- dummy_cols(testdf_sem1, select_columns = c("si_type", "sexual", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
#view(results_tr_com1)
...
```{r si_model_tr2}
## Dummy col SEM w/ continuous
results_tr_com1 <- dummy_cols(testdf_sem1, select_columns = c("si_type", "sexual", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
#view(results_tr_com1)
