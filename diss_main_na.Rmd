...
traindf.v18 <- traindf.v18 %>%
  mutate(binge = case_when(binge_fr < 2 ~ 0
                           binge_fr >= 4 ~ 1,
                           binge_fr < 4 ~ 0,
                           is.na(binge_fr) ~ 0))

traindf.v18$binge <-as.numeric(traindf.v18$binge)
...
## GAD7 total
```{r anx_score}
table(traindf.v20$anx_score)    # PROBLEMSSSSSS

# Convert to categories for later
traindf.v20 <- traindf.v20 %>%
  mutate(anx_type = case_when(is.na(anx_score) ~ 0,                     # non answer
                              (anx_score < 5) ~ 1,                                 # anx none
                              ((anx_score > 4) & (anx_score < 10)) ~ 2,            # anx mild
                              ((anx_score > 9) & (anx_score < 15)) ~ 3,           # anx mod
                              (anx_score > 14) ~ 4))                    # anx severe
table(traindf.v20$anx_type)
```
