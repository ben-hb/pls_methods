...
clm_le <- clm(si_type ~ ins_cover + assault + gpa_sr + aca_impa + fincur + binge + 
                psyhx + drugs_yn,
              data = clm_le)
summary(clm_le)
coef(clm_le)
...
traindf.v22 <- traindf.v22 %>%
  ...
  mutate(anx_none = case_when(anx_score < 6 ~ 1,
                              anx_score > 5 ~ 0)) %>%
  mutate(anx_na = case_when(is.na(anx_score) ~ 1,
                            !is.na(anx_score) ~ 0))
# Check outputs
...
table(traindf.v22$anx_na)

traindf.v22 <- traindf.v22 %>%
  mutate(anx_type = case_when(anx_none == 1 ~ 1,
                              anx_mild == 1 ~ 2,
                              anx_mod == 1 ~ 3,
                              anx_sev == 1 ~ 4,
                              anx_na == 1 ~ 0))
...
traindf_tsm <- traindf.v23 %>% select(persist, anx_type, dep_secret, achieve4, aaq_tot)
