...
data.v9$dx_any <- as.logical(data.v9$dx_any)
data.v9$dx_dep <- as.logical(data.v9$dx_dep)
data.v9$dx_anx <- as.logical(data.v9$dx_anx)
data.v9$dx_bi <- as.logical(data.v9$dx_bi)
data.v9$dx_psy <- as.logical(data.v9$dx_psy) 
data.v9$dx_ea <- as.logical(data.v9$dx_ea)
data.v9$dx_tr <- as.logical(data.v9$dx_tr)
data.v9$dx_sa <- as.logical(data.v9$dx_sa)
data.v9$dx_adhd <- as.logical(data.v9$dx_adhd)
data.v9$dx_pers <- as.logical(data.v9$dx_pers)
data.v9$dx_none <- as.logical(data.v9$dx_none)
data.v9$dx_none <- as.numeric(data.v9$dx_none)
...
traindf_dia <- traindf.v12 %>% select(age, sexual, gender_tgd, finpast, sib_freq, race, body_sr, ed_any,
...
traindf_dia <- traindf.v12 %>% select(age, sexual, gender_tgd, finpast, sib_freq, race, body_sr,
                                dx_dep, dx_bi, dx_adhd, dx_tr, dx_anx, ed_any, meds_count)
...
clm_dia <- traindf.v12 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race,
                                  ed_any, dx_dep, dx_bi, dx_anx, dx_tr, dx_adhd, meds_count)

view(clm_dia)
dia_mod <- as.data.frame(clm_dia)

clm_mod <- clm(si_type ~ ., data = dia_mod)
...
traindf.v13$discrim <- as.factor(traindf.v13$discrim)
...
traindf.v14$fam_support_aca <- as.factor(traindf.v14$fam_support_aca)
```
...
traindf.v14 <- traindf.v14 %>%
  mutate(satisfied_overall = case_when(satisfied_overall == 1 ~ 1,
                                       ...
                                       satisfied_overall == NA ~ 0))

table(traindf.v14$satisfied_overall)
traindf.v14$satisfied_overall <- as.factor(traindf.v14$satisfied_overall)
```
...
traindf.v14$activ_ac <- as.numeric(traindf.v14$activ_ac)
traindf.v14$activ_athc <- as.numeric(traindf.v14$activ_athc)
traindf.v14$activ_athv <- as.numeric(traindf.v14$activ_athv)
traindf.v14$activ_athi <- as.numeric(traindf.v14$activ_athi)
traindf.v14$activ_cs <- as.numeric(traindf.v14$activ_cs)
traindf.v14$activ_cu <- as.numeric(traindf.v14$activ_cu)
traindf.v14$activ_da <- as.numeric(traindf.v14$activ_da)
traindf.v14$activ_fs <- as.numeric(traindf.v14$activ_fs)
traindf.v14$activ_gs <- as.numeric(traindf.v14$activ_gs)
traindf.v14$activ_gov <- as.numeric(traindf.v14$activ_gov)
traindf.v14$activ_mp <- as.numeric(traindf.v14$activ_md)
traindf.v14$activ_md <- as.numeric(traindf.v14$activ_none)
traindf.v14$activ_soc <- as.numeric(traindf.v14$activ_soc)
traindf.v14$activ_rel <- as.numeric(traindf.v14$activ_rel)
traindf.v14$activ_art <- as.numeric(traindf.v14$activ_art)
traindf.v14$activ_other <- as.numeric(traindf.v14$activ_other)
...
##EFA Environment
```{r EFA Environment}
traindf_env <- traindf.v15 %>% select(discrim, religios, env_mh, residenc, activ_yn, divers, 
...
# results_env <- dummy_cols(traindf_env, select_columns = c("discrim", "religios", "env_mh", "residenc","talk",
...
clm_env <- clm_env %>% 
 ...
 clm_env2 <- clm(si_type ~ discrim + residenc + religios + activ_yn + divers + school2_type +
                   satisfied_overall + talk, data = clm_env)
 summary(clm_env2)
...
traindf.v16 <- traindf.v16 %>% 
  mutate(ins_cover = case_when((ins_1 == 1 | ins_mh == 4 | ins_mh == 5 | ins_ade == 3) ~ 1,   # inadequate coverage
                               ...
                               (ins_1 == NA | ins_mh == NA | ins_ade == NA) ~ 0))             # non-response
table(traindf.v16$ins_cover)   # check output
traindf.v16$ins_cover <- as.logical(traindf.v16$ins_cover)

traindf.v16 = subset(traindf.v15, select= -c(ins_1, ins_mh, ins_ade, ins_inade)) # clean up var
view(traindf.v16$ins_cover)
```
...
traindf.v16$fincur <- as.factor(traindf.v16$fincur)
```
...
traindf.v17 <- traindf.v16 %>%
  mutate(abuse_year = case_when(abuse_recent == 1 ~ 1,
                                ...
                                abuse_recent == 5 ~ 0,
                                abuse_life == 0 ~ 0))
traindf.v17$abuse_year <- as.numeric(traindf.v17$abuse_year)
...
table(traindf.v18$binge_fr) # check output
traindf.v18$binge_fr <- as.factor(traindf.v18$binge_fr)
...
traindf_le <- traindf.v19 %>% select(ins_cover, gpa_sr, aca_impa, fincur, binge_fr,
                                     assault_sex, assault_phys, assault_sex, psyhx, drugs_yn)
...
## EFA LE
```{r le_pca}
traindf_le <- traindf.v19 %>% select(ins_cover, gpa_sr, aca_impa, fincur, binge_fr, drugs_yn,
                                     assault, psyhx)
...
results_le <- dummy_cols(traindf_le, select_columns = c("ins_cover", "assault", "gpa_sr", "psyhx", 
                                                        "aca_impa", "fincur", "binge_fr", "drugs_yn"),
                         remove_first_dummy = TRUE)
...
```{r clm_LE}
clm_le <- traindf.v19 %>% select(si_type, ins_cover, gpa_sr, aca_impa, fincur, assault,
                                 binge_fr, psyhx, drugs_yn)

clm_le <- NA_preproc(clm_le)
clm_le <- as.data.frame(clm_le)

# clm_le <- clm(si_type ~ ins_cover + assault_emo + assault_phys + assault_phys + gpa_sr + 
#                 aca_impa + fincur + binge_fr + psyhx + drugs_yn,
#               data = clm_le)
# summary(clm_le)
# coef(clm_le)
...
traindf.v20$aca_impa <- as.factor(traindf.v20$aca_impa)
```
...
traindf_def <- traindf.v21 %>% select(aca_impa, deprawsc)
...
results_def <- dummy_cols(traindf_def, select_columns = c("aca_impa", "deprawsc"),
                         remove_first_dummy = TRUE)
...
## CLM defeat
```{r CLM defeat}
results2_def <- traindf.v21 %>% select(si_type, aca_impa, deprawsc)

clm_def <- NA_preproc(results2_def)
clm_def <- as.data.frame(clm_def)

clm_def <- clm(si_type ~ aca_impa + deprawsc, data = clm_def)
...
## GAD7 total
```{r anx_score}
table(traindf.v22$anx_score)
traindf.v22$anx_score <- as.numeric(traindf.v22$anx_score)
...
traindf.v22 <- traindf.v22 %>%
  ...
  mutate(anx_sev = case_when(anx_score > 15 ~ 1,
                             anx_score < 16 ~ 0))
# Check outputs
table(traindf.v22$anx_none)
table(traindf.v22$anx_mild)
table(traindf.v22$anx_mod)
table(traindf.v22$anx_sev)
table(traindf.v22$anx_score)
```
...
traindf.v22$dep_secret <- as.factor(traindf.v22$dep_secret)
```
...
traindf.v22$aaq_tot <- as.numeric(traindf.v22$aaq_tot)
...
## Persist
```{r persist}
traindf.v22$persist <- as.factor(traindf.v22$persist)
...
traindf_tsm <- traindf.v23 %>% select(persist, anx_score, dep_secret, achieve4, aaq_tot)
...
results_tsm <- dummy_cols(traindf_tsm, select_columns = c("persist", "anx_score", "dep_secret", "achieve4", "aaq_tot"),
                         remove_first_dummy = TRUE)
...
## CLM TSM
```{r CLM TSM}
clm_tsm <- traindf.v23 %>% select(si_type, persist, anx_score, dep_secret, aaq_tot)

clm_tsm <- NA_preproc(clm_tsm)
clm_tsm <- as.data.frame(clm_tsm)

clm_tsm <- clm(si_type ~ persist + anx_score + dep_secret + aaq_tot,
               data = clm_tsm) # ordered probit or logit may be needed
...
traindf.v24$dep_impa <- as.factor(traindf.v24$dep_impa)
```

## Anxiety impact
```{r gad7_impa}
table(traindf.v24$gad7_impa)
traindf.v24$gad7_impa <- as.factor(traindf.v24$gad7_impa)
```

## Current needs
```{r perneed_cur}
table(traindf.v24$percneed_cur)
traindf.v24$percneed_cur <- as.factor(traindf.v24$percneed_cur)
```
...
traindf.v26$belong1 <- as.factor(traindf.v26$belong1)
...
# clean up vars
traindf.v26 = subset(traindf.v26, select= -c(inf_1:inf_help, inf_sum))
```
...
table(traindf.v26$stig_pcv_2b)  # Most people feel that MH tx is a sign of personal failure.
table(traindf.v26$stig_pcv_3b)  # Most people think less of a person who has received MH tx.
table(traindf.v26$stig_per_2b)  # I feel that receiving MH tx is a sign of personal failure.
table(traindf.v26$stig_per_3b)  # I would think less of a person who has received MH tx.
...
traindf_mm <- traindf.v27 %>% select(inf, flourish_dum, belong1, mh_stigma, brs_dum)
...
results_mm <- dummy_cols(traindf_mm, select_columns = c("inf", "flourish_dum", "belong1", "mh_stigma", "brs_dum"),
                         remove_first_dummy = TRUE)
...
```{r CLM mm}
clm_mm <- traindf.v27 %>% select(si_type, inf, flourish, belong1, mh_stigma, brs_tot)

clm_mm <- NA_preproc(clm_mm)
clm_mm <- as.data.frame(clm_mm)

clm_mm <- clm(si_type ~ inf + flourish + belong1 + mh_stigma + brs_tot, data = clm_mm)
...
var_names <- c(
  # diathesis
  "age", "sexual", "gender_tgd", "sib_freq", "race", "ed_any", "dx_dep", "dx_bi", "dx_tr", "dx_anx", 
  "dx_adhd", "meds_count",
  # environment
  ...
  "aca_impa", "deprawsc",
  # threats to self-moderation
  "persist", "anx_score", "dep_secret", "aaq_tot",
  # entrapment
...
traindf_sem2 <- traindf_sem %>% 
  # mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
traindf_sem2   # check output

traindf_sem2_test = traindf_sem2[!is.na(traindf_sem2),]
traindf_sem2_test
# # xlsx output
...
## Create dummy vars
```{r dummy_vars}
results_tr_sem <- dummy_cols(traindf_sem2_test, select_columns = c(
    # outcome var
    "si_type",
    # diathesis
...
```{r}
# zeroinfl <- function(si_type, results_tr_sem2, na.action = TRUE, weights = TRUE, offset = TRUE,
#                      dist = c("binomial"),
#                      link = c("logit"))
```
...
```{r model_lev_corr}
results_corr <- NA_preproc(results_tr_sem2)
...
```{r save dataset}
# results_tr_df <- as.data.frame(results_tr_sem2)
...
SI_model_tr <- '
              # measurement model
                  ...
                  dia_sem =~ age_19 + age_20 + age_21 + age_22 + age_23 + age_24 + age_25 +
                             sexual_lgb + sexual_qq + sexual_het + gender_tgd_2 +
                             sib_freq_2 + sib_freq_3 + sib_freq_4 + sib_freq_5 + 
                                sib_freq_6 + sib_freq_7 + sib_freq_NA + 
                             race_ame + race_asi + race_his + race_pac + race_ara + 
                                race_whi + race_oth + race_non + race_bla + 
                             ed_any_2 + ed_any_NA + 
                             dx_dep_1 + dx_dep_NA + 
                             dx_bi_1 + dx_bi_NA + 
                             dx_anx_1 +  dx_anx_NA + 
                             dx_tr_1 + dx_tr_NA + 
                             dx_adhd_1 + dx_adhd_NA +
                             meds_count_2 + meds_count_3 + meds_count_4 + meds_count_5 + 
                                meds_count_6 + meds_count_7
                  
                  env_sem =~ divers_2 + divers_3 + divers_4 + divers_5 + divers_6 + divers_NA +
                             satisfied_overall_2 + satisfied_overall_3 + satisfied_overall_4 + 
                                satisfied_overall_5 + satisfied_overall_6 + satisfied_overall_NA +
                             talk_for + talk_inf + talk_com + talk_no +
                             religios_2 + religios_3 + religios_4 + religios_5 +
                             discrim_2 + discrim_3 + discrim_4 + discrim_5 + discrim_6 + discrim_NA +
                             sch_cc + sch_ad + sch_4yr +
                             activ_yn_1 + activ_yn_NA
                  
                  le_sem =~  ins_cover_1 + ins_cover_NA +
                             gpa_sr_2 + gpa_sr_3 + gpa_sr_4 + gpa_sr_5 + gpa_sr_6 + gpa_sr_7 + 
                                gpa_sr_8 + gpa_sr_9 + gpa_sr_10 + gpa_sr_11 + gpa_sr_NA + 
                             binge_fr_2 + binge_fr_3 + binge_fr_4 + binge_fr_5 + binge_fr_6 + 
                                binge_fr_7 + binge_fr_8 + 
                             psyhx_1 + psyhx_NA +
                             fincur_2 + fincur_3 + fincur_4 + fincur_5 + fincur_NA + 
                             drugs_yn_2 + drugs_yn_NA +
                             assault_2 + assault_NA
                  
                  def_sem =~ aca_impa_2 + aca_impa_3 + aca_impa_4 + aca_impa_NA +
                             deprawsc_2 + deprawsc_3 + deprawsc_4 + deprawsc_5 + deprawsc_6 + 
                                deprawsc_7 + deprawsc_8 + deprawsc_9 + deprawsc_10 + deprawsc_11 + 
                                deprawsc_12 + deprawsc_13 + deprawsc_14 + deprawsc_15 + deprawsc_16 + 
                                deprawsc_17 + deprawsc_18 + deprawsc_19 + deprawsc_20 + deprawsc_21 + 
                                deprawsc_22 + deprawsc_23 + deprawsc_24 + deprawsc_25 + deprawsc_26 + 
                                deprawsc_27 + deprawsc_28 + deprawsc_NA
                  
                  tsm_sem =~ persist_2 + persist_3 + persist_4 + persist_5 + persist_6 + persist_NA +
                             anx_score_2 + anx_score_3 + anx_score_4 + anx_score_5 + anx_score_6 +
                                anx_score_7 + anx_score_8 + anx_score_9 + anx_score_10 + anx_score_11 + 
                                anx_score_12 + anx_score_13 + anx_score_14 + anx_score_15 + anx_score_16 + 
                                anx_score_17 + anx_score_18 + anx_score_19 + anx_score_20 + anx_score_21 + 
                                anx_score_22 + anx_score_NA + 
                             dep_secret_2 + dep_secret_3 + dep_secret_4 + dep_secret_5 + dep_secret_6 + 
                                dep_secret_NA
                  
                  entrap_sem =~ dep_impa_2 + dep_impa_3 + dep_impa_4 + dep_impa_NA + 
                                gad7_impa_2 + gad7_impa_3 + gad7_impa_4 + gad7_impa_NA +
                                percneed_cur_2 + percneed_cur_3 + percneed_cur_4 + percneed_cur_5 + 
                                    percneed_cur_6 + percneed_cur_NA
                  
                  mm_sem =~  inf_2 + inf_3 + inf_4 + inf_5 + inf_6 + inf_7 + inf_8 + inf_9 + inf_NA + 
                             flourish_2 + flourish_3 + flourish_4 + flourish_5 + flourish_6 + flourish_7 + 
                                flourish_8 + flourish_9 + flourish_10 + flourish_11 + flourish_12 + flourish_13 + 
                                flourish_14 + flourish_15 + flourish_16 + flourish_17 + flourish_18 + flourish_19 + 
                                flourish_20 + flourish_21 + flourish_22 + flourish_23 + flourish_24 + flourish_25 + 
                                flourish_26 + flourish_27 + flourish_28 + flourish_29 + flourish_30 + flourish_31 + 
                                flourish_32 + flourish_33 + flourish_34 + flourish_35 + flourish_36 + flourish_37 + 
                                flourish_38 + flourish_39 + flourish_40 + flourish_41 + flourish_42 + flourish_43 +
                                flourish_44 + flourish_45 + flourish_46 + flourish_47 + flourish_48 + flourish_49 + 
                                flourish_NA +
                             belong1_2 + belong1_3 + belong1_4 + belong1_5 + belong1_6 + belong1_NA + 
                             brs_tot_2 + brs_tot_3 + brs_tot_4 + brs_tot_5 + brs_tot_6 + brs_tot_7 + brs_tot_8 + 
                                brs_tot_9 + brs_tot_10 + brs_tot_11 + brs_tot_12 + brs_tot_13 + brs_tot_14 + 
                                brs_tot_15 + brs_tot_16 + brs_tot_17 + brs_tot_18 + brs_tot_19 + brs_tot_20 +
                                brs_tot_21 + brs_tot_NA
...
results_tr_sem2 <- as.data.frame(results_tr_sem2)
results_tr_sem2 <- NA_preproc(results_tr_sem2)

fit_tr <- cfa(SI_model_tr, data = results_tr_sem2)
           # ordered = c("si_type_2", "si_type_3", "si_type_4",
           ...
           #                      "anx_score_17", "anx_score_18", "anx_score_19", "anx_score_20", "anx_score_21",
           #                      "anx_score_22", "anx_score_NA",
           #            # "aaq_tot_2", "aaq_tot_3", "aaq_tot_4", "aaq_tot_5", "aaq_tot_6", "aaq_tot_7",
           #            #           "aaq_tot_8", "aaq_tot_9", "aaq_tot_10", "aaq_tot_11", "aaq_tot_12", "aaq_tot_13",
           #            #           "aaq_tot_14", "aaq_tot_15", "aaq_tot_16", "aaq_tot_17", "aaq_tot_18", "aaq_tot_19",
           #            #           "aaq_tot_20", "aaq_tot_21", "aaq_tot_22", "aaq_tot_23", "aaq_tot_24", "aaq_tot_25",
           #            #           "aaq_tot_26", "aaq_tot_27", "aaq_tot_28", "aaq_tot_29", "aaq_tot_30", "aaq_tot_31",
           #            #           "aaq_tot_32", "aaq_tot_33", "aaq_tot_34", "aaq_tot_35", "aaq_tot_36", "aaq_tot_37",
           #            #           "aaq_tot_38", "aaq_tot_39", "aaq_tot_40", "aaq_tot_41", "aaq_tot_42", "aaq_tot_43",
           #            #           "aaq_tot_NA",
           #            "dep_impa_2", "dep_impa_3", "dep_impa_4", "dep_impa_NA",
           #            "gad7_impa_2", "gad7_impa_3", "gad7_impa_4", "gad7_impa_NA",
           #            "flourish_2", "flourish_3", "flourish_4", "flourish_5", "flourish_6", "flourish_7",
           #                      "flourish_8", "flourish_9", "flourish_10", "flourish_11", "flourish_12", "flourish_13",
           #                      "flourish_14", "flourish_15", "flourish_16", "flourish_17", "flourish_18", "flourish_19",
...
#lavInspect(results_tr_sem2, "cov.lv")
...
results_test_sem <- dummy_cols(testdf_sem2, select_columns = c(
    ...
    "divers", "satisfied_overall", "talk", "religios", "discrim", "school2_type", "activ_yn",
    # life events
    "ins_cover", "gpa_sr", "binge_fr", "psyhx", "fincur", "drugs_yn", "assault_emo", "assault_phys", 
    "assault_sex",
    # defeat
    "aca_impa", "deprawsc",
    # threats to self-moderation
