...
```{r}
# Define diathesis vars for correlation matrix
traindf_prem <- traindf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, meds_count,
                                religios, activ_yn, school2_type, ins_cover, gpa_sr, fincur, assault, binge, drugs_yn, drug_mar, psyhx)
...
results_prem <- dummy_cols(traindf_prem, select_columns = c("age", "sexual", "gender_tgd", "sib_freq", "race", "ed_any", 
                                                          "dx_mh", "meds_count","religios", "activ_yn", "school2_type", 
                                                          "ins_cover", "gpa_sr", "fincur", "assault", "binge", "drugs_yn", 
                                                          "drug_mar", "psyhx"),
                         remove_first_dummy = TRUE)

clm_prem <- clm(si_type ~ age + sexual + gender_tgd + sib_freq + race + ed_any + dx_mh + meds_count +
                religios + activ_yn + school2_type + ins_cover + gpa_sr + fincur + assault + binge + 
                drugs_yn + drug_mar + psyhx,
              data = results_prem)
summary(clm_prem)
coef(clm_prem)
...
results_tr_com1 <- dummy_cols(traindf_sem1, select_columns = c("si_type", "age", "sexual", "inf", "race", "school2_type"),
...
fit_train1 <- sem(SI_model_tr1, data=results_tr_com1, ordered = TRUE)
...
SI_model_test1 <- '
                # measurement model
                  ...
                  prem  =~ age_2 + age_3 +
                           sexual_2 + sexual_3 + gender_tgd + sib_freq + 
                           race_2 + race_3 + race_4 + race_5 + ed_any + dx_mh +
                           meds_count + satisfied_overall +  religios + drug_mar +
...
fit_test1 <- sem(SI_model_test1, data=results_test_com1, ordered = TRUE)
