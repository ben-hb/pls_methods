...
## Age
```{r age}
data.v3$age <- as.numeric(data.v3$age)
table(data.v3$age) 
```
...
data.v3 <- data.v3 %>%  
  mutate(gender_tgd = case_when(gender == 2 ~ 1,
                                gender == 3 ~ 1,
                                gender == 1 ~ 0))
table(data.v3$gender_tgd)
data.v3$gender_tgd <- as.factor(data.v3$gender_tgd)
```
...
table(data.v3$gender_tgd)   # check output
```
...
data.v5 <- data.v4 %>%
  ...
  mutate(sexual = case_when((sexual == 9 | sexual == 10 | sexual == 5 | sexual == 6 | sexual == 4) ~ 3,   # queer, nonbinary
                             ...
                             sexual == 6 ~ 4))
data.v4$sexual <- as.factor(data.v4$sexual)
...
data.v9 <- data.v8 %>%   # filters out any respondents who skipped the MH section 
  ...
  mutate(dx_mh = case_when(dx_mh == 1 ~ 1,
                           is.na(dx_mh) ~ 0))
data.v9$dx_dep <- as.logical(data.v9$dx_mh)
...
table(data.v10$religios)       # Check output
```
...
table(traindf.v11$meds_count)  # check output
...
### EFA diathesis
```{r EFA diathesis}
traindf_dia <- traindf.v12 %>% select(age, sexual, gender_tgd, finpast, sib_freq, race, body_sr,
...
results_dia <- dummy_cols(traindf_dia, select_columns = c("age", "sexual", "gender_tgd", "finpast", 
                                                       "sib_freq", "race", "body_sr", "dx_mh", "ed_any",
                                                       "meds_count"), 
...
table(traindf.v14$talk)  # check output
traindf.v14$talk <- as.factor(traindf.v14$talk)

traindf.v14 = subset(traindf.v14, select= -c(talk1_1:talk1_9, talk1sig, talk1fr, talk1rel, talk1no, talk1sup, talk1rm, talk1pro, talk1fam)) # clean up
```
...
```{r medtherc}
table(traindf.v14$medther)  # med or therapy
table(traindf.v14$medtherc) # meds + therapy
```
...
table(traindf.v14$satisfied_overall)
```
...
table(traindf.v14$activ_yn)     # tabulate and check output
...
table(traindf.v14$school2_type)
...
traindf_env <- traindf.v15 %>% select(discrim, religios, env_mh, residenc, ther_vis, activ_yn, divers, 
...
##EFA Environment
```{r EFA Environment}
traindf_env <- traindf.v15 %>% select(discrim, religios, residenc, activ_yn, school2_type, 
...
results_env <- dummy_cols(traindf_env, select_columns = c("discrim", "religios", "residenc", "activ_yn",
...
```{r CLM environment}
clm_env <- traindf.v15 %>% select(si_type, discrim, religios, residenc, activ_yn, 
                                  school2_type, satisfied_overall, talk)
...
clm_env2 <- clm(si_type ~ discrim + religios + residenc + activ_yn +
                  school2_type + satisfied_overall + talk, 
                data = clm_env)
summary(clm_env2)
coef(clm_env2)

# clm_env2 <- confint(clm_env2)
# clm_env2
# 
# model.env <- tidy(clm_env2)  # Convert model to dataframe for easy manipulation
# model.env
# model.env  %>%
#    mutate(or = exp(estimate),  # Odds ratio/gradient
#           var.diag = diag(vcov(clm_env2)),  # Variance of each coefficient
#           or.se = sqrt(or^2 * var.diag))  # Odds-ratio adjusted
```
...
traindf.v16 <- traindf.v16 %>% 
  mutate(ins_cover = case_when((ins_1 == 1 | ins_mh == 4 | ins_mh == 5 | ins_ade == 3) ~ 1,   # inadequate coverage
                               (ins_1 == 0 | ins_mh == 1 | ins_mh == 2 | ins_ade == 2) ~ 2,   # adequate MH coverage
                               (is.na(ins_1) ~ 3)))       # non-response
table(traindf.v16$ins_cover)   # check output
traindf.v16$ins_cover <- as.factor(traindf.v16$ins_cover)
...
```{r fincur}
traindf.v16 <- traindf.v16 %>%
  mutate(fincur = case_when(fincur == 1 ~ 1,  # always or often stressful
                            fincur == 2 ~ 1,  
                            fincur == 3 ~ 2,  # sometimes
                            fincur == 4 ~ 3,  # rarely or never
                            fincur == 5 ~ 3,
                            is.na(fincur) ~ 0))  # non-response
traindf.v16$fincur <- as.numeric(traindf.v16$fincur)
table(traindf.v16$fincur)
```
...
table(traindf.v18$binge) # check output
...
table(traindf.v18$psyhx)   # check output
...
traindf.v18 <- traindf.v18 %>%
  mutate(gpa_sr = case_when(gpa_sr == 0 ~ 1,   # A+
                            ...
                            gpa_sr == 10 ~ 0,  # Don't know
                            is.na(gpa_sr) ~ 0))
table(traindf.v18$gpa_sr)
...
results_le <- dummy_cols(traindf_le, select_columns = c("ins_cover", "gpa_sr", "aca_impa", "psyhx",
                                                        "fincur", "binge","drugs_yn", "assault"),
                         remove_first_dummy = TRUE)
...
table(traindf.v20$anx_type) # check output
```
...
clm_def <- clm(si_type ~ aca_impa + dep_type, data = clm_def)
...
# deem 'rs_df_arranged[1,1]' as SI cutoff
 traindf.v22 <- traindf.v22 %>%
   mutate(aaq_dum = ifelse(aaq_tot > rs_df_arranged[1,1], 1, 2)) %>%
   mutate(aaq_dum = case_when(aaq_dum == 1 ~ 1,
                              aaq_dum == 2 ~ 2)) %>%
   filter(!is.na(aaq_dum))

traindf.v22$aaq_dum <- as.factor(traindf.v22$aaq_dum)
table(traindf.v22$aaq_dum)
```

## Achievement
n = 594  ** too small
```{r achieve4}
table(traindf.v22$achieve4)
```
...
traindf_tsm <- traindf.v23 %>% select(persist, dep_secret, achieve4, aaq_dum)
...
traindf_tsm <- traindf.v23 %>% select(persist, anx_type, dep_secret, achieve4, aaq_dum)
...
results_tsm <- dummy_cols(traindf_tsm, select_columns = c("persist", "dep_secret", "achieve4", "aaq_dum"),
                         remove_first_dummy = TRUE)
...
## CLM TSM
```{r CLM TSM}
clm_tsm <- traindf.v23 %>% select(si_type, persist, dep_secret, aaq_dum)

clm_tsm <- NA_preproc(clm_tsm)
clm_tsm <- as.data.frame(clm_tsm)

clm_tsm <- clm(si_type ~ persist + dep_secret + aaq_dum,
               data = clm_tsm) 
...
traindf.v24 <- traindf.v24 %>%
  mutate(dep_impa = case_when(is.na(dep_impa) ~ 0,
                              dep_impa == 1 ~ 1,
                              dep_impa == 2 ~ 2,
                              dep_impa == 3 ~ 3,
                              dep_impa == 4 ~ 4))
traindf.v24$dep_impa <- as.numeric(traindf.v24$dep_impa)
table(traindf.v24$dep_impa)
...
```{r gad7_impa}
traindf.v24 <- traindf.v24 %>%
  mutate(gad7_impa = case_when(is.na(gad7_impa) ~ 0,
                               gad7_impa == 1 ~ 1,
                               gad7_impa == 2 ~ 2,
                               gad7_impa == 3 ~ 3,
                               gad7_impa == 4 ~ 4))
traindf.v24$gad7_impa <- as.numeric(traindf.v24$gad7_impa)
table(traindf.v24$gad7_impa)
```

## Current needs
```{r percneed_cur}
traindf.v24 <- traindf.v24 %>%
  mutate(percneed_cur = case_when(is.na(percneed_cur) ~ 0,
                                 percneed_cur == 1 ~ 1,
                                 percneed_cur == 2 ~ 2,
                                 percneed_cur == 3 ~ 3,
                                 percneed_cur == 4 ~ 4,
...
table(traindf.v24$percneed_cur)
...
traindf_entrap <- traindf.v25 %>% select(dep_impa, gad7_impa, percneed_cur)
...
### EFA Entrapment
```{r CFA entrap}
traindf_entrap <- traindf.v25 %>% select(dep_impa, gad7_impa, percneed_cur)   # all continuous... treat
...
```{r clm_entrap}
clm_entrap <- traindf.v25 %>% select(si_type, dep_impa, gad7_impa, percneed_cur)

clm_entrap <- NA_preproc(clm_entrap)
clm_entrap <- as.data.frame(clm_entrap)

clm_entrap <- clm(si_type ~ dep_impa + gad7_impa + percneed_cur,
                  data = clm_entrap)
...
```{r achieve_1_14_18}
table(traindf.v26$achieve1)
table(traindf.v26$achieve14)
table(traindf.v26$achieve18)

# sample too small in the elective, remove
traindf.v22 = subset(traindf.v22, select= -c(achieve1, achieve14, achieve18)) # clean up var
```
...
# deem 'rs_df_arranged[1,1]' as SI cutoff
traindf.v26 <- traindf.v26 %>%
  mutate(flourish_dum = ifelse(flourish > rs_df_arranged[1,1], 1, 2))  # 1 for over bin, # 2 for below bin

rs_df_arranged[1,1]
table(traindf.v26$flourish_dum)

traindf.v26 <- traindf.v26 %>%
  mutate(flourish_dum = case_when(flourish_dum == 1 ~ 1,
                                  flourish_dum == 2 ~ 2,
                                  is.na(flourish_dum) ~ 0))
table(traindf.v26$flourish_dum)
```
...
# deem 'rs_df_arranged[1,1]' as SI cutoff
traindf.v26 <- traindf.v26 %>%
  mutate(brs_dum = ifelse(brs_tot > rs_df_arranged[1,1], 1, 2)) %>%
...
rs_df_arranged[1,1]
table(traindf.v26$brs_tot)
table(traindf.v26$brs_dum)
```

## Friendship development
```{r friend_devel}
table(traindf.v26$friend_devel)

# sample too small in the elective, remove
traindf.v26 = subset(traindf.v26, select= -c(friend_devel)) # clean up var
```
...
traindf_mm <- traindf.v27 %>% select(inf, flourish_dum, belong1)
...
var_names <- c(
  ...
  # environment
  "satisfied_overall", "talk", "religios", "discrim", "school2_type", "activ_yn",
  # life events
  "ins_cover", "gpa_sr", "binge", "psyhx", "fincur", "drugs_yn", "assault",
  # defeat
  "aca_impa", "dep_type", "anx_type",
  # threats to self-moderation
  "persist", "dep_secret", "aaq_dum",
  # entrapment
...
traindf_sem2 <- traindf_sem %>% 
  # mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
view(traindf_sem2)   # check output

# # xlsx output
traindf_sem3 <- as.data.frame(traindf_sem2)
save(traindf_sem3 , file = "traindf_nondummy.RData")
load("traindf_nondummy.RData")
write_xlsx(traindf_sem3, path = "traindf_nondummy.xls", col_names = TRUE, format_headers = TRUE)
```
...
```{r}
# zeroinfl <- function(si_type, results_tr_sem2, na.action = TRUE
#                      dist = c("binomial"),
#                      link = c("logit"))
```
...
#   missing_rates[i, 2] = sum(!is.na(traindf.v28[[i]]))
#   missing_rates[i, 3] = missing_rates[i, 2] / nrow(traindf.v28)
# }
# # view matrix
# view(missing_rates)
# # save matrix
...
```{r model_lev_corr}
results_corr <- NA_preproc(results_tr_sem)
...
# rename SEM dummy cols
```{r rename_cols}
# results_tr_sem2 <- results_tr_sem
...
#   rename(age_25 = age_8) %>%
# 
#   rename(sexual_het = sexual_NA) %>%
#   rename(sexual_lgb = sexual_2) %>%
#   rename(sexual_qq = sexual_3) %>%
# 
#   rename(race_bla = race_NA) %>%
#   rename(race_ame = race_2) %>%
#   rename(race_asi = race_3) %>%
#   rename(race_his = race_4) %>%
#   rename(race_pac = race_5) %>%
#   rename(race_ara = race_6) %>%
#   rename(race_whi = race_7) %>%
#   rename(race_oth = race_8) %>%
#   rename(race_non = race_9) %>%
# 
#   rename(talk_no = talk_NA) %>%
#   rename(talk_for = talk_2) %>%
#   rename(talk_inf = talk_3) %>%
#   rename(talk_com = talk_4) %>%
# 
#   rename(sch_4yr = school2_type_NA) %>%
#   rename(sch_cc = school2_type_2) %>%
#   rename(sch_ad = school2_type_3)

# view(results_tr_sem2)
```
...
#               # measurement model
#                   si_sem =~  si_type
#                   dia_sem =~ age + sexual + sib_freq + race + ed_any + dx_dep + dx_bi + 
#                              dx_anx + dx_tr + dx_adhd + meds_count
#                   env_sem =~ divers + satisfied_overall + religios + school2_type + activ_yn
#                   le_sem =~  ins_cover + gpa_sr + binge + psyhx + fincur + drugs_yn + assault
#                   def_sem =~ aca_impa + dep_type
#                   tsm_sem =~ persist + anx_type + dep_secret
#                   entrap_sem =~ dep_impa + gad7_impa + percneed_cur
#                   mm_sem =~  inf + flourish_dum + belong1 + brs_dum
#                   
...
```{r SEM specification w/ dum}
SI_model_tr <- '
                # measurement model
                  si_sem =~  si_type_2 + si_type_3 + si_type_4

                  dia_sem =~ age_19 + age_20 + age_21 + age_22 + age_23 + age_24 + age_25 +
                             sexual_2 + sexual_3 + sexual_NA +
                             gender_tgd_2 +
                  ...
                  env_sem =~ satisfied_overall_2 + satisfied_overall_3 + satisfied_overall_4 +
                                satisfied_overall_5 + satisfied_overall_6 + satisfied_overall_7 +
                             talk_2 + talk_3 + talk_4 + talk_NA +
                             religios_2 + religios_3 + religios_4 + religios_5 +
                             discrim_2 + discrim_3 + discrim_4 + discrim_5 + discrim_6 + discrim_7 +
                             school2_type_2 + school2_type_3 + school2_type_NA +
                             activ_yn_1 + activ_yn_NA

                  le_sem =~  ins_cover_1 +
                             gpa_sr_1 + gpa_sr_2 + gpa_sr_3 + gpa_sr_4 +
                             binge_1 +
                             psyhx_1 + psyhx_NA +
                             fincur_2 + fincur_3 + fincur_4 + 
                             drugs_yn_1 +
                             assault_1 + assault_NA

                  def_sem =~ aca_impa_1 + aca_impa_2 + aca_impa_3 + aca_impa_4 + 
                             dep_type_1 + dep_type_2 + dep_type_3 + dep_type_4 + dep_type_5

                  tsm_sem =~ persist_1 + persist_2 + persist_3 + persist_4 + persist_5 + persist_6 +
                                anx_type_1 + anx_type_4 +
                             dep_secret_1 + dep_secret_2 + dep_secret_3 + dep_secret_4 + dep_secret_5 + dep_secret_6
                             
                  entrap_sem =~ dep_impa_1 + dep_impa_2 + dep_impa_3 + dep_impa_4 +
                                gad7_impa_1 + gad7_impa_2 + gad7_impa_3 + gad7_impa_4 +
                                percneed_cur_1 + percneed_cur_2 + percneed_cur_3 + percneed_cur_4 + 
                                    percneed_cur_5 + percneed_cur_6

                  mm_sem =~  inf_2 + inf_NA +
                             flourish_dum_1 + flourish_dum_2 +
...
results_tr_sem2 <- as.data.frame(results_corr)

is.atomic(results_tr_sem2)
is.recursive(results_tr_sem2)

fit_train <- sem(SI_model_tr, data=results_tr_sem2, 
                 ordered = "si_type_2", "si_type_3", "si_type_4",
...
fit_train2 <- update(fit_train, add="nox ~ log_crim")
...
table(testdf.v10$wcs_1_f)
table(testdf.v10$wcs_1_m)
table(testdf.v10$wcs_1_o)

table(testdf.v10$wcs_2)
table(testdf.v10$wcs_3)
table(testdf.v10$wcs_4)
table(testdf.v10$wcs_5)

testdf.v10 = subset(testdf.v10, select= -c(wcs_1_f, wcs_1_m, wcs_1_o, 
                                           wcs_2, wcs_3, wcs_4, wcs_5)) # clean up vars/columns
```
...
table(testdf.v11$meds_count)  # check output
...
```{r Env frame}
testdf.v14 <- testdf.v11
```

## Discrimination
```{r discrim}
table(testdf.v14$discrim)
testdf.v14$discrim <- as.factor(testdf.v14$discrim)
```
...
testdf.v14 <- testdf.v14 %>% 
  ...
  mutate(talk = case_when(((talk1_1 == 1 | talk1_6 == 1 | talk1_7 == 1 | talk1_8 == 1) &      # combined
                          ...
                          (talk1_1 == 1 | talk1_6 == 1 | talk1_7 == 1 | talk1_8 == 1) ~ 2,    # formal support
                          (talk1_2 == 1 | talk1_3 == 1 | talk1_4 == 1 | talk1_5 == 1) ~ 3))    # informal support
table(testdf.v14$talk)  # check output
testdf.v14$talk <- as.factor(testdf.v14$talk)


testdf.v14 = subset(testdf.v14, select= -c(talk1_1:talk1_9, talk1sig, talk1fr, talk1rel, talk1no, talk1sup, talk1rm, talk1pro, talk1fam)) # clean up
```

## Overall school satisfaction
```{r satisfied_overall}
table(testdf.v14$satisfied_overall)
testdf.v14$satisfied_overall <- as.factor(testdf.v14$satisfied_overall)
```
...
testdf.v14$activ_ac <- as.numeric(testdf.v14$activ_ac)
testdf.v14$activ_athc <- as.numeric(testdf.v14$activ_athc)
testdf.v14$activ_athv <- as.numeric(testdf.v14$activ_athv)
testdf.v14$activ_athi <- as.numeric(testdf.v14$activ_athi)
testdf.v14$activ_cs <- as.numeric(testdf.v14$activ_cs)
testdf.v14$activ_cu <- as.numeric(testdf.v14$activ_cu)
testdf.v14$activ_da <- as.numeric(testdf.v14$activ_da)
testdf.v14$activ_fs <- as.numeric(testdf.v14$activ_fs)
testdf.v14$activ_gs <- as.numeric(testdf.v14$activ_gs)
testdf.v14$activ_gov <- as.numeric(testdf.v14$activ_gov)
testdf.v14$activ_mp <- as.numeric(testdf.v14$activ_md)
testdf.v14$activ_md <- as.numeric(testdf.v14$activ_none)
testdf.v14$activ_soc <- as.numeric(testdf.v14$activ_soc)
testdf.v14$activ_rel <- as.numeric(testdf.v14$activ_rel)
testdf.v14$activ_art <- as.numeric(testdf.v14$activ_art)
testdf.v14$activ_other <- as.numeric(testdf.v14$activ_other)
...
table(testdf.v14$activ_yn)     # tabulate and check output
...
table(testdf.v16$ins_cover)   # check output
...
table(testdf.v16$drugs_yn)
```

## Current financial stressors
```{r fincur}
table(testdf.v16$fincur)
testdf.v16$fincur <- as.factor(testdf.v16$fincur)
```
...
testdf.v17 <- testdf.v16 %>%
  mutate(abuse_year = case_when(abuse_recent == 1 ~ 1,
                                ...
                                abuse_recent == 5 ~ 0,
                                abuse_life == 0 ~ 0))
testdf.v17$abuse_year <- as.numeric(testdf.v17$abuse_year)
...
# tabulate and check output
table(testdf.v17$assault_sex) 
table(testdf.v17$assault_emo)
table(testdf.v17$assault_phys)
```
...
table(testdf.v18$binge_fr) # check output
testdf.v18$binge_fr <- as.factor(testdf.v18$binge_fr)
...
table(testdf.v18$psyhx)   # check output
...
## Academic impact
```{r aca_impa}
table(testdf.v20$aca_impa)
testdf.v20$aca_impa <- as.factor(testdf.v20$aca_impa)
```
...
table(testdf.v22$anx_score)
testdf.v22$anx_score <- as.numeric(testdf.v22$anx_score)

# Convert to categories for later
testdf.v22 <- testdf.v22 %>%
  ...
  mutate(anx_sev = case_when(anx_score > 15 ~ 1,
                             anx_score < 16 ~ 0))
# Check outputs
table(testdf.v22$anx_none)
table(testdf.v22$anx_mild)
table(testdf.v22$anx_mod)
table(testdf.v22$anx_sev)
table(testdf.v22$anx_score)
```
...
```{r dep_secret}
## Secretive MH
table(testdf.v22$dep_secret)
testdf.v22$dep_secret <- as.factor(testdf.v22$dep_secret)
```
...
testdf.v22$aaq_tot <- as.numeric(testdf.v22$aaq_tot)
```

## Persist
```{r persist}
testdf.v22$persist <- as.factor(testdf.v22$persist)
table(testdf.v22$persist)
```
...
table(testdf.v24$dep_impa)
testdf.v24$dep_impa <- as.factor(testdf.v24$dep_impa)
```

## Anxiety impact
```{r gad7_impa}
table(testdf.v24$gad7_impa)
testdf.v24$gad7_impa <- as.factor(testdf.v24$gad7_impa)
```

## Current needs
```{r perneed_cur}
table(testdf.v24$percneed_cur)
testdf.v24$percneed_cur <- as.factor(testdf.v24$percneed_cur)
```
...
table(testdf.v26$belong1)
testdf.v26$belong1 <- as.factor(testdf.v26$belong1)
...
table(testdf.v26$inf)
testdf.v26$inf <- as.factor(testdf.v26$inf)

# clean up vars
testdf.v26 = subset(testdf.v26, select= -c(inf_1:inf_help, inf_sum))
```
...
table(testdf.v26$stig_pcv_2b)  # Most people feel that MH tx is a sign of personal failure.
table(testdf.v26$stig_pcv_3b)  # Most people think less of a person who has received MH tx.
table(testdf.v26$stig_per_2b)  # I feel that receiving MH tx is a sign of personal failure.
table(testdf.v26$stig_per_3b)  # I would think less of a person who has received MH tx.
...
testdf_sem <- testdf.v28 %>% select(
  ...
  si_type,
  # diathesis
  age, sexual, gender_tgd, sib_freq, race, ed_any, dx_dep, dx_bi, dx_anx, dx_tr, dx_adhd, meds_count,
  # environment
  divers, satisfied_overall, talk, religios, discrim, school2_type, activ_yn,
  # life events
  ins_cover, gpa_sr, binge_fr, psyhx, fincur, drugs_yn, assault_emo, assault_phys, assault_sex,
  # defeat
  aca_impa, deprawsc,
  # threats to self-moderation
  persist, anx_score, dep_secret,
  # entrapment
  dep_impa, gad7_impa, percneed_cur,
  # motivational moderators
  inf, flourish, belong1, mh_stigma, brs_tot)

# create sem matrix, with dummy vars 
testdf_sem2 <- testdf_sem %>% 
  mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
testdf_sem2   # check output

# xlsx output
testdf_sem3 <- as.data.frame(testdf_sem2)
save(testdf_sem3 , file = "testdf_nondummy.RData")
load("testdf_nondummy.RData")
write_xlsx(testdf_sem3, path = "testdf_nondummy.xls", col_names = TRUE, format_headers = TRUE)
```

## Descriptive stats
```{r}
# test data
var_desc_test <- describe(testdf_sem2) %>% view()
...
results_test_sem <- dummy_cols(testdf_sem2, select_columns = c(
    ...
    "si_type",
    # diathesis
    "age", "sexual", "gender_tgd", "sib_freq", "race", "ed_any", "dx_dep", "dx_bi", "dx_anx",
    "dx_tr", "dx_adhd", "meds_count",
    # environment
    "divers", "satisfied_overall", "talk", "religios", "discrim", "school2_type", "activ_yn",
    # life events
    "ins_cover", "gpa_sr", "binge_fr", "psyhx", "fincur", "drugs_yn", "assault",
    # defeat
    "aca_impa", "deprawsc",
    # threats to self-moderation
    "persist", "anx_score", "dep_secret",
    ...
    "dep_impa", "gad7_impa", "percneed_cur",
    # motivational moderators
    "inf", "flourish", "belong1", "brs_tot"), 
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)

view(results_test_sem)
```
...
SI_model_tr <- '
              # measurement model
                  si_sem =~  si_type
                  dia_sem =~ age + sexual + gender_tgd + sib_freq + race + ed_any + dx_dep +
                             dx_bi + dx_anx + dx_tr + dx_adhd + meds_count
                  env_sem =~ divers + satisfied_overall + talk + religios + discrim +
                             school2_type + activ_yn
                  le_sem =~  ins_cover + gpa_sr + binge_fr + psyhx + fincur + drugs_yn + 
                             assault_emo + assault_phys + assault_sex
                  def_sem =~ aca_impa + deprawsc
                  tsm_sem =~ persist + anx_score + dep_secret 
                  entrap_sem =~ dep_impa + gad7_impa + percneed_cur
                  mm_sem =~  inf + flourish + belong1 + mh_stigma + brs_tot
               ...
               # residual covariances
                  ...
                  dx_tr ~~ dx_dep
                  dx_bi ~~ dx_dep
                  assault_emo ~~ assault_sex
                  assault_phys ~~ assault_sex
                  assault_phys ~~ assault_emo     '
```

## Model estimation
```{r SEM model estimation}
fit <- cfa(SI_model_tr, data = results_test_sem,
             ordered = c("age","si_type", "sib_freq","meds_count", "binge_fr", "deprawsc",
                         "anx_score", "dep_impa", "gad7_impa", "flourish", "brs_tot"))

fit_train <- sem(SI_model_tr, data=results_test_sem)
