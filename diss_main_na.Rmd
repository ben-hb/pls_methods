...
```{r clm_dia}
clm_dia <- traindf.v12 %>% select(si_type, age, sexual, gender_tgd, finpast, sib_freq, race, body_sr, dx_anx,
                                dx_anx, dx_dep, dx_bi, dx_adhd, dx_tr, ed_any, meds_count)

clm_dia <- NA_preproc(clm_dia)
dia_mod <- as.data.frame(clm_dia)

clm_mod <- clm(si_type ~ age + sexual + gender_tgd + finpast + sib_freq + body_sr + dx_tr + race + dx_anx +
                        ed_any + dx_anx + dx_dep + dx_bi + dx_adhd + meds_count,
               data = dia_mod)
summary(clm_mod)
coef(clm_mod)
...
results_env <- dummy_cols(traindf_env, select_columns = c("discrim", "religios", "env_mh", "residenc", "ther_vis",
...
```{r CLM environment}
clm_env <- traindf.v15 %>% select(si_type, discrim, religios, env_mh, residenc, ther_vis, activ_yn, religios,
...
# clm_env2 <- glm(si_type ~ discrim + satisfied_overall + ther_vis + talk + activ_yn + international + religios +
...
traindf.v15 = subset(traindf.v15, select= -c(ins_1, ins_mh, ins_ade, ins_inade)) # clean up var
```
