...
# create sem matrix 
traindf_sem3 <- traindf_sem3 %>% 
  # mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
traindf_sem3 <- na.omit(traindf_sem3)
head(traindf_sem3)   # check output
```
...
results_tr_com3 <- dummy_cols(traindf_sem3, select_columns = c("si_type", "sexual", "inf", "race", "talk", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)

fit_train2 <- sem(SI_model_tr2, data=results_tr_com3)
                  # ordered = c("si_type", "sib_freq", "satisfied_overall", "religios",
                  #             "gpa_sr", "aca_impa", "persist",
                  #             "flourish_dum", "belong1", "dep_impa", "gad7_impa"))
summary_train <- summary(fit_train2, fit.measures=TRUE, standardized = TRUE)
...
## model diagnostics
fitmeasures(fit_train2)
fitted(fit_train2)
inspect(fit_train2, what="cor.all")

fitMeasures(fit_train2, c("cfi","chisq", "df", "pvalue","rmsea","srmr"), output = "matrix")
print(fitMeasures(fit_train2, c("chisq", "df", "pvalue", "cfi", "rmsea"),
                  output = "text"), add.h0 = TRUE)

## running NULL model, baseline model's RMSEA = 0.1107675 
nullRMSEA(fit_train2, scaled = FALSE, silent = FALSE)

## compare to observed
lavCor(fit_train2)

## residuals in coef unit
resid(fit_train2, "cor")

## modification indices
modificationindices(fit_train2, minimum.value = 20)

mod_fit2 <- cfa(SI_model_tr2, data = results_tr_com3)
mi <- modindices(mod_fit2, sort = TRUE, maximum.number = 10)
View(mi)
mi[mi$op == "=~",] %>% View()

# MODEL ESTIMATES

semPaths(fit_train2, what = "col", whatLabels = "std", style = "mx", rotation = 1, layout = "spring", 
    nCharNodes = 8, shapeMan = "rectangle", sizeMan = 8, sizeMan2 = 8)

parameterEstimates(fit_train2)
resid(fit_train2)
```
...
fit_train_M <- sem(SI_model_tr3, data=results_tr_com3)
summary_mod_train <- summary(fit_train_M, fit.measures=TRUE, standardized = TRUE)

mod_fit1 <- cfa(SI_model_tr3, data = results_tr_com3)
