...
fit_train1 <- sem(SI_model_tr1, data=results_tr_com1)
                  # ordered = c("si_type", "sib_freq", "satisfied_overall", "religios", "dep_impa",
                  #             "gpa_sr", "aca_impa", "persist", "flourish_dum", "belong1", "gad7_impa"))
...
summary_mod_train <- summary(fit_train_M, fit.measures=TRUE, standardized = TRUE)
    ...
    ## residuals in coef unit
resid(fit_train_M, "cor")

    ## modification indices
modificationindices(fit_train_M, minimum.value = 20)

mod_fit1 <- cfa(fit_train_M, data = results_tr_com1)
mi <- modindices(mod_fit1, sort = TRUE, maximum.number = 10)
View(mi)
mi[mi$op == "=~",] %>% View()

# MODEL ESTIMATES
semPaths(fit_train_M, what = "col", whatLabels = "std", style = "mx", rotation = 1, layout = "spring", 
    nCharNodes = 8, shapeMan = "rectangle", sizeMan = 8, sizeMan2 = 8)
...
fit_train_d <- sem(SI_model_trD, data=results_tr_df1)

summary_train <- summary(fit_train_d, fit.measures=TRUE, standardized = TRUE)

mod_fit_D <- cfa(SI_model_trD, data = results_tr_df1)
