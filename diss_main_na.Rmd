...
model.0 <- lm(Y ~ X, results_tr_com2)
summary(model.0)
# model.0 <- lm(Y ~ X, results_tr_com2)
# summary(model.0)
# model.0 <- lm(Y ~ X, results_tr_com2)
...
model.M <- lm(M ~ X, (model.0))
model.Y <- lm(Y ~ X + M, (model.0))
results <- mediate(model.M, model.Y, treat='X', mediator='M',
                   boot=TRUE, sims=100)
summary(results)
...
# TSM as a moderator or mediator?
  ## entrap ~ def
model.X1 <- lm(entrap ~ def, results_tr_com2)
summary(model.0)
  ## def ~ tsm
model.M1 <- lm(tsm ~ def, results_tr_com2)
summary(model.M)
  ## entrap ~ def + tsm
model.Y1 <- lm(entrap ~ def + tsm, results_tr_com2)
summary(model.Y1)

med_results1 <- mediate(model.M1, model.Y1, treat='def', mediator='tsm',
                   boot=TRUE, sims=500)
summary(med_results1)

# MM as a moderator or mediator?
  ## entrap ~ def
model.X2 <- lm(si_type ~ entrap, results_tr_com2)
summary(model.0)
  ## def ~ tsm
model.M2 <- lm(mm ~ entrap, results_tr_com2)
summary(model.M)
  ## entrap ~ def + tsm
model.Y2 <- lm(si_type ~ entrap + mm, results_tr_com2)
summary(model.Y1)

med_results2 <- mediate(model.M2, model.Y2, treat='X', mediator='M',
                   boot=TRUE, sims=500)
