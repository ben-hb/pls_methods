...
SI_model_tr <- ' # measurement model
                  ...
                  dia_sem =~ age + sexual + gender_tgd + sib_freq + race + 
                             ed_any + dx_mh + meds_count

                  env_sem =~ satisfied_overall + talk + religios + school2_type +
                             + activ_yn

                  le_sem =~  ins_cover + gpa_sr + binge + psyhx + fincur + 
                             drugs_yn + drug_mar + assault
               ...
               # structural model - regressions
                  # premotivational phase
                    def_sem ~ dia_sem + env_sem + le_sem + dia_sem*env_sem + env_sem*le_sem + le_sem*dia_sem
                  
                  # motivational phase
                    si_type ~ def_sem + tsm_sem + def_sem*tsm_sem + entrap_sem + mm_sem + entrap_sem*mm_sem  
...
## Model estimation (non-dummy)
```{r SEM model estimation}
traindf_sem3 <- as.matrix(traindf_sem2)
traindf_sem3 <- na.omit(traindf_sem3)

fit_train <- sem(SI_model_tr, data=traindf_sem3, estimator = "GLS")
                 # ordered =c("si_type", "age", "sib_freq", "meds_count", "satisfied_overall",
                 # "religios", "gpa_sr", "aca_impa", "dep_type", "anx_type", "persist",
                 # "dep_impa", "gad7_impa", "flourish_dum", "belong1"))
...
SI_model_tr_dummy <- '
                # measurement model
                  si_sem =~  si_type_2 + si_type_ + si_type_4
                  ...
                  env_sem =~ satisfied_overall_2 + satisfied_overall_3 + satisfied_overall_4 +
                   ...
                   le_sem =~  ins_cover_1 + gpa_sr_2 + gpa_sr_3 + gpa_sr_4 +  gpa_sr_5 + 
                   ...
                   tsm_sem =~ persist_2 + persist_3 + persist_4 + persist_5 + persist_6 + aaq_dum_1

                   entrap_sem =~ dep_impa_2 + dep_impa_3 + dep_impa_4 +
                                 gad7_impa_2 + gad7_impa_3 + gad7_impa_4

                   mm_sem =~  inf_1 +
                              flourish_dum_2 + flourish_dum_3 + flourish_dum_4 + flourish_dum_5 +
                              belong1_2 + belong1_3 + belong1_4 + belong1_5 + belong1_6

                # structural model - regressions
                   def_sem ~ dia_sem + env_sem + le_sem + dia_sem*env_sem + env_sem*le_sem + le_sem*dia_sem
                   si_sem ~ entrap_sem + mm_sem + entrap_sem*mm_sem + def_sem + tsm_sem + def_sem*tsm_sem
...
fit_train <- sem(SI_model_tr_dummy, data=results_tr_sem3, estimator = "GLS")
summary_train <- summary(fit_train, fit.measures=TRUE, standardized = TRUE)
```
