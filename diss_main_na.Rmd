...
## Create dummy vars
```{r dummy_vars}
results_tr_sem1 <- dummy_cols(traindf_sem2, select_columns = c(
    # outcome var
    "si_type",
    # diathesis
    "age", "sexual", "gender_tgd", "sib_freq", "race", "ed_any", "dx_mh", "meds_count",
...
results_tr_com1 <- dummy_cols(traindf_sem1, select_columns = c("si_type", "sexual", "inf", "race", "talk", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)

fit_train1 <- sem(SI_model_tr1, data=results_tr_com1)
...
## model diagnostics
fitmeasures(fit_train1)
fitted(fit_train1)
inspect(fit_train1, what="cor.all")
parameterEstimates(fit_train1)

# fitMeasures(fit_train1, c("cfi","chisq", "df", "pvalue","rmsea","srmr"), output = "matrix")
# print(fitMeasures(fit_train1, c("chisq", "df", "pvalue", "cfi", "rmsea"),
...
resid(fit_train1, "cor")

## modification indices
modificationindices(fit_train1, minimum.value = 20)

mod_fit1 <- cfa(SI_model_tr1, data = results_tr_com1)
mi <- modindices(mod_fit1, sort = TRUE, maximum.number = 10)
View(mi)
mi[mi$op == "=~",] %>% View()

# MODEL ESTIMATES
semPaths(fit_train1, what = "col", whatLabels = "std", style = "mx", rotation = 1, layout = "spring", 
...
SI_model_tr_M <- '
                # measurement model
                   ...
                   tsm =~ persist + aaq_dum
                   entrap =~ dep_impa + gad7_impa
                   mm =~ inf_1 + flourish_dum + belong1 + satisfied_overall
...
fit_train_M <- sem(SI_model_tr_M, data=results_tr_com1)
summary_mod_train <- summary(fit_train_M, fit.measures=TRUE, standardized = TRUE)

fitMeasures(fit_train_M, c("cfi","chisq", "df", "pvalue","rmsea","srmr"), output = "matrix")

print(fitMeasures(fit_train_M, c("chisq", "df", "pvalue", "cfi", "rmsea"),
                  output = "text"), add.h0 = TRUE)
```
...
```{r SEM, all dummies}
## Dummy col SEM w/ continuous
SI_model_trD <- '# measurement model
                  si =~  si_type_2 + si_type_3 + si_type_4
                  ...
                  prem  =~ age_19 + age_20 + age_21 + age_22 + age_23 + age_24 + age_25 +
                           ...
                           meds_count_1 + meds_count_2 + meds_count_3 + talk_2 + talk_3 + talk_4 + religios_2 + 
                           religios_3 + religios_4 + religios_5 + school2_type_2 + school2_type_3 + 
                           activ_yn_1 + ins_cover_1 + gpa_sr_2 + gpa_sr_3 + gpa_sr_4 + gpa_sr_5 +
                           + binge_1 + psyhx_1 + fincur_1 + drugs_yn_1 + assault_1 + drug_mar_1   

                   def =~ aca_impa_2 + aca_impa_3 + aca_impa_4 + dep_type_2 + dep_type_3 + 
                           dep_type_4 + dep_type_5 + anx_type_2 + anx_type_3 + anx_type_4

                   tsm =~ persist_2 + persist_3 + persist_4 + persist_5 + persist_6 + aaq_dum_1

                   entrap =~ dep_impa_2 + dep_impa_3 + dep_impa_4 + gad7_impa_2 + 
                             gad7_impa_3 + gad7_impa_4

                   mm =~  inf_1 + flourish_dum_2 + flourish_dum_3 + flourish_dum_4 +
                          flourish_dum_5 + belong1_2 + belong1_3 + belong1_4 + belong1_5 + belong1_6 +
                          satisfied_overall_2 + satisfied_overall_3 + satisfied_overall_4 + 
                          satisfied_overall_5 + satisfied_overall_6 
                ...
                # residual covars
                  ...
                  assault_1 ~~ dx_mh_1
                  drugs_yn_1 ~~ drug_mar _1

                  sexual_3 ~~ gender_tgd_1
                  talk_3 ~~ talk_2
                  talk_2 ~~ talk_4
                  talk_3 ~~ talk_4
                  race_2 ~~ race_4
...
view(results_tr_sem1)

fit_train_d <- sem(SI_model_trD, data=results_tr_sem1)
summary_train <- summary(fit_train_d, fit.measures=TRUE, standardized = TRUE)

mod_fit_D <- cfa(SI_model_trD, data = results_tr_sem1)
mi <- modindices(mod_fit_D, sort = TRUE, maximum.number = 10)
View(mi)
mi[mi$op == "=~",] %>% View()

fitMeasures(mod_fit_D, c("cfi","chisq", "df", "pvalue","rmsea","srmr"), output = "matrix")

print(fitMeasures(mod_fit_D, c("chisq", "df", "pvalue", "cfi", "rmsea"),
                  output = "text"), add.h0 = TRUE)
```
...
# SEM model -- mod indices, all dummies
```{r si_model_tr2}
## Dummy col SEM w/ continuous
view(traindf_sem5)
...
SI_model_tr3 <- '
                # measurement model
                   ...
                   prem  =~ age_19 + age_20 + age_21 + age_22 + age_23 + age_24 + age_25 +
                            sexual_2 + sexual_3 + gender_tgd + sib_freq + 
                            race_2 + race_3 + race_4 + race_5 + ed_any + dx_mh +
                            meds_count + talk_2 + talk_3 + talk_4 + religios + 
                            school2_type_2 + school2_type_3 + activ_yn +
                            ins_cover + gpa_sr + binge + psyhx + fincur + drugs_yn + assault + drug_mar         
...
fit_train4 <- sem(SI_model_tr3, data=results_tr_com)
...
colnames(imv_path) = rownames(imv_path) # rename path
