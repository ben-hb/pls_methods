...
```{r residenc}
traindf.v13 <- traindf.v13 %>%
  mutate(residenc = case_when(residenc == 1 ~ 1,
                              residenc == 2 ~ 1,
                              residenc == 3 ~ 2, 
                              residenc == 4 ~ 2, 
                              residenc == 5 ~ 3,
...
jarque.bera.test(traindf.v14$satisfied_overall)
```
...
traindf_prem <- traindf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       meds_count, residenc, religios, activ_yn, school2_type, ins_cover, 
                                       gpa_sr, fincur, assault, binge, drugs_yn, drug_mar, psyhx)
...
traindf_sem1 <- traindf.v28 %>% select(
  ...
  age, sexual, gender_tgd, sib_freq, ed_any, dx_mh, meds_count,
  # environment
  satisfied_overall, religios, school2_type, activ_yn, international,
  # life events
  ins_cover, gpa_sr, binge, psyhx, fincur, drugs_yn, assault, drug_mar,
  # defeat
...
results_tr_com1 <- dummy_cols(traindf_sem1, select_columns = c("si_type", "age", "sexual", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
SI_model_tr1 <- '
                # measurement model
                  ...
                  prem  =~ age_2 + age_3 +
                           sexual_2 + sexual_3 + gender_tgd + sib_freq + ed_any + dx_mh +
                           meds_count + religios + international +
...
SI_model_tr_M <- '
                # measurement model
                   ...
                   prem =~ age_2 + age_3 + sexual_2 + sexual_3 + gender_tgd + sib_freq + 
                           international + ed_any + dx_mh +
                           meds_count + religios + school2_type_2 + school2_type_3 + activ_yn +
                           ins_cover + gpa_sr + binge + psyhx + fincur + drugs_yn + assault + drug_mar
...
fit_train_M <- sem(SI_model_tr_M, data=results_tr_com1, ordered=T)
...
results_test_sem <- na.omit(results_test_sem)
results_test_df <- as.data.frame(results_test_sem)
head(results_test_df)
```
...
results_test_com1 <- dummy_cols(testdf_sem5, select_columns = c("si_type", "age", "sexual", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
view(results_test_com1)

SI_model_test1 <- '
                # measurement model
                  si =~    si_type_2 + si_type_3 + si_type_4
    
                  prem  =~ age_2 + age_3 + sexual_2 + sexual_3 + gender_tgd + 
                           sib_freq + ed_any + dx_mh + meds_count + religios + 
                           drug_mar + international + school2_type_2 + school2_type_3 + 
                           activ_yn + assault +  ins_cover + gpa_sr + binge + psyhx + 
                           fincur + drugs_yn
...
summary_train <- summary(fit_test1, fit.measures=TRUE, standardized = TRUE)

## running NULL model, baseline model's RMSEA 
nullRMSEA(fit_test1, scaled = FALSE, silent = FALSE)

# Matrix math

lavaan_cov <- cov(results_test_com1)
lavaan_det <- det(lavaan_cov)
lavaan_eig <- eigen(lavaan_cov)
eig_var_prop <- round(lavaan_eig$values / sum(lavaan_eig$values), 3)
eig_var_cum_prop <- round(cumsum(lavaan_eig$values) / sum(lavaan_eig$values), 3)
...
rankifremoved <- sapply(1:ncol(results_test_com1), function (x) qr(results_test_com1[, -x])$rank)
which(rankifremoved == max(rankifremoved))

corr_matrix <- stats::cor(results_test_com1)
diag(corr_matrix) <- 0
which(corr_matrix == max(corr_matrix), arr.ind = TRUE)
```
...
SI_model_test_M <- '
                # measurement model
                   si =~  si_type_2 + si_type_3 + si_type_4
                   prem  =~ age_2 + age_3 + sexual_2 + sexual_3 + gender_tgd + sib_freq + 
                           ed_any + dx_mh + meds_count + religios + school2_type_2 + school2_type_3 + 
                           activ_yn + ins_cover + gpa_sr + binge + psyhx + fincur + drugs_yn + 
                           assault + drug_mar + international
                   def =~ aca_impa + dep_type + anx_type
                   tsm =~ persist + aaq_dum
                   entrap =~ dep_impa + gad7_impa
...
# matrix_dummied <- results_tr_com1 %>% as.matrix()
# matrix_dummied <- matrix_dummied[, c("race_2", "race_4")]
# boot::corr(matrix_dummied)
```
