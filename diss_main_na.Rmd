...
```{r CLM environment}
clm_env <- traindf.v15 %>% select(si_type, discrim, religios, residenc, activ_yn, divers, 
                                  school2_type, satisfied_overall, talk)
...
clm_env <- clm_env %>% 
 ...
 clm_env2 <- clm(si_type ~ discrim + religios + residenc + activ_yn + divers + 
                   school2_type + satisfied_overall + talk, 
                 data = clm_env)
 summary(clm_env2)
...
# ci_env
# 
# model.env <- tidy(clm_env)  # Convert model to dataframe for easy manipulation
# model.env
# model.env  %>%
#    mutate(or = exp(estimate),  # Odds ratio/gradient
...
traindf.v24 <- traindf.v24 %>%
  mutate(gad7_impa = case_when(gad7_impa == 1 ~ 1,
                               ...
                               gad7_impa == 4 ~ 4,
                               is.na(gad7_impa) ~ 5))
raindf.v24$gad7_impa <- as.numeric(traindf.v24$gad7_impa)
...
```{r percneed_cur}
traindf.v24 <- traindf.v24 %>%
  mutate(percneed_cur = case_when(is.na(percneed_cur) ~ 7,
                                 percneed_cur == 1 ~ 1,
                                 percneed_cur == 2 ~ 2,
                                 percneed_cur == 3 ~ 3,
                                 percneed_cur == 4 ~ 4,
...
traindf.v26$inf <- as.factor(traindf.v26$inf)
...
traindf_mm <- traindf.v27 %>% select(inf, flourish_dum, belong1, brs_dum)
...
results_mm <- dummy_cols(traindf_mm, select_columns = c("inf", "flourish_dum", "belong1", "brs_dum"),
                         remove_first_dummy = TRUE)
...
```{r CLM mm}
clm_mm <- traindf.v27 %>% select(si_type, inf, flourish_dum, belong1, brs_tot)

clm_mm <- NA_preproc(clm_mm)
clm_mm <- as.data.frame(clm_mm)

clm_mm <- clm(si_type ~ inf + flourish_dum + belong1 + brs_tot, data = clm_mm)
...
var_names <- c(
  ...
  # motivational moderators
  "inf", "flourish_dum", "belong1", "brs_dum")  
...
traindf_sem <- traindf.v28 %>% select(
  ...
  dep_impa, gad7_impa, percneed_cur,
  # motivational moderators
  inf, flourish_dum, belong1, brs_dum)  #mh_stigma, 
