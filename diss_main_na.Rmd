...
## EFA LE
```{r le_pca}
traindf_le <- traindf.v19 %>% select(ins_cover, gpa_sr, aca_impa, fincur, binge, drugs_yn,
                                     assault, psyhx)
...
results_le <- dummy_cols(traindf_le, select_columns = c("ins_cover", "gpa_sr", "aca_impa", 
                                                        "fincur", "binge","drugs_yn", "assault", "psyhx"),
                         remove_first_dummy = TRUE)
...
traindf_def <- traindf.v21 %>% select(aca_impa, dep_type)
...
results_def <- dummy_cols(traindf_def, select_columns = c("aca_impa", "dep_type"),
                         remove_first_dummy = TRUE)
...
## CLM defeat
```{r CLM defeat}
results2_def <- traindf.v21 %>% select(si_type, aca_impa, dep_type)
...
## GAD7 total
```{r anx_score}
table(traindf.v22$anx_score)    # PROBLEMSSSSSS
...
results_tsm <- dummy_cols(traindf_tsm, select_columns = c("persist", "anx_type", "dep_secret", "achieve4", "aaq_dum"),
                         remove_first_dummy = TRUE)
...
## CLM TSM
```{r CLM TSM}
clm_tsm <- traindf.v23 %>% select(si_type, persist, anx_type, dep_secret, aaq_dum)

clm_tsm <- NA_preproc(clm_tsm)
clm_tsm <- as.data.frame(clm_tsm)

clm_tsm <- clm(si_type ~ persist + anx_type + dep_secret + aaq_dum,
               data = clm_tsm) 
...
```{r CLM mm}
clm_mm <- traindf.v27 %>% select(si_type, inf, flourish_dum, belong1, mh_stigma, brs_tot)

clm_mm <- NA_preproc(clm_mm)
clm_mm <- as.data.frame(clm_mm)

clm_mm <- clm(si_type ~ inf + flourish_dum + belong1 + mh_stigma + brs_tot, data = clm_mm)
...
SI_model_tr <- '
                # measurement model
                  ...
                  dia_sem =~ age_19 + age_20 + age_21 + age_22 + age_23 + age_24 + age_25 +
                             sexual_2 + sexual_3 + sexual_hNA +
                             gender_tgd_2 +
                             sib_freq_2 + sib_freq_3 + sib_freq_4 + sib_freq_5 + sib_freq_6 + 
...
fit_train <- sem(SI_model_tr, data=results_tr_sem2,          
                 ordered = c( "si_type_2", "si_type_3", "si_type_4",
                       ...
                       "dep_type,
                      "anx_type,
                      "aaq_dum_2", "aaq_dum_NA",
                      "dep_impa_2", "dep_impa_3", "dep_impa_4", "dep_impa_NA",
                      "gad7_impa_2", "gad7_impa_3", "gad7_impa_4", "gad7_impa_NA",
                      "flourish_dum_1", "flourish_dum_NA",
                      "brs_dum_2", "brs_dum_NA"))
