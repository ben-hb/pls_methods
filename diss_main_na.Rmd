...
test1.v24 <- test1.v24 %>%
  mutate(percneed_cur = case_when(percneed_cur == 1 ~ 1,
                                 ...
                                 percneed_cur == 5 ~ 5,
                                 percneed_cur == 6 ~ 6))
test1.v24$percneed_cur <- as.numeric(test1.v24$percneed_cur)
table(test1.v24$percneed_cur, useNA = "always")
```
...
#create skeleton of missingness matrix
var <- count(test1.v28, test1.v28[var_names[k]])
var_ncol <- ncol(var)
...
for (j in 1:(length(var_names))) {
for (i in 1:nrow(var)) {   # i corresponds to rows, j to columns
  var[i, 2 + j] <- sum(!is.na(test1.v28[test1.v28[[var_names[k]]] == var[[i,1]], var_names[j]]))
}
}
# add percentage in each column
var_m <- as.matrix(var[-1]) 
...
fit_test1 <- sem(SI_model_tr1, data=results_test_com1, ordered=T)
...
```{r si_model_tr2}
## Dummy col SEM w/ continuous
results_tr_com1 <- dummy_cols(test1_sem1, select_columns = c("si_type", "sexual", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
#view(results_tr_com1)
