...
results_tr_com <- dummy_cols(traindf_sem5, select_columns = c("si_type", "sexual", "inf", "anx_type", "dep_type",
                                                              "race", "talk", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
results_tr_com2 <- cbind(traindf_sem5, results_tr_com)
```
...
## DIATHESIS ANALYSES
```{r Diathesis analyses}
testdf.v12 <- testdf.v11 # create new dataframe for summary stats. Will jump to v13 for environmental
```
...
# Define diathesis vars for correlation matrix
testdf_dia <- testdf.v12 %>% select(age, sexual, gender_tgd, finpast, sib_freq, race, ed_scoff, body_sr,
                                dx_mh, meds_count, ed_any, meds_sti, meds_dep, meds_psy, meds_anx, meds_mood,
                                meds_sle, meds_oth)

# Corr_simple function
corr_simple <- function(testdf=testdf_dia, sig=0.6){
  # convert testdf to numeric in order to run correlations 
  testdf_dia <- testdf_dia %>% mutate_if(is.character, as.factor)
  testdf_dia <-  testdf_dia %>% mutate_if(is.factor, as.numeric)
  # run a correlation and drop the insignificant ones
  corr <- cor(testdf_dia, use = "pairwise.complete.obs")                               
  corr[lower.tri(corr,diag=TRUE)] <- NA                   # prepare to drop duplicates and correlations of 1    
  corr[corr %in% c(-1, 1)] <- NA                          # drop perfect correlations
  corr <- as.data.frame(as.table(corr))                   # turn into a 3-column table
  corr <- na.omit(corr)                                   # remove the NA values from above 
...
# Extracting overlapping vars to visualize correlation for regression
testdf_dia <- testdf.v12 %>% select(age, sexual, gender_tgd, finpast, sib_freq, race, 
                                      body_sr, ed_any, dx_mh, meds_count, meds_sti, meds_dep, 
                                      meds_psy, meds_anx, meds_mood, meds_sle, meds_oth)
...
testdf_dia <- na.omit(testdf_dia)

# create corr matrix, with dummy vars
testdf_dia <- testdf_dia %>% 
  mutate_if(is.numeric, as.factor) %>% 
  mutate(
    age = as.numeric(age),
    meds_count = as.numeric(meds_count),
  ...
  dummy_cols(remove_first_dummy = TRUE) %>% 
  data.matrix()
testdf_dia   # check output

results_dia <- dummy_cols(testdf_dia, select_columns = c("age", "sexual", "gender_tgd", "finpast", 
                                                       "sib_freq", "race", "dx_mh", "ed_any",
                                                       "meds_count"), 
                          remove_first_dummy = TRUE)

# factoextra -- https://rpkgs.testdfnovia.com/factoextra/
dia_pca <- PCA(results_dia, graph=FALSE)  # apply PCA
eigval_dia <- get_eigenvalue(dia_pca)
eigval_dia
...
# Clean-up school2 var
testdf.v14 = subset(testdf.v14, select= -c(var_Boston, var_CalPoly, var_ColoradoCollege, var_ColoradoMountain,
                                       var_Delaware, var_DetroitMercy, var_Emerson, var_GeorgeMason,
                                       var_GeorgiaTech, var_Kalamazoo, var_Kansas, var_Merrimack, 
                                       var_MichiganState, var_MontclairState, var_NevadaReno,
                                       var_PennCollegeTech, var_RhodeIsland, var_Rollins, var_Sewanee, var_Smith,
...
```{r}
# Define environment vars for correlation matrix
testdf_env <- testdf.v15 %>% select(discrim, religios, residenc, activ_yn, divers, 
                                school2_type, satisfied_overall, talk, military, international)

 # Convert testdf to numeric in order to run correlations
testdf_env <- testdf_env %>% mutate_if(is.character, as.factor)
testdf_env <-  testdf_env %>% mutate_if(is.factor, as.numeric)
testdf_env <-  testdf_env %>% mutate_if(is.logical, as.numeric)

# run a correlation and drop the insignificant ones
corr <- cor(testdf_env, use = "pairwise.complete.obs")                               
corr[lower.tri(corr,diag=TRUE)] <- NA                   # prepare to drop duplicates and correlations of 1    
corr[corr %in% c(-1, 1)] <- NA                          # drop perfect correlations
corr <- as.data.frame(as.table(corr))                   #turn into a 3-column table
corr <- na.omit(corr)                                   #remove the NA values from above 
...
##EFA Environment
```{r EFA Environment}
testdf_env <- testdf.v15 %>% select(religios, residenc, activ_yn, school2_type, 
                                      satisfied_overall, talk)

testdf_env <- na.omit(testdf_env)
# create corr matrix, with dummy vars
testdf_env <- testdf_env %>% 
  mutate_if(is.numeric, as.factor) %>% 
  mutate(
    age = as.numeric(satisfied_overall)) %>%
  dummy_cols(remove_first_dummy = TRUE) %>% 
  data.matrix()
testdf_env   # check output

# PCA map with eigenvalues
results_env <- dummy_cols(testdf_env, select_columns = c("religios", "residenc", "activ_yn",
                                                         "school2_type", "satisfied_overall", "talk"),
                          remove_first_dummy = TRUE)
...
```{r LE correlation}
# Define life event var candidates for correlation matrix
testdf_le <- testdf.v19 %>% select(ins_cover, gpa_sr, aca_impa, fincur, binge,
                                     assault, drugs_yn, drug_mar, psyhx)
...
## EFA LE
```{r le_pca}
testdf_le <- testdf.v19 %>% select(ins_cover, gpa_sr, aca_impa, fincur, 
                                     binge, drugs_yn, drug_mar, assault, psyhx)
...
results_le <- dummy_cols(testdf_le, select_columns = c("ins_cover", "gpa_sr", "aca_impa", "fincur",
                                                        "binge","drugs_yn", "drug_mar", "assault", "psyhx"),
                         remove_first_dummy = TRUE)
...
```{r Defeat cor}
# Define defeat variable candidates for correlation matrix
testdf_def <- testdf.v21 %>% select(aca_impa, dep_type, anx_type)
...
results_def <- dummy_cols(testdf_def, select_columns = c("aca_impa", "dep_type", "anx_type"),
                         remove_first_dummy = TRUE)
...
```{r CORR TSM}
# Define TSM variable candidates for correlation matrix
testdf_tsm <- testdf.v23 %>% select(persist, aaq_dum)

# convert data to numeric in order to run correlations 
testdf_tsm <- testdf_tsm %>% mutate_if(is.character, as.factor)
testdf_tsm <-  testdf_tsm %>% mutate_if(is.factor, as.numeric)

# run a correlation and drop the insignificant ones
corr <- cor(testdf_tsm, use = "pairwise.complete.obs")                               
corr[lower.tri(corr,diag=TRUE)] <- NA                   # prepare to drop duplicates and correlations of 1    
corr[corr %in% c(-1, 1)] <- NA                          # drop perfect correlations
corr <- as.data.frame(as.table(corr))                   #turn into a 3-column table
corr <- na.omit(corr)                                   #remove the NA values from above 
...
```{r EFA TSM}
testdf_tsm <- testdf.v23 %>% select(persist, aaq_dum)
testdf_tsm <- na.omit(testdf_tsm)

# create corr matrix, with dummy vars 
testdf_tsm <- testdf_tsm %>% 
  mutate_if(is.numeric, as.factor) %>% 
  dummy_cols(remove_first_dummy = TRUE) %>% 
  data.matrix()
testdf_tsm   # check output

results_tsm <- dummy_cols(testdf_tsm, select_columns = c("persist", "aaq_dum"),
                         remove_first_dummy = TRUE)
...
```{r Entrapment corr}
# Define entrapment variable candidates for correlation matrix
testdf_entrap <- testdf.v25 %>% select(dep_impa, gad7_impa)

 # convert data to numeric in order to run correlations
testdf_entrap <- testdf_entrap %>% mutate_if(is.character, as.factor)
testdf_entrap <-  testdf_entrap %>% mutate_if(is.factor, as.numeric)

# run a correlation and drop the insignificant ones
corr <- cor(testdf_entrap, use = "pairwise.complete.obs")                               
corr[lower.tri(corr,diag=TRUE)] <- NA                   # prepare to drop duplicates and correlations of 1    
corr[corr %in% c(-1, 1)] <- NA                          # drop perfect correlations
corr <- as.data.frame(as.table(corr))                   #turn into a 3-column table
corr <- na.omit(corr)                                   #remove the NA values from above 
...
```{r CFA entrap}
testdf_entrap <- testdf.v25 %>% select(dep_impa, gad7_impa)
testdf_entrap <- na.omit(testdf_entrap)

# create corr matrix, with dummy vars 
results_entrap <- testdf_entrap %>% 
  mutate_if(is.numeric, as.factor) %>% 
  mutate_if(is.character, as.factor) %>% 
  data.matrix()
results_entrap   # check output
...
#                              medther = 2 ~ 0,
#                               is.na(medther) ~0))
# testdf.v26$medther <- as.logical(testdf.v26$medther)
table(testdf.v26$medther)  # med or therapy

testdf.v26$medtherc <- as.logical(testdf.v26$medtherc)
table(testdf.v26$medtherc) # meds + therapy
```
...
table(testdf.v26$brs_tot)
testdf.v26$brs_tot <- as.numeric(testdf.v26$brs_tot)

table(is.na(testdf.v26$brs_tot))

#  for loop for predictive cutoff point needed
rs_df <- data.frame("cutoff" = c(), "rs" = c())
for (i in 6:24) {
  brs_bin <- ifelse(testdf.v26$brs_tot >= i, TRUE, FALSE)
  model = lm(testdf.v26$sui_idea ~ brs_bin)
  rs_df <- rbind(rs_df, c(i, summary(model)$r.squared))
}
...
testdf.v26 <- testdf.v26 %>%
  ...
  mutate(brs_dum = case_when(brs_dum == 1 ~ 1,
                             brs_dum == 0 ~ 0)) %>%
  filter(!is.na(brs_dum))
testdf.v26$brs_dum <- as.logical(testdf.v26$brs_dum)

# output
rs_df_arranged[1,1]
table(testdf.v26$brs_tot, useNA = "always")
table(testdf.v26$brs_dum, useNA = "always")
```
...
```{r MM_corr}
# Define MM variable candidates for correlation matrix
testdf_mm <- testdf.v27 %>% select(inf, flourish_dum, belong1, mh_stigma)
...
testdf_mm <- testdf_mm %>% 
  ...
  dummy_cols(remove_first_dummy = TRUE) %>% 
  data.matrix()
testdf_mm   # check output
results_mm <- dummy_cols(testdf_mm, select_columns = c("inf", "flourish_dum", "belong1"),
                         remove_first_dummy = TRUE)
mm_pca <- PCA(results_mm, graph=FALSE)  # apply PCA
eigval_mm <- get_eigenvalue(mm_pca)
eigval_mm
...
# build table for absolute missingness based on school2
school2 <- count(testdf.v28, school2)
school2_ncol <- ncol(school2)
...
for (j in 1:(length(var_names))) {
for (i in 1:nrow(school2)) {   # i corresponds to rows, j to columns
  school2[i, 2 + j] <- sum(!is.na(testdf.v28[testdf.v28$school2 == school2[[i,1]], var_names[j]]))
}
}
# add percentage in each column
school2_m <- as.matrix(school2[-1]) 
...
#create skeleton of missingness matrix
var <- count(testdf.v28, testdf.v28[var_names[k]])
var_ncol <- ncol(var)
...
for (j in 1:(length(var_names))) {
for (i in 1:nrow(var)) {   # i corresponds to rows, j to columns
  var[i, 2 + j] <- sum(!is.na(testdf.v28[testdf.v28[[var_names[k]]] == var[[i,1]], var_names[j]]))
}
}
# add percentage in each column
var_m <- as.matrix(var[-1]) 
...
results_tr_com <- dummy_cols(testdf_sem5, select_columns = c("si_type", "sexual", "inf", "anx_type", "dep_type",
                                                              "race", "talk", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
results_tr_com2 <- cbind(testdf_sem5, results_tr_com)
```
...
fit_test3 <- sem(SI_model_tr2, data=results_tr_com2)
