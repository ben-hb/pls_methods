...
results_mm <- dummy_cols(traindf_mm, select_columns = c("inf", "flourish_dum", "belong1", "mh_stigma"),
                         remove_first_dummy = TRUE)
...
```{r CLM mm}
clm_mm <- traindf.v27 %>% select(si_type, inf, flourish_dum, belong1, mh_stigma)
clm_mm <- na.omit(clm_mm)

# run MM regression
clm_mm <- clm(si_type ~ inf + flourish_dum + belong1 + mh_stigma, data = clm_mm)
...
traindf_sem <- traindf.v28 %>% select(
  ...
  dep_impa, gad7_impa,
  # motivational moderators
  inf, flourish_dum, belong1, mh_stigma)  #mh_stigma, 
...
SI_model_tr <- ' # measurement model
                  ...
                  mm_sem =~  inf + flourish_dum + belong1 + mh_stigma
...
```{r}
# diss_cfa <- cfa(model = SI_model_tr, data = traindf_sem2, 
#                 ordered =c("si_type", "age", "meds_count", "satisfied_overall",
#                  "gpa_sr", "aca_impa", "dep_type", "anx_type", "persist",
...
fit_train <- sem(SI_model_tr, data=traindf_sem3,
                 ...
                 "gpa_sr", "aca_impa", "dep_type", "anx_type", "persist",
                 "dep_impa", "gad7_impa", "flourish_dum", "belong1"))
summary_train <- summary(fit_train, fit.measures=TRUE, standardized = TRUE, condition.x=TRUE)
...
# model prediction
fit_train$prediction <- predict(SI_model_test, newdata = traindf_sem2, type = "response")
