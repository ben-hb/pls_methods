...
```{r CLM environment}
clm_env <- traindf.v15 %>% select(si_type, religios, residenc, activ_yn, military,
...
clm_env2 <- clm(si_type ~ religios + residenc + activ_yn + military +
                  school2_type + international, 
                data = clm_env)
summary(clm_env2)
coef(clm_env2)
...
SI_model_tr_M <- '
                # measurement model
                   si =~  si_type_2 + si_type_3 + si_type_4
                   prem =~ age_2 + age_3 + sexual_2 + sexual_3 + gender_tgd + sib_freq + military +
                           international + ed_any + dx_mh + meds_count + religios + school2_type_2 + 
                           school2_type_3 + activ_yn + ins_cover + gpa_sr + binge + psyhx + fincur + 
                           drugs_yn + assault + drug_mar + residenc_2 + residenc_3 + residenc_4
                   def =~ aca_impa + dep_type + anx_type
                   tsm =~ persist + aaq_dum
                   entrap =~ dep_impa + gad7_impa
                   mm =~ inf + flourish_dum + belong1 + satisfied_overall
