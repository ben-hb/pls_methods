...
SI_model_tr1 <- '
                # measurement model
                  ...
                  prem  =~ age_19 + age_20 + age_21 + age_22 + age_23 + age_24 + age 25 +
...
traindf_sem_d <- traindf.v28 %>% select(
  # outcome var
  si_type,
  # diathesis
  age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, meds_count,
...
# create sem matrix 
traindf_sem_d <- traindf_sem_d %>% 
  # mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
traindf_sem_d <- na.omit(traindf_sem_d)

results_tr_sem_d <- dummy_cols(traindf_sem_d, select_columns = c(
    # outcome var
    "si_type",
    # diathesis
    "age", "sexual", "gender_tgd", "sib_freq", "race", "ed_any", "dx_mh", "meds_count",
...
results_tr_sem_d <- na.omit(results_tr_sem_d)
results_tr_d <- as.data.frame(results_tr_sem_d)
head(results_tr_d)
...
fit_train_d <- sem(SI_model_trD, data=results_tr_d)
