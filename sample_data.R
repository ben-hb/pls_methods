...
table(data.v10$audit_tot) # check output (too small)

data.v10 = subset(data.v10, select= -c(audit_1:audit_10, audit_3_m, 
                                             audit_3_f, audit_3_o)) # clean up vars/columns


## ETOH treatment
table(data.v10$alc_tx) # check output (too small)
data.v10 = subset(data.v10, select= -c(alc_tx)) # clean up vars/columns

## Eating disorder, any type
data.v10 <- data.v10 %>%
  filter(!is.na(ed_any))  # filter out all NAs for complete data

table(data.v10$ed_any, useNA = "always") # check output, confirms no NAs

## WCS
# check output (too small)
table(data.v10$wcs_1_f)
table(data.v10$wcs_1_m)
table(data.v10$wcs_1_o)

table(data.v10$wcs_2)
table(data.v10$wcs_3)
table(data.v10$wcs_4)
table(data.v10$wcs_5)

data.v10 = subset(data.v10, select= -c(wcs_1_f, wcs_1_m, wcs_1_o, 
...
data.v11 <- data.v11 %>%
  ...
  mutate(meds_count = case_when(meds_count == 0 ~ 0,
                                ...
                                meds_count == 5 ~ 5,
                                meds_count == 6 ~ 6))
data.v11$meds_count <- as.numeric(data.v11$meds_count) # return var to numeric cass
...
data.v11 <- data.v11 %>%
  mutate(meds_ther = case_when(meds_any == 1 ~ 1,
                               ther_cur == 1 ~ 1,
                               ther_cur1 == 1 ~ 1,
                               ther_ever == 3 ~ 1,
                               ther_ever == 4 ~ 1,
                               meds_any == 0 ~ 0)) %>%
  filter(!is.na(meds_ther))
data.v11$meds_ther <- as.logical(data.v11$meds_ther)

table(data.v11$meds_ther, useNA = "always")
...
traindf_dia <- data.v12 %>% select(age, sexual, gender_tgd, finpast, sib_freq, race, ed_scoff, body_sr,
                                      dx_mh, meds_count, ed_any, meds_sti, meds_dep, meds_psy, meds_anx, meds_mood,
                                      meds_sle, meds_oth, meds_ther)
...
pca_dia <- data.matrix(traindf_dia)
results_dia <- dummy_cols(pca_dia, select_columns = c("age", "sexual", "gender_tgd", "finpast", 
                                                      "sib_freq", "race", "dx_mh", "ed_any", "meds_ther"), 
...
data.v13$discrim<- as.numeric(data.v13$discrim)
table(data.v13$discrim, useNA = "always") # check output (lots of NAs)
...
table(data.v13$residenc, useNA = "always")  # check output


## MH campus environment

data.v13$env_mh <- as.factor(data.v13$env_mh)
table(data.v13$env_mh, useNA = "always")  # check output (excessive NAs)


## Military status

data.v14 <- data.v13
table(data.v14$military, useNA = "always")
...
data.v14$international <- as.logical(data.v14$international)

table(data.v14$international, useNA = "always")
 

## Family support
data.v14$fam_support_aca <- as.numeric(data.v14$fam_support_aca)
table(data.v14$fam_support_aca, useNA = "always")  # check output (excessive NAs)
 

## Activity Involvement
# set all NAs in activities equal to 0
data.v14$activ_ac[is.na(data.v14$activ_ac)] <- 1
data.v14$activ_athc[is.na(data.v14$activ_athc)] <- 1
data.v14$activ_athv[is.na(data.v14$activ_athv)] <- 1
data.v14$activ_athi[is.na(data.v14$activ_athi)] <- 1
data.v14$activ_cs[is.na(data.v14$activ_cs)] <- 1
data.v14$activ_cu[is.na(data.v14$activ_cu)] <- 1
data.v14$activ_da[is.na(data.v14$activ_da)] <- 1
data.v14$activ_fs[is.na(data.v14$activ_fs)] <- 1
data.v14$activ_gs[is.na(data.v14$activ_gs)] <- 1
data.v14$activ_gov[is.na(data.v14$activ_gov)] <- 1
data.v14$activ_mp[is.na(data.v14$activ_md)] <- 1
data.v14$activ_md[is.na(data.v14$activ_none)] <- 1
data.v14$activ_soc[is.na(data.v14$activ_soc)] <- 1
data.v14$activ_rel[is.na(data.v14$activ_rel)] <- 1
data.v14$activ_art[is.na(data.v14$activ_art)] <- 1
data.v14$activ_other[is.na(data.v14$activ_other)] <- 1
...
data.v14 <- data.v14 %>% 
  mutate(activ_no = case_when(activ_none == 1 ~ 1,
                              ...
                              activ_art == 1 ~ 0,
                              activ_other == 1 ~ 0))
data.v14$activ_no <- as.logical(data.v14$activ_no)

table(data.v14$activ_no, useNA = "always")     # tabulate and check output
jarque.bera.test(data.v14$activ_no)
...
traindf_env <- data.v15 %>% select(discrim, religios, residenc, activ_no, divers, 
                                      school2_type, international)
...
traindf_env <- as.matrix(traindf_env)
...
clm_env <- data.v15 %>% select(si_type, religios, residenc, activ_no, school2_type)
clm_env <- NA_preproc(clm_env) %>% as.data.frame()

# run ordinal regression
clm_env2 <- clm(si_type ~ religios + residenc + activ_no + school2_type, 
                data = clm_env)
...
data.v16 <- data.v16 %>% 
  ...
  filter(!is.na(ins_no))   
data.v16$ins_no <- as.logical(data.v16$ins_no) # switch class to boolean
table(data.v16$ins_no, useNA = "always")   # check output, skew
jarque.bera.test(data.v16$ins_no)
...
data.v16$drug_coc[is.na(data.v16$drug_coc)] <- 0
data.v16$drug_her[is.na(data.v16$drug_her)] <- 0
data.v16$drug_met[is.na(data.v16$drug_met)] <- 0
data.v16$drug_stim[is.na(data.v16$drug_stim)] <- 0
data.v16$drug_ect[is.na(data.v16$drug_ect)] <- 0
data.v16$drug_opi[is.na(data.v16$drug_opi)] <- 0
data.v16$drug_other[is.na(data.v16$drug_other)] <- 0
...
data.v16 <- data.v16 %>%
  mutate(drugs_yn = case_when(drug_coc == 1 ~ 1,
                              ...
                              drug_opi == 1 ~ 1,
                              drug_other == 1 ~ 1))
data.v16$drugs_yn[is.na(data.v16$drugs_yn)] <- 0
data.v16$drugs_yn <- as.logical(data.v16$drugs_yn)

table(data.v16$drugs_yn, useNA = "always")
jarque.bera.test(data.v16$drugs_yn)
 

## Marijuana use
data.v16$drug_mar[is.na(data.v16$drug_mar)] <- 0
...
jarque.bera.test(data.v16$drug_mar)
...
table(data.v16$fincur, useNA = "always")  # check output, skew
jarque.bera.test(data.v16$fincur)
 

## ISI scale
table(data.v16$isi_1)
table(data.v16$isi_2)
data.v16 = subset(data.v16, select= -c(isi_1, isi_2, isi_3, isi_4, 
...
jarque.bera.test(data.v17$assault)
...
jarque.bera.test(data.v18$binge)
...
data.v18$psyhx <- as.logical(data.v18$psyhx)  # change class to boolean

table(data.v18$psyhx, useNA = "always")   # check output, skew
jarque.bera.test(data.v18$psyhx)
...
data.v18$gpa_sr <- as.numeric(data.v18$gpa_sr) # change to numeric scale

table(data.v18$gpa_sr, useNA = "always")  # check output, skew
jarque.bera.test(data.v18$gpa_sr)
...
traindf_le <- data.v19 %>% select(ins_no, gpa_sr, fincur, binge,
...
## EFA LE
traindf_le <- data.v19 %>% select(ins_no, gpa_sr, fincur, assault,
...
clm_le <- data.v19 %>% select(si_type, ins_no, gpa_sr, assault, binge, drugs_yn, 
                                 drug_mar, psyhx, fincur)
clm_le <- na.omit(clm_le)

# run regression
clm_le <- clm(si_type ~ ins_no + gpa_sr + assault + binge + drugs_yn + drug_mar + 
                psyhx + fincur,
              data = clm_le)
summary(clm_le)
coef(clm_le)
...
# Define diathesis vars for correlation matrix
traindf_prem <- data.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       meds_ther, religios, activ_no, school2_type, fincur, ins_no, residenc,
                                       gpa_sr, assault, drugs_yn, drug_mar, psyhx)
...
## EFA PREMOTIVATION
# Define diathesis vars for correlation matrix
traindf_prem <- data.v19 %>% select(age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                       meds_ther, religios, activ_no, school2_type, ins_no, residenc,
                                       gpa_sr, assault, drugs_yn, drug_mar, psyhx, fincur)
...
results_prem <- dummy_cols(traindf_prem, select_columns = c("age", "sexual", "gender_tgd", "sib_freq",
                                                            "race", "ed_any", "dx_mh", "meds_ther", "religios",
                                                            "activ_no", "school2_type", "ins_no", "residenc",
                                                            "gpa_sr", "assault", "drugs_yn","drug_mar", 
                                                            "psyhx", "fincur"),
                           remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
clm_prem <- data.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                   religios, activ_no, school2_type, ins_no, residenc, meds_ther,
                                   gpa_sr, assault, drugs_yn, drug_mar, psyhx, fincur)
clm_prem <- na.omit(clm_prem)

# run regression
clm_prem <- clm(si_type ~ age + sexual + gender_tgd + sib_freq + race + ed_any + dx_mh +
                  religios + activ_no + school2_type + ins_no + residenc + gpa_sr + assault +
                  drugs_yn + drug_mar + psyhx + meds_ther + fincur,
                data = clm_prem)
summary(clm_prem)
...
table(data.v20$deprawsc, useNA ="always")
...
data.v20$anx_type <- as.numeric(data.v20$anx_type)   # check output, skew
jarque.bera.test(data.v20$anx_type)
...
table(data.v20$aca_impa, useNA = "always") # check output, skew
jarque.bera.test(data.v20$aca_impa)
...
traindf_def <- data.v21 %>% select(aca_impa, anx_type, dep_type)
...
## EFA defeat
traindf_def <- data.v21 %>% select(aca_impa, dep_type, anx_type)
...
table(data.v22$dep_secret, useNA = "always")  # check output, too many NAs
...
table(data.v22$aaq_tot) # check output
...
data.v22$aaq_dum <- as.numeric(data.v22$aaq_dum)

table(data.v22$aaq_dum, useNA = "always")  # check output, NAs
...
table(data.v22$persist, useNA = "always") # check output, skew
jarque.bera.test(data.v22$persist)
...
traindf_tsm <- data.v23 %>% select(persist, aaq_dum)
...
table(data.v24$dep_impa, useNA = "always")  # check output, NAs, skew
jarque.bera.test(data.v24$dep_impa)
 

## Anxiety impact
data.v24$gad7_impa <- as.numeric(data.v24$gad7_impa)
table(data.v24$gad7_impa, useNA = "always") # check output, skew, NAs
...
table(data.v24$percneed, useNA = "always") # check output, NAs
...
traindf_entrap <- data.v25 %>% select(dep_impa, gad7_impa, percneed)
...
clm_entrap <- data.v25 %>% select(si_type, dep_impa, gad7_impa, percneed)
...
jarque.bera.test(data.v26$belong1)
...
data.v26$satisfied_overall <- as.numeric(data.v26$satisfied_overall)

table(data.v26$satisfied_overall, useNA = "always")
jarque.bera.test(data.v26$satisfied_overall)
...
table(data.v26$inf, useNA ="always") # check output, skew
jarque.bera.test(data.v26$inf)
data.v26 = subset(data.v26, select= -c(inf_1:inf_help))  # clean up vars
...
data.v26$brs_tot <- as.numeric(data.v26$brs_tot)
...
data.v26 <- data.v26 %>%
  ...
  mutate(brs_dum = case_when(brs_dum == 1 ~ 1,
                             brs_dum == 0 ~ 0))
data.v26$brs_dum <- as.logical(data.v26$brs_dum)

# output
rs_df_arranged[1,1]
table(data.v26$brs_tot, useNA = "always")
table(data.v26$brs_dum, useNA = "always")
 

## Distress & social support
data.v26$talk1_1[is.na(data.v26$talk1_1)] <- 1
data.v26$talk1_2[is.na(data.v26$talk1_2)] <- 1
data.v26$talk1_3[is.na(data.v26$talk1_3)] <- 1
data.v26$talk1_4[is.na(data.v26$talk1_4)] <- 1
data.v26$talk1_5[is.na(data.v26$talk1_5)] <- 1
data.v26$talk1_6[is.na(data.v26$talk1_6)] <- 1
data.v26$talk1_7[is.na(data.v26$talk1_7)] <- 1
data.v26$talk1_8[is.na(data.v26$talk1_8)] <- 1
...
data.v26$talk <- as.logical(data.v26$talk)

table(data.v26$talk, useNA = "always") # check output

data.v26 = subset(data.v26, select= -c(talk1_1:talk1_9, talk1sig, talk1fr, 
...
table(data.v26$mh_stigma, useNA = "always") # check out, NAs
...
traindf_mm <- data.v27 %>% select(inf, flourish_dum, belong1, mh_stigma, satisfied_overall)
...
results_mm <- dummy_cols(traindf_mm, select_columns = c("flourish_dum", "belong1", "inf", 
...
clm_mm <- data.v27 %>% select(si_type, inf, flourish_dum, belong1, satisfied_overall, mh_stigma)
clm_mm <- na.omit(clm_mm)

# run MM regression
clm_mm <- clm(si_type ~ inf + flourish_dum + belong1 + satisfied_overall + mh_stigma, 
              data = clm_mm)
...
var_names <- c(
  # diathesis
  "age", "sexual", "gender_tgd", "sib_freq", "ed_any", "dx_mh", "meds_ther",
  # environment
  "religios", "activ_no", "international",
  # life events
  "ins_no", "gpa_sr", "psyhx", "fincur", "drugs_yn", "assault",
  # defeat
  ...
  "dep_impa", "percneed",
  # motivational moderators
  "inf", "flourish_dum", "satisfied_overall")  
...
# Prep SEM data vars
traindf_sem <- data.v28 %>% select(
  # outcome var
  si_type,
  # diathesis
  age, sexual, gender_tgd, sib_freq, ed_any, dx_mh, meds_ther,
  # environment
  religios, activ_no, residenc,
  # life events
  ins_no, gpa_sr, psyhx, fincur, drugs_yn, assault,
  # defeat
...
write_xlsx(traindf_sem_out, path = "traindf_nondummy_ALL.xls", col_names = TRUE, format_headers = TRUE)
...
results_tr_sem1 <- dummy_cols(traindf_sem, select_columns = c(
  ...
  "si_type",
  # diathesis
  "age", "sexual", "gender_tgd", "sib_freq", "ed_any", "dx_mh", "meds_count", "meds_ther",
  # environment
  "religios", "activ_no",
  # life events
  "ins_no", "gpa_sr", "psyhx", "fincur", "drugs_yn", "assault",
...
write_xlsx(results_tr_df1, path = "traindf_sem_ALL.xls", col_names = TRUE, format_headers = TRUE)
...
traindf_sem1 <- data.v28 %>% select(
  ...
  religios, activ_no,
  # life events
  ins_no, gpa_sr, psyhx, drugs_yn, assault,
  # defeat
  aca_impa, dep_type, anx_type,
  # threats to self-moderation
  persist, aaq_dum,
