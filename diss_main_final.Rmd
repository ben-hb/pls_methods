...
traindf_sem <- traindf.v28 %>% select(
  ...
  ins_no, gpa_sr, psyhx, fincur, drugs_yn, assault,
  # defeat
  aca_impa, anx_score, deprawsc, dep_impa, percneed,
  # threats to self-moderation
  persist, aaq_tot, inf, flourish, talk)
...
results_tr_sem1 <- dummy_cols(traindf_sem, select_columns = c(
    ...
    "si_type",
    # premotivation
    "age", "sexual", "race", "sib_freq", "ed_any", "dx_mh", "meds_ther", "gender_tgd",
    "religios", "satisfied_overall", "residenc",
    # life events
    "ins_no", "gpa_sr", "psyhx", "fincur", "drugs_yn", "assault",
    # defeat
    "aca_impa", "anx_score", "deprawsc", "dep_impa", "percneed",
...
testdf.v13 <- testdf.v13 %>% 
  mutate(res_status = case_when(citizen == 1 ~ 1,
                                ...
                                undoc == 1 ~ 4,
                                otherdoc == 1 ~ 5))
table(testdf.v13$res_status)  # check output
# data.v13 = subset(testdf.v12, select= -c(citizen, permanentres, visa, otherdoc, undoc))   # clean up var
```

## Discrimination
```{r train discrim}
testdf.v13$discrim<- as.numeric(testdf.v13$discrim)
table(testdf.v13$discrim, useNA = "always") # check output (lots of NAs)
```
...
## MH campus environment
```{r train env_mh}
testdf.v13$env_mh <- as.factor(testdf.v13$env_mh)
table(testdf.v13$env_mh, useNA = "always")  # check output (excessive NAs)
```
...
```{r train results_prem}
# Define diathesis vars for correlation matrix
testdf_prem <- testdf.v15 %>% select(si_type, race, age, sexual, gender_tgd, sib_freq, ed_any, 
                                       dx_mh, international, meds_any, ther, religios, school2_type, 
                                       residenc, satisfied_overall)
...
```{r train prem_pca}
# Define diathesis vars for correlation matrix
testdf_prem <- testdf.v15 %>% select(age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, international,
                                       meds_any, ther, religios, school2_type, residenc, satisfied_overall)
...
## CLM Premotivation regression
```{r train clm_prem}
clm_prem <- testdf.v15 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, 
                                   dx_mh, international, religios, school2_type, residenc, 
                                   meds_any, ther, satisfied_overall)
clm_prem <- na.omit(clm_prem)
...
testdf.v16 = subset(testdf.v16, select= -c(isi_1, isi_2, isi_3, isi_4, 
                                             isi_5, isi_6, isi_7)) # clean up var
```
...
```{r train defeat cor}
# Define defeat variable candidates for correlation matrix
testdf_def <- testdf.v21 %>% select(aca_impa, anx_score, deprawsc, percneed, gad7_impa, dep_impa)
...
### EFA defeat
```{r train EFA_def}
testdf_def <- testdf.v21 %>% select(aca_impa, deprawsc, anx_score, percneed, gad7_impa, dep_impa)
...
results_def <- dummy_cols(testdf_def, select_columns = c("aca_impa", "deprawsc", "anx_score",
                                                          "percneed", "gad7_impa", "dep_impa"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
```{r train CLM defeat}
# prepare regression analysis
results2_def <- testdf.v21 %>% select(si_type, aca_impa, deprawsc, anx_score, percneed, gad7_impa, dep_impa)
clm_def <- na.omit(results2_def)
...
table(testdf.v22$dep_secret, useNA = "always")  # check output, too many NAs
```
...
```{r train MM_corr}
# Define MM variable candidates for correlation matrix
testdf_mm <- testdf.v27 %>% select(inf, flourish, belong1, mh_stigma, talk, brs_tot, persist, aaq_tot)
...
### EFA MM
```{r train EFA MM}
testdf_mm <- testdf.v27 %>% select(inf, flourish, belong1, talk, mh_stigma, persist, aaq_tot)
...
results_mm <- dummy_cols(testdf_mm, select_columns = c("flourish", "belong1", "inf", 
                                                        "mh_stigma", "talk", "persist", "aaq_tot"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
mm_pca <- PCA(results_mm, graph=FALSE)  # apply PCA
eigval_mm <- get_eigenvalue(mm_pca)
...
```{r train CLM mm}
# prepare data for regression
clm_mm <- testdf.v27 %>% select(si_type, inf, flourish, belong1, talk, mh_stigma, persist, aaq_tot)
clm_mm <- na.omit(clm_mm)
...
# demographics
table(testdf.v28$age)
table(testdf.v28$si_type)
table(testdf.v28$gender_tgd)
table(testdf.v28$sexual)
table(testdf.v28$race)
table(testdf.v28$dx_mh)
...
testdf_sem2 <- as.data.frame(testdf_sem1)

# xlsx output
save(testdf_sem2, file = "testdf_sem2.RData")
load("testdf_sem2.RData")
write_xlsx(testdf_sem2, path = "testdf_sem2.xlsx", col_names = TRUE, format_headers = TRUE)
```
...
## Dummy col SEM w/ continuous
results_tr_com1 <- dummy_cols(testdf_sem1, select_columns = c("si_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)

# xlsx output
save(results_tr_com1, file = "testdf_pls.RData")
