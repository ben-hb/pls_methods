...
traindf_prem <- traindf.v15 %>% select(sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, international, finpast,
                                       meds_ther, religios, school2_type, residenc, satisfied_overall, undercls)
...
clm_all <- traindf.v27 %>% select(
  ...
  inf, flourish, belong1, talk, mh_stigma, persist, aaq_tot)

clm_mm <- na.omit(clm_mm)

# run MM regression
clm_mm <- glm(si_type ~ religios +  satisfied_overall +  residenc +  school2_type +  international +  undercls + 
  # life events
  ins_no +  gpa_sr +  psyhx +  fincur +  drugs_yn +  assault +  binge +  drug_mar + 
  # defeat-entrapment
  deprawsc +  anx_score +  dep_impa +  percneed + 
