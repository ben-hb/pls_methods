...
traindf_prem <- traindf.v15 %>% select(si_type, race, undercls, sexual, gender_tgd, sib_freq, ed_any, 
                                       dx_mh, international, meds_ther, religios, school2_type, finpast,
...
```{r train prem_pca}
# Define diathesis vars for correlation matrix
traindf_prem <- traindf.v15 %>% select(sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, international, finpast,
                                       meds_any, religios, school2_type, residenc, satisfied_overall, undercls)
...
results_prem <- dummy_cols(traindf_prem, select_columns = c("sexual", "gender_tgd", "sib_freq", "finpast",
                                                            "race", "ed_any", "dx_mh", "meds_any",
                                                            "religios", "satisfied_overall", "international",
                                                            "school2_type", "residenc", "undercls"),
...
```{r train clm_prem}
clm_prem <- traindf.v15 %>% select(si_type, sexual, gender_tgd, sib_freq, race, ed_any, finpast,
                                   dx_mh, international, religios, school2_type, residenc, 
                                   meds_ther, satisfied_overall, undercls)
clm_prem <- na.omit(clm_prem)

# run regression
clm_prem <- glm(si_type ~ sexual + gender_tgd + sib_freq + race + ed_any + dx_mh + undercls + finpast +
                religios +  school2_type + international + residenc + meds_ther + satisfied_overall,
              data = clm_prem)
summary(clm_prem)
...
traindf.v26 <- traindf.v26 %>%
  mutate(ther = case_when(ther_cur == 1 ~ 0,
                          ther_cur1 == 1 ~ 0,
                          ther_ever == 3 ~ 0,
                          ther_ever == 4 ~ 0,
                              ther_cur == 0 ~ 1,
...
## Threats to moderation analyses
```{r train MM var analyses}
traindf.v27 <- traindf.v26
```

### Threats to moderation correlation
```{r train MMTSM_corr}
# Define MM variable candidates for correlation matrix
traindf_mm <- traindf.v27 %>% select(inf, flourish, belong1, mh_stigma, talk, brs_tot, ther, persist, aaq_tot)
...
### EFA MM
```{r train EFA MM}
traindf_mm <- traindf.v27 %>% select(inf, flourish, belong1, talk, mh_stigma, persist, aaq_tot, ther)
...
results_mm <- dummy_cols(traindf_mm, select_columns = c("flourish", "belong1", "inf", "ther",
...
glm_mmtsm <- traindf.v27 %>% select(si_type, inf, flourish, belong1, talk, mh_stigma, persist, ther, aaq_tot)
glm_mmtsm <- na.omit(glm_mmtsm)

# run MM regression
glm_mmtsm <- glm(si_type ~ inf + flourish + belong1 + mh_stigma + talk + persist + aaq_tot + ther, 
              data = glm_mmtsm)
...
glm_all <- traindf.v27 %>% select(
  ...
  religios, satisfied_overall, residenc, school2_type, international, undercls,
  # life events
  ...
  inf, flourish, belong1, talk, mh_stigma, persist, aaq_tot, ther)

glm_all <- na.omit(glm_all)

# run MM regression
glm_all <- glm(si_type ~ religios +  satisfied_overall +  residenc +  school2_type +  international +  undercls + 
  # life events
  ins_no +  gpa_sr +  psyhx +  fincur +  drugs_yn +  assault +  binge +  drug_mar + 
  # defeat-entrapment
  deprawsc +  anx_score +  dep_impa +  percneed + 
...
var_names <- c(
  # premotivation
  "age", "sexual", "race", "gender_tgd", "sib_freq", "ed_any", "dx_mh", "meds_ther", "finpast",
  "religios", "satisfied_overall", "residenc", "school2_type", "international", "undercls",
  # life events
  "ins_no", "gpa_sr", "psyhx", "fincur", "drugs_yn", "assault", "binge", "drug_mar",
  # defeat-entrapment
...
# training v. testing
wilcox.test(testdf.v26B$age,traindf.v28B$age, alternative = "two.sided") # W = 18459578, p-value = 0.8103
wilcox.test(testdf.v26B$si_type,traindf.v28B$si_type, alternative = "two.sided") # W = 18354510, p-value = 0.2093
wilcox.test(testdf.v26B$gender_tgd,traindf.v28B$gender_tgd, alternative = "two.sided") # W = 18440319, p-value = 0.2759
wilcox.test(testdf.v26B$sexual, traindf.v28B$sexual, alternative = "two.sided") # W = 18089541, p-value = 0.003971
wilcox.test(testdf.v26B$race, traindf.v28B$race, alternative = "two.sided") # W = 18322271, p-value = 0.2655
wilcox.test(testdf.v26B$dx_mh, traindf.v28B$dx_mh, alternative = "two.sided") # W = 18480308, p-value = 0.8775

# 18-19 v. 20-25
wilcox.test(test20.v26B$age, test18.v26B$age, alternative = "two.sided") # W = 9258080, p-value < 2.2e-16 ***
wilcox.test(test20.v26B$si_type, test18.v26B$si_type, alternative = "two.sided") # W = 4470268, p-value = 0.0002689 ***
wilcox.test(test20.v26B$gender_tgd, test18.v26B$gender_tgd, alternative = "two.sided") # W = 4596288, p-value = 0.1257
wilcox.test(test20.v26B$sexual, test18.v26B$sexual, alternative = "two.sided") #  W = 4443808, p-value = 0.0003753 ***
wilcox.test(test20.v26B$race, test18.v26B$race, alternative = "two.sided") # W = 4773152, p-value = 0.01635
wilcox.test(test20.v26B$dx_mh, test18.v26B$dx_mh, alternative = "two.sided") # W = 4759928, p-value = 0.02914
``` 
