...
traindf_dia <- traindf.v12 %>% select(age, sexual, gender_tgd, finpast, sib_freq, race, ed_scoff, body_sr,
                                ...
                                meds_sle, meds_oth, meds_any, ther)
...
traindf_dia <- traindf.v12 %>% select(age, sexual, gender_tgd, finpast, sib_freq, race, ed_scoff, body_sr,
                                dx_mh, meds_any, ed_any, ther)

pca_dia <- data.matrix(traindf_dia)
results_dia <- dummy_cols(pca_dia, select_columns = c("age", "sexual", "gender_tgd", "finpast", "meds_any",
                                                       "sib_freq", "race", "dx_mh", "ed_any", "ther"), 
...
clm_dia <- traindf.v12 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race,
                                  ed_any, dx_mh, meds_any, ther)
clm_dia <- na.omit(clm_dia)
dia_mod <- as.data.frame(clm_dia)

# regression call
dia_mod <- glm(si_type ~ ., data = dia_mod)
...
clm_env2 <- glm(si_type ~ religios + residenc + activ_no + school2_type + satisfied_overall, 
                data = clm_env)
...
# Define diathesis vars for correlation matrix
traindf_prem <- traindf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, binge,
...
```{r train prem_pca}
# Define diathesis vars for correlation matrix
traindf_prem <- traindf.v19 %>% select(age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, binge,
...
results_prem <- dummy_cols(traindf_prem, select_columns = c("age", "sexual", "gender_tgd", "sib_freq",
                                                            "race", "ed_any", "dx_mh", "meds_ther", "binge",
...
traindf_mm <- traindf.v27 %>% select(inf, flourish, belong1, mh_stigma, talk, brs_tot)
...
### EFA MM
```{r train EFA MM}
traindf_mm <- traindf.v27 %>% select(inf, flourish, belong1, talk, mh_stigma)
...
clm_mm <- traindf.v27 %>% select(si_type, inf, flourish, belong1, talk, mh_stigma)
clm_mm <- na.omit(clm_mm)

# run MM regression
clm_mm <- clm(si_type ~ inf + flourish + belong1 + mh_stigma + talk, data = clm_mm)
...
traindf_sem <- traindf.v28 %>% select(
  ...
  si_type,
  # diathesis
  age, sexual, sib_freq, ed_any, dx_mh, meds_ther, gender_tgd,
  # environment
  religios, activ_no, satisfied_overall,
  # life events
  ins_no, gpa_sr, psyhx, fincur, drugs_yn, assault,
...
traindf_sem1 <- traindf.v28 %>% select(
  ...
  si_type,
  # diathesis
  age, race, sexual, sib_freq, ed_any, dx_mh, meds_ther, religios, activ_no, satisfied_overall, gender_tgd,
  # life events
  ins_no, gpa_sr, psyhx, drugs_yn, assault,
  # defeat
  aca_impa, deprawsc, anx_score, dep_impa, percneed,
  # MM+ TSM
  inf, flourish, persist, aaq_tot, talk) 
...
testdf_sem <- testdf.v26 %>% select(
  ...
  si_type,
  # diathesis
  age, sexual, sib_freq, ed_any, dx_mh, meds_ther, undercls, gender_tgd,
  # environment
  religios, activ_no, satisfied_overall,
  # life events
  ins_no, gpa_sr, psyhx, fincur, drugs_yn, assault,
...
results_test_sem1 <- dummy_cols(testdf_sem, select_columns = c(
    ...
    "si_type",
    # diathesis
    "age", "sexual", "sib_freq", "ed_any", "dx_mh", "meds_count", "meds_ther", "undercls", "gender_tgd",
    # environment
    "religios", "activ_no", "satisfied_overall",
    # life events
    "ins_no", "gpa_sr", "psyhx", "fincur", "drugs_yn", "assault",
...
test_sem <- testdf.v26 %>% select(
  ...
  si_type,
  # premotivation
  age, sexual, sib_freq, ed_any, dx_mh, meds_ther, religios, activ_no, satisfied_overall, undercls, gender_tgd,
  # life events
  ins_no, gpa_sr, psyhx, drugs_yn, assault,
  # defeat
  aca_impa, deprawsc, anx_score, dep_impa, percneed,
...
## Dummy col SEM w/ continuous
results_test_com <- dummy_cols(test_sem, select_columns = c("si_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
```
...
test18_sem <- test18.v26 %>% select(
  ...
  si_type,
  # diathesis
  age, sexual, sib_freq, ed_any, dx_mh, meds_ther, religios, activ_no, satisfied_overall, gender_tgd,
  # life events
  ins_no, gpa_sr, psyhx, drugs_yn, assault,
  # defeat
  aca_impa, deprawsc, anx_score, dep_impa, percneed,
...
test20_sem <- test20.v26 %>% select(
  ...
  aca_impa, deprawsc, anx_score, dep_impa, percneed,
  # MM+ TSM
  inf, flourish, satisfied_overall, persist, aaq_tot, talk) 

# create sem matrix 
test20_sem <- data.matrix(test20_sem)
  
## Dummy col SEM w/ continuous
results_test_com20 <- dummy_cols(test20_sem, select_columns = c("si_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
