...
traindf_dia <- traindf.v12 %>% select(age, race, sexual, gender_tgd, finpast, sib_freq, race, ed_scoff, body_sr,
...
results_dia <- dummy_cols(pca_dia, select_columns = c("age", "race", "sexual", "gender_tgd", "finpast", "meds_any",
...
clm_env <- traindf.v15 %>% select(si_type, religios, residenc, activ_no, school2_type, satisfied_overall, international)
clm_env <- NA_preproc(clm_env) %>% as.data.frame()

# run ordinal regression
clm_env2 <- glm(si_type ~ religios + residenc + activ_no + school2_type + satisfied_overall + international, 
                data = clm_env)
...
# Define diathesis vars for correlation matrix
traindf_prem <- traindf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, 
                                       dx_mh, international, meds_any, ther, religios, school2_type, 
                                       fincur, ins_no, residenc, satisfied_overall)
...
```{r train prem_pca}
# Define diathesis vars for correlation matrix
traindf_prem <- traindf.v19 %>% select(age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, international,
                                       meds_any, ther, religios, school2_type, ins_no, residenc, satisfied_overall)
...
clm_prem <- traindf.v19 %>% select(si_type, age, sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, 
                                 religios, school2_type, ins_no, residenc, meds_any, ther, satisfied_overall)
...
clm_def <- glm(si_type ~ aca_impa + deprawsc + anx_score, data = clm_def)
...
# run regression
clm_tsm <- glm(si_type ~ persist + aaq_tot, data = clm_tsm) 
...
clm_entrap <- glm(si_type ~ dep_impa + gad7_impa + percneed, data = clm_entrap)
...
traindf.v26$flourish <- as.numeric(traindf.v26$flourish)
table(traindf.v26$flourish, useNA = "always")
```
...
# run MM regression
clm_mm <- glm(si_type ~ inf + flourish + belong1 + mh_stigma + talk, data = clm_mm)
...
var_names <- c(
  ...
  # environment
  "religios", "satisfied_overall", "residenc", "school2_type",
  # life events
...
traindf_sem <- traindf.v28 %>% select(
  ...
  si_type,
  # diathesis
  age, sexual, race, sib_freq, ed_any, dx_mh, meds_any, ther, gender_tgd,
  # environment
  religios, satisfied_overall, residenc, school2_type,
  # life events
  ins_no, gpa_sr, psyhx, fincur, drugs_yn, assault,
