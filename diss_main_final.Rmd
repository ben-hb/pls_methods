...
```{r train prem_pca}
# Define diathesis vars for correlation matrix
traindf_prem <- traindf.v15 %>% select(sexual, age, gender_tgd, sib_freq, race, ed_any, dx_mh, international, finpast,
...
results_prem <- dummy_cols(traindf_prem, select_columns = c("sexual", "age", "gender_tgd", "sib_freq", "finpast",
...
glm_prem <- traindf.v15 %>% select(si_type, sexual, gender_tgd, sib_freq, race, ed_any, finpast,
                                   ...
                                   meds_any, satisfied_overall, undercls)
glm_prem <- na.omit(glm_prem)

# run regression
glm_prem <- glm(si_type ~ sexual + gender_tgd + sib_freq + race + ed_any + dx_mh + undercls + finpast +
                religios +  school2_type + international + residenc + meds_any + satisfied_overall,
              data = glm_prem)
summary(glm_prem)
...
glm_le <- traindf.v19 %>% select(si_type, ins_no, gpa_sr, assault, binge, drugs_yn, 
                                 drug_mar, psyhx, fincur)
glm_le <- na.omit(glm_le)

# run regression
glm_le <- glm(si_type ~ ins_no + gpa_sr + assault + binge + drugs_yn + drug_mar + 
                psyhx + fincur,
              data = glm_le)
summary(glm_le)
coef(glm_le)
...
glm_def <- glm(si_type ~ aca_impa + deprawsc + anx_score + percneed + gad7_impa + dep_impa, 
               data = glm_def)
...
### PCA MMTSM
```{r train PCA MMTSM}
traindf_mmtsm <- traindf.v27 %>% select(inf, flourish, belong1, talk, mh_stigma, persist, aaq_tot, ther)
...
results_mmtsm <- dummy_cols(traindf_mmtsm, select_columns = c("flourish", "belong1", "inf", "ther",
...
var_names <- c(
  ...
  "deprawsc", "anx_score", "dep_impa", "percneed",
  # threats to internal/external moderation
  "persist", "aaq_tot", "flourish", "talk", "ther")  
...
testdf.v10 <- testdf.v10 %>%
  ...
  mutate(audit_tot = as.numeric(audit_1) + as.numeric(audit_2) + as.numeric(audit_3) + 
           as.numeric(audit_4) + as.numeric(audit_5) + as.numeric(audit_6) + as.numeric(audit_7) + 
           as.numeric(audit_8) + as.numeric(audit_9) + as.numeric(audit_10))
table(testdf.v10$audit_tot) # check output (too small)

testdf.v10 = subset(testdf.v10, select= -c(audit_1:audit_10, audit_3_m, 
                                             audit_3_f, audit_3_o)) # clean up vars/columns
```

## ETOH treatment
```{r test alc_tx}
table(testdf.v10$alc_tx) # check output (too small)
testdf.v10 = subset(testdf.v10, select= -c(alc_tx)) # clean up vars/columns
```
...
```{r test environment vars}
testdf.v13 <- testdf.v11
```

## US resident status
```{r test resident status}
# aggregate all residency variables into 1 variable called 'res_status'
...
```{r test results_prem}
# Define diathesis vars for correlation matrix
testdf_prem <- testdf.v15 %>% select(si_type, race, undercls, sexual, gender_tgd, sib_freq, ed_any, 
                                       dx_mh, international, meds_any, religios, school2_type, finpast,
                                       residenc, satisfied_overall)
...
```{r test prem_pca}
# Define diathesis vars for correlation matrix
testdf_prem <- testdf.v15 %>% select(sexual, gender_tgd, sib_freq, race, ed_any, dx_mh, international, finpast,
                                       meds_any, religios, school2_type, residenc, satisfied_overall, undercls)
...
results_prem <- dummy_cols(testdf_prem, select_columns = c("sexual", "gender_tgd", "sib_freq", "finpast",
                                                            "race", "ed_any", "dx_mh", "meds_any",
                                                            "religios", "satisfied_overall", "international",
                                                            "school2_type", "residenc", "undercls"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
## GLM Premotivation regression
```{r test clm_prem}
glm_prem <- testdf.v15 %>% select(si_type, sexual, gender_tgd, sib_freq, race, ed_any, finpast,
                                   dx_mh, international, religios, school2_type, residenc, 
                                   meds_any, satisfied_overall, undercls)
glm_prem <- na.omit(glm_prem)
...
```{r test glm_LE}
# prepare dataframe for regression
glm_le <- testdf.v19 %>% select(si_type, ins_no, gpa_sr, assault, binge, drugs_yn, 
                                 drug_mar, psyhx, fincur)
glm_le <- na.omit(glm_le)
...
jarque.bera.test(testdf.v22$belong1)
...
jarque.bera.test(testdf.v24$inf)
...
## Dummy col SEM w/ continuous
results_test <- dummy_cols(testdf_sem1, select_columns = c("si_type", "race", "residenc", "school2_type"),
                         remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
# training v. testing
wilcox.test(testdf.v26B$age,traindf.v28B$age, alternative = "two.sided") # W = 15979286, p-value = 0.593
wilcox.test(testdf.v26B$si_type,traindf.v28B$si_type, alternative = "two.sided") # W = 16281395, p-value = 0.05201
wilcox.test(testdf.v26B$gender_tgd,traindf.v28B$gender_tgd, alternative = "two.sided") # W = 16044680, p-value = 0.6258
wilcox.test(testdf.v26B$sexual, traindf.v28B$sexual, alternative = "two.sided") # W = 16090712, p-value = 0.8825
wilcox.test(testdf.v26B$race, traindf.v28B$race, alternative = "two.sided") # W = 16016273, p-value = 0.7108
wilcox.test(testdf.v26B$dx_mh, traindf.v28B$dx_mh, alternative = "two.sided") # W = 16332928, p-value = 0.07548

# 18-19 v. 20-25
wilcox.test(test20.v26B$age, test18.v26B$age, alternative = "two.sided") # W = 6928929, p-value < 2.2e-16 ***
wilcox.test(test20.v26B$si_type, test18.v26B$si_type, alternative = "two.sided") # W = 3306980, p-value = 1.244e-05 ***
wilcox.test(test20.v26B$gender_tgd, test18.v26B$gender_tgd, alternative = "two.sided") # W = 3424143, p-value = 0.02272 *
wilcox.test(test20.v26B$sexual, test18.v26B$sexual, alternative = "two.sided") # W = 3244443, p-value = 3.035e-07 ***
wilcox.test(test20.v26B$race, test18.v26B$race, alternative = "two.sided") # W = 3492851, p-value = 0.5591
wilcox.test(test20.v26B$dx_mh, test18.v26B$dx_mh, alternative = "two.sided") # W = 3523642, p-value = 0.2214
``` 
