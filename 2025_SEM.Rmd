...
# PREPARING SAMPLE FOR AGES 20 - 25

## Load sample
```{r Load sample, cache = TRUE}
#IMPORT file
...
data_le <- data.v19 %>% select(ins_cover, gpa_sr, aca_impa, fincur, drug_mar, binge_fr,
...
results_le <- dummy_cols(data_le, select_columns = c("ins_cover", "assault_phys", "assault_sex",
...
```{r clm_LE}
clm_le <- data.v19 %>% select(si_type, ins_cover, gpa_sr, aca_impa, fincur, drug_mar, 
...
### EFA Entrapment
```{r CFA entrap}
data_entrap <- data.v25 %>% select(dep_impa, gad7_impa, percneed_cur)   # all continuous... treat
...
```{r CLM mm}
clm_mm <- data.v27 %>% select(si_type, inf, flourish, belong1, mh_stigma, brs_tot)
...
data_sem2_2025 <- data_sem_2025 %>% 
  mutate_if(is.numeric, as.factor) %>% 
  data.matrix()
data_sem2_2025   # check output

# xlsx output
data_sem3_2025 <- as.data.frame(data_sem2_2025)
save(data_sem3_2025, file = "data_nondummy_2025.RData")
load("data_nondummy_2025.RData")
write_xlsx(data_sem3_2025, path = "data_nondummy_2025.xls", col_names = TRUE, format_headers = TRUE)
```

## Create dummy vars
```{r dummy_cols SEM}
results_sem_2025 <- dummy_cols(data_sem2_2025, select_columns = c(
    # outcome var
    "si_type",
    # diathesis
...
```{r Var_sum}
# data
var_desc_2025 <- describe(data_sem_2025) %>% view()

# xlsx output
save(var_desc_2025, file = "var_desc_2025.RData")
load("var_desc_2025.RData")
write_xlsx(var_desc_2025, path = "var_desc_2025.xls", col_names = TRUE, format_headers = TRUE)
```


## Model correlations
```{r model_corr}
results_corr_2025 <- NA_preproc(results_sem_2025)
results_corr_2025 <- as.data.frame(results_corr_2025)

# run a correlation and drop the insignificant ones
sig <- 0.8
corr <- cor(results_corr_2025, use = "pairwise.complete.obs")                               
corr[lower.tri(corr,diag=TRUE)] <- NA                   # prepare to drop duplicates and correlations of 1    
corr[corr %in% c(-1, 1)] <- NA                          # drop perfect correlations
corr <- as.data.frame(as.table(corr))                   #turn into a 3-column table
corr <- na.omit(corr)                                   #remove the NA values from above 
corr <- subset(corr, abs(Freq) > sig)                   #select significant values  
corr_2025 <- corr[order(-abs(corr$Freq)),]                   #sort by highest correlation
print(corr_2025)
  
#turn corr back into matrix in order to plot with corrplot
mtx_corr_2025 <- acast(corr_2025, Var1~Var2, value.var="Freq")  
corrplot(mtx_corr_2025, is.corr=FALSE, tl.col="black", na.label=" ")
```

## Create data matrix, save into files
```{r save dataset}
results_sem_2025 <- as.data.frame(results_sem_2025)

# save full var set for SEM
save(results_sem_2025, file = "data_sem_2025.RData")
load("data_sem_2025.RData")
write_xlsx(results_sem_2025, path = "data_sem_2025.xls", col_names = TRUE, format_headers = TRUE)
```
...
# Model estimation
results_sem_2025 <- as.matrix(results_sem_2025)

fit <- cfa(SI_model_tr, data = results_sem_2025,
             ordered = c("age","si_type", "sib_freq","meds_count", "binge_fr", "deprawsc",
                         "anx_score", "aaq_tot", "dep_impa", "gad7_impa", "flourish", "brs_tot"))

fit_2025 <- sem(SI_model_2025, data=results_sem_2025)

summary_2025 <- summary(fit_2025, fit.measures=TRUE, standardized = TRUE)

list(summary_2025$coefficient,
...
fitmeasures(fit_2025)

# model diagnostics
fitted(fit_2025)
inspect(fit_2025, what="cor.all")
...
resid(fit_2025, "cor")
plot_matrix(residuals(fit_2025, type="cor")$cor)

# model prediction
fit_2025$prediction <- predict(SI_model_test, newdata = results_sem_2025, type = "response")

# distribution of the prediction score grouped by outcome
ggplot(fit_2025, aes(prediction, color = as.factor(left) ) ) +
geom_density(size = 1 ) +
...
modificationindices(fit_2025, minimum.value = 20)
fit_2025_2 <- update(fit_2025, add="nox ~ log_crim")
summary(fit_2025_2, fit.measures=TRUE)
...
```{r SEM path diagram}
# pathway output
semPaths(fit_2025_2, what='std', nCharNodes=6, sizeMan=10,
...
```{r parameter_est}
#train
parameterEstimates(fit_2025)
fitted(fit_2025)
resid(fit_2025)
fitMeasures(fit_2025, c("cfi","rmsea","srmr"))
lavInspect(fit_2025, what = "start")
lavInspect(fit_2025, what = "list")
findRMSEApower(rmsea0, rmseaA, fit_2025, n, alpha = 0.05, group = 1)
```
